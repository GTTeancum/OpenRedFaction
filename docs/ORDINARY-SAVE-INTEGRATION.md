...
## Live cross-level undercover quick-load (2026-09-26)

The image-free 280-frame L8S1 replay enables the scientist disguise through authored event6447, saves at replay frame60, walks into exit5625 at frame126, then requests a quick-load from L8S2 at frame180. The load transitions back to L8S1 at frame181 and restores the48996-byte ordinary snapshot. PC and stock64MiB Xbox match both transitions and final player form, model and weapon ammunition; the native save/load status is successful and the minimum sampled free memory is5608 pages. The repeatable PC regression is `tools/replay_undercover_handoff.py`; the Xbox state-only run is `artifacts/xemu/render-20260926-152324/report.json` using `--no-images`. The harness now accepts scene-local quick-load log frames after a level exit while checking the scheduled replay transition separately. This confirms one live cross-level return with undercover state, not arbitrary level pairs, user-facing slot selection or visual correctness. No image was captured.

## Planted remote-charge continuation (2026-09-26)

The ordinary loader now accepts active remote charges that have no moving NPC/mover host. The existing remote codec validates identities and stages the charge before the ordinary transaction publishes it; moving-host charges still reject because host poses have not yet been committed. An unchanged static-world CTF06 fixture saves one planted charge, quick-loads, then detonates it with no new throw. PC and stock64MiB Xbox pass the image-free380-frame replay with matching remote counters and ammunition; the saved remote section is276 bytes and the minimum sampled native headroom is3088 pages. `tools/check_ordinary_remote_reload.py` reproduces the PC flow, and `artifacts/xemu/render-20260926-153502/report.json` records the native comparison. This does not cover active terrain destruction or arbitrary campaign scenes.
