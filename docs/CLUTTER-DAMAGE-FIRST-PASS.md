...
PC replay breaks UID13025 at frame72 after80→40→0 health. The inspected frame73 image shows the authored flame burst at the removed lamp; frame76 and frame89 show it fading away. The effect is deliberately not a claim of complete break presentation: the vclip's60 sparks, genericlight debris, other vclip classes, corpse substitution and live chain-reaction presentation remain open. Audio is dispatched but has not been auditioned. Native run render-20260918-204953 passes all harness comparisons at74frames on stock64MiB, with4,185 free pages (16.35MiB). The inspected Xbox framebuffer shows the same authored flame burst, exact damage/profile/binding counters match PC, and disc restoration passes. The earlier SWITCH_DETAIL diagnostic fix is also verified by this pass.

## Authored prop blasts and chains

Class preparation now resolves each used vclip damage value once and retains the original world-wrapper magnitude: vclip damage × class explosion radius × class explode-damage multiplier. No-effect classes retain zero blast magnitude. The binary evidence is docs/research/secondary-re/campaign-explode-world-20260916.md (436490); source remains invalid handle and kind3. The installed lamps have zero vclip damage and therefore still produce a visual without an invented damaging blast. Oil Drum/gas_can use30×3×1=90 damage and radius3.

Deferred health-break processing applies shared-class50ms cooldown, optional visuals, then shared covered radial damage. Newly killed props set their existing one-shot pending flag; the bounded owner scan never recursively invokes itself. Later owners can break in the same tick, earlier owners on the next tick. Visual-resource availability does not suppress gameplay damage.

The real-scene focused test uses installed Oil Drum/gas_can descriptors and vclip values in an empty collision world. Positions0,2,4 prove two blasts: the first kills the middle prop, and the middle kills a third outside the initial radius. The third shares the first class and is cooldown-suppressed. Repeating the update emits no new blasts. This is simulation evidence, not an in-level oil-drum visual test. PC lamp replay still emits the visual with no blast; PC/NXDK builds pass. Profiles now consume34,480 bytes for the431 classes, plus10,120 bytes of instance bindings.

## Solid-prop admission and live chain

Weapon selection previously required only class collide_weapon bit2, incorrectly excluding Oil Drum/gas_can classes that author collide_object bit4. Ordinary, precision and projectile queries now admit either flag before their existing exact geometry/cover/identity checks. This is the practical shared weapon policy; the earlier49c690 multiplayer-wrapper evidence establishes hidden-object and nearest-hit rules, not complete SP flag equivalence.

`tools/check_clutter_chain.py` prepares three original class/model assets at isolated CTF06 positions without modifying installed data. A measured downward aim puts the first drum under the reticle. The PC baseline preserves the visible drum/can; one frame40 pistol shot applies40 damage to UID910001, the90-damage radius3 blast applies30 to gas_can910002, and its blast applies30 to drum910003 outside the first blast radius. Exactly three props retire. Inspected baseline/fire images confirm the visible objects disappear. The third owner is established by target-specific logs, not claimed visible in the camera. Oil-drum explosion graphics/audio remain absent; yellboom lamp visuals are the only current break presentation binding. Native builds pass; this chain fixture has not yet run in XEMU.

## Oil-drum explosion presentation

The bounded break-effect owner now supports two independently demand-loaded families: yellboom/rocket hit and oil drum explode/generic. Each has eight active instances, its authored central emitter recipe and its own texture indices; both can coexist without bitmap aliases. Oil-drum central materials require98,588 bytes (three textures) under their128KiB ceiling. Foley names are loaded from the original vclip rather than hardcoded. Generic explosion sparks/trails and prop debris remain separate.

The same live three-prop PC chain now produces visible lingering authored smoke in the inspected frame89 image. Its damage/retirement trace remains unchanged. Sound is submitted through the existing audio service but has not been auditioned. Native run render-20260918-205945 passes every harness comparison at46frames, with4,372 free pages (17.08MiB) on stock64MiB. The inspected Xbox framebuffer shows both authored central explosion flashes; exact counters confirm one shot, three prop retirements and two radial blasts. Disc restoration passes. Audio playback has not been auditioned.
