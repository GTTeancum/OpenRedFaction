...
### Steering rotation ownership and body filter (2026-09-13)

Original433c80 copies its input and clears each component whose movement descriptor rotation reference (14/18/1c) equals 0 or 1; all other component bits survive. rf_movement_body_rotation reconstructs that full helper without allocation and supports aliased output. This stage is called by49fe40 before body angular integration. verify_body_rotation.py executes unhooked original433c80 against PC and NXDK across2048 cases, all64 valid reference combinations, arbitrary other reference values and arbitrary float bits (including negative zero and nonfinite payloads). NXDK separate/in-place outputs and three null guards pass. Both builds and22 CTest checks pass.

Integration findings: actor150 maps to existing body.state.vector_c8 because the original body starts at actor88; actorE4 maps to body.state.position. Movement descriptor14/18 are rotation[0]/rotation[1]. At42325d..423267 NPC creation copies actor48 body orientation into actor7e0 eye orientation after4194e0 eye-position update. The current scene retains eye position but has no separately updated NPC eye orientation. Original49fe7a copies orientation into7e0 then applies eye-angle rotations;49cf40 updates/clamps retained eye angles before that stage. Do not substitute a permanently frozen spawn matrix for ongoing eye orientation. These ownership/update bindings and full follower/arrival services remain open; no new live NPC steering or native XEMU evidence is claimed here.


### Full eye-angle update49cf40 (2026-09-13)

rf_eye_angles_step reconstructs49cf40 with resolved class flags; actual42d7b0 tests class724 mask401200. When enabled, original49cf55..49cf80 computes exp(-3*frame_seconds) via x87 and stores the factor as float. It multiplies offset894 by that factor, adds vector870, then clears offset898 to positive zero. Independently it adds delta888 to angles87c, clamps each component with the ordered40a4c0 comparisons against1434/1440 limits, then adds/subtracts one float turn6.2831854820251465 only outside the inclusive +/-turn range. It does not loop or use modulo. Source/delta/limit fields are preserved.

verify_eye_angles.py executes the entire original routine, including actual predicate, x87 exponential and vector/clamp callees, with no hooks. All72 retained bytes match compiled PC and NXDK across2048 cases, including individual/combined flags, reversed limits, boundary angles and varied frame times. Six PC/NXDK invalid-input guards preserve state. Both builds and all22 CTest checks pass. This adds no scene allocations; retained NPC owner binding, per-frame scheduling, matrix rebuilding and native XEMU verification remain open.


### Physics eye-matrix composition (2026-09-13)

rf_eye_physics_orientation reconstructs49fe7a matrix composition: copy physics orientation; sum float pitch offset894+angle87c and call4fd240; sum float roll offset89c+angle884 and call4fd310. The first rotates up around right then rebuilds forward=right cross up; the second rotates up around forward then rebuilds right=up cross forward. Both use4fb210: normalize axis through4fcea0, align with its strict near-vertical threshold, transform to local coordinates, rotate about local Z, and transform back. No eye yaw is applied here. Shared code reuses the existing verified x87 normalization/cross/trigonometric helpers.

verify_eye_physics_orientation.py executes full unhooked4fd240 and4fd310 including all their callees, and compares all9 matrix floats to compiled PC/NXDK for2048 inputs. Coverage includes identity and nonunit bases, vertical-threshold axes, zero angles and in-place output. Three PC/NXDK invalid-input guards preserve output. Both builds and22 CTest checks pass. This covers the matrix composition, not all49fe40 branches or live NPC stepping. NPC retained eye-angle/matrix ownership, constructor binding and scheduled physics integration remain open; no native XEMU or new visual evidence in this change.


### Retained NPC look and steering services (2026-09-13)

NPC body owners now retain136 bytes of look state: eye-angle fields/limits, body angles, eye orientation, rotation command708 and turn clock7b0. Startup uses verified422e2c..422e82 spawn-angle extraction, copies class6c/78 limits to1434/1440 (423910..423951), and initializes eye orientation from authored body orientation (42325d..423267). Pending vectors start clear in the existing zero-initialized scene owner; full subsequent physics production of870/888 remains open. Budget accounting includes sizeof each owner.

rf_scene_npc_eye_angles_step resolves a live registered owner, runs49cf40 and the physics eye-matrix composition on local values, then publishes retained angles/orientation. rf_scene_npc_steer resolves authored class60 rate, class724 flags, movement rotation references14/18, body positionE4, eye position7d4 and retained eye bases7e0/7f8; it publishes verified40cb50 output to existing body.vector_c8 (actor150), command708 and clock7b0. Stale handles leave result/state untouched. These services require caller scheduling; no autonomous AI destinations, integration timestep or angular delta generation is fabricated.

The opt-in frame0 NPC look fixture snapshots/restores each owner, supplies known eye deltas, checks retained angle/matrix updates, steers toward an offset target, and checks stale handles. PC and stock64MiB XEMU replay-20260913-050832 pass180 frames with matching NPC_LOOK_TEST [38,38,38,1164703586,5304,0]. Memory base67108864, plugged0;39 allocated owners add5304 bytes. NPC_BODIES [39,38,114,42660,416652,1174174919]. Existing replay assertions and22 CTest checks pass; harness restored/rebuilt normal Xbox image. This validates retained service ownership/composition, not continuous NPC physics, route following or new visible gameplay.


### Composed angular prediction49f566 (2026-09-13)

rf_angular_predict reconstructs the ordinary49f566..49f634 stage after angular velocity preparation. Scale actor150 by timestep1b0, filter body delta870 using433c80, add to body angles864 with49f2a0 pitch clamp +/-pi/2 and single yaw/roll wrap +/-2pi, build current/predicted4a0d70 matrices (actorFC/120), and select eye delta888 with433d30 rotation reference exactly1. It leaves incoming angles/velocity unchanged and returns all108 bytes of predicted state. No allocation. Preceding49f3c0 dispatch, damping/forces and subsequent collision/commit remain outside this stage.

verify_angular_predict.py runs the original instruction interval with all callees intact, comparing2048 cases. NXDK all108 bytes match exactly. PC body/eye deltas and angles match exactly;155 cases differ only in tiny matrix residuals near pitch clamp poles, maximum absolute difference4.385608375615706e-17, tested tolerance1e-15. This is not a bit-exact PC matrix claim; the residual discrepancy between native PC execution and the Unicorn original oracle remains recorded. Four PC/NXDK invalid-input guards preserve output. Both builds and22 CTest checks pass. No scene/native XEMU binding was added in this change.


### Ordinary angular velocity preparation (2026-09-13)

rf_angular_velocity_step covers49f485/49f527 through49f565 with the drive predicate supplied. Driven response factor is float(1-exp(dt/((class60/class64)/ln(0.05)))); the original computes ln(0.05) from qword589880 with x87 fyl2x. Target command708*class60, target-minus-velocity and scaled delta each retain original float stores before addition to actor150. Undriven damping uses float(1-dt*float0.8999999761581421). Actor174 force is divided by actor98 mass IN PLACE and added to angular velocity. Body flag1000000 bypasses this preparation, matching its upstream skip.

verify_angular_velocity.py executes driven or undriven original instruction paths with actual x87 exponential and vector callees; the drive predicate is resolved by the fixture.2048 original/PC/NXDK cases match all24 state bytes, including low-byte drive values and unchanged bypass state. Seven invalid reached-input guards preserve output. Both builds and22 CTest checks pass. The current positive rate/acceleration/mass contract excludes singular inputs. Scene binding must supply original drive predicates48aa30/4895d0 and select ordinary versus special branches before using preparation/prediction. No new native replay or continuous movement is claimed.


### Angular dispatch precedence and drive-source audit (2026-09-13)

rf_movement_angular_route preserves49f3e4..49f446/49f636 precedence: body8000 and nonprimary ->484650; body4000 ->49e180; movement descriptor index15 ->49e050; byte720 or body80 ->49de50 unless body1000000 skips angular work; otherwise ordinary preparation/prediction. Ordinary flag1000000 skips velocity preparation but still predicts; angular SKIP does not skip subsequent translation. Caller primary input represents pointer identity with5cb054.
...
### Automatic SP angular drive binding (2026-09-13)

rf_scene_npc_prepare_angular now resolves drive state through48aa30 using the registered SP player object handle and existing linked_handle. A missing player registration supplies an empty list; stale/type-invalid entries are rejected by the shared lookup. Body flag1000000 bypasses lookup and velocity preparation while preserving prediction. The caller no longer supplies a drive boolean. The ordinary angular branch remains a caller precondition; this does not schedule all49f3c0 branches.

NPC_DRIVE_TEST snapshots each NPC and the player link, supplies nonzero rotation command/velocity/force, tests unlinked and linked player states through the real scene lookup, compares prepared output against the verified numeric routines with independently selected expected drive0/1, then restores both owners. Replay-20260913-053057 passes180 frames on stock64MiB XEMU; telemetry [38,38,2162879618,0] matches PC. Base memory67108864, plugged0. Runtime NPC memory unchanged; diagnostic counter adds16 static bytes. Both builds,22 CTest checks and existing native replay assertions pass; normal Xbox image restored/rebuilt. Special angular dispatch, continuous physics scheduling, collision and movement commit remain open.


### Authored opening-level angular branch audit (2026-09-13)

The entity seed probe now exports SEED_PHYSICS per UID/class with creation flags, class724/728 flags, use kind and movement index. audit_authored_angular_routes.py loads installed movement descriptors and class seeds, applies shared SP nonprimary constructor flags, enabled descriptor fallback and verified angular dispatch. L1S1:78 ordinary records; L1S2:38 ordinary plus1 Driller01 selecting49e180; L1S3:26 ordinary plus2 APC selecting49e180. Total142 ordinary/3 rigid-body among145 records. Body flags are0x80000078 for these ordinary spawns and0x80004070 for rigid-body spawns before later initialization adjustments.

This profiles authored entity records, including records outside current registered skeletal NPC coverage, and does not prove runtime branch behavior after script/mode changes. The evidence supports prioritizing ordinary49d0a0 collision/commit for opening campaign actors while retaining49e180 and vehicle ownership as required work. PC probe build and three installed-level seed/budget audits succeeded. Only the diagnostic probe/audit changed; no core runtime or new XEMU execution in this change.


### Ordinary motion commit49d0a0 (2026-09-13)

rf_ordinary_motion_commit reconstructs the complete49d0a0 path with body4000 clear: accept predicted positionF0 intoE4; run49cf40 eye-angle update; apply body delta870 to864 using49f2a0 clamp/wrap; construct bodyFC and eye7e0 matrices via4a0d70; copy body orientation to published48 and predicted120; rebuild world tensor with49cd30; set fraction1cc to1; rebuild190/19c bounds from position and radius180. It rejects the rigid-body branch explicitly. Caller must have accepted collision movement; no collision query or object position3c publication is implied. Local464-byte staging preserves state on errors and allocates no heap storage.

Matrix calculation is factored from the existing look helper so combined body/eye angles can exceed the player-look API domain, as the original commit permits. Existing public look input validation is preserved. verify_ordinary_commit.py executes full unhooked49d0a0 with its actual angle, matrix, tensor and bounds callees across1024 cases. NXDK matches all464 retained bytes exactly. PC nonmatrix fields match exactly;247 cases have tiny matrix residuals, max4.385608375615706e-17, within explicitly recorded1e-15 tolerance. Three failure guards preserve state. Both builds,2048 angular-prediction regressions,2048 eye-matrix regressions and22 CTest checks pass. No scene/native replay binding added yet; accepted collision and render-pose publication remain open.

### Retained ordinary commit and live NPC orientation (2026-09-13)

rf_scene_npc_commit_ordinary stages the retained body, eye angles, body angles and matrices through verified49d0a0, then publishes the accepted body and live model basis. It introduces no retained allocation. Collision forward, visibility matrix, death clearance, volume basis and eye-position input now read that live basis instead of immutable spawn orientation; construction initializes the basis before first eye refresh. Object position3c publication, room/eye refresh after movement, collision acceptance and continuous scheduling remain subsequent work. This service alone does not make NPCs walk autonomously.

The restored-state drive fixture now checks preparation followed by commit for38 driven and38 undriven NPC cases, including body/eye/angle/matrix publication and stale handle rejection. Replay-20260913-054242 passes180 frames in XEMU with base memory67108864 and plugged memory0; NPC_DRIVE_TEST [38,38,830159860,0] matches PC. Normal Xbox image restored/rebuilt. PC collision fixture uses distinct authored/live forward vectors; death-clearance fixture supplies a live model owner. Both builds and all22 CTest checks pass. Post-replay edits were test fixtures and a corrected source comment only.

### Completed NPC position publication binding (2026-09-13)

rf_scene_npc_publish_position binds verified487962..487973 to the retained body, published object position and object flags, then synchronizes registry flags and model position. The underlying48a230 behavior copies bodyE4 to object3c and pendingF0, rebuilds bounds, sets object04000000 and clears body40000000. No allocation is added. The caller must establish physics substep completion; this is deliberately separate from ordinary49d0a0 commit. Existing later room and eye passes consume the published position. The source487770 loop runs prediction, pair collision48ca60, solid collision49bb70 and49fe40 before deciding completion/list removal; partial collision resolution and continuous scheduling remain open.

The native drive fixture now supplies nonzero pending displacement and body40000000, commits and publishes for38 driven and38 undriven actors, checks complete body/public position/model position/flags against the verified helper, rejects stale handles, then restores actor and model state. Replay-20260913-055021 passes180 frames at base memory67108864, plugged0; NPC_DRIVE_TEST [38,38,4216523580,0] matches PC. Both builds and22 CTest checks pass; normal Xbox image restored/rebuilt. No autonomous movement or new visual milestone is claimed.

### Ordinary partial-contact motion prefix (2026-09-13)

rf_ordinary_motion_partial reconstructs49ffd2..4a011b for ordinary body4000 clear and contact fraction1cc in[0,1). Compute remaining time from original time1b0 and incoming fraction BEFORE the contact backoff. Build pending-minus-current displacement, measure length through40a000 ordering, and unless400000 is set reduce fraction by0.05000000074505806 world units divided by length, clamping to zero. Advance bodyE4 by the corrected fraction; rotate body864 by filtered angular150 times corrected fraction times1b0; rebuild bodyFC and world tensor through4a0d70/49cd30. Eye, published and pending matrices, bounds and unrelated state remain untouched. Output remaining time is separate because the later contact dispatch can replace it before publication. No heap allocation.

verify_ordinary_partial.py executes original prefix with all numeric callees unhooked across2048 cases including zero/short displacements, both backoff flag states, descriptor rotation modes and varied times/fractions. NXDK matches all464 retained bytes plus remaining time exactly. PC nonmatrix state matches exactly;34 cases differ only in tiny matrix residuals, maximum4.385608375615706e-17 under1e-15 tolerance. Five invalid-input guards preserve state and output on PC/NXDK. Both builds and22 CTest checks pass. This adds no scene binding or new XEMU claim. Remaining49fe40 tail: body20000000 bypass/clear; otherwise427550 contact dispatch, zero return cancels remaining time, return2 invokes49d7e0, then set10000000 and publish1b0. Full427550 has gameplay/object-type effects and must not be replaced by a constant response.

### Ordinary49fe40 orchestration (2026-09-13)

rf_ordinary_motion_resolve composes full ordinary49fe40 with explicit427550 contact and49d7e0 response services. Fraction>=1 runs49d0a0 and publishes previously calculated remaining time. Partial motion runs the verified prefix; flag20000000 bypasses callbacks and is cleared. Otherwise contact return0 cancels remaining time, return2 invokes response, and other full-width decisions retain it. Final flags are reread after callbacks, OR10000000, then time1b0 is overwritten with the saved remaining time. Callback mutations to body/velocity persist, while callback time writes are overridden only on successful completion. Callback errors retain preceding effects and skip final flags/time. The rigid branch is rejected. No allocations or live scene scheduling added.

verify_ordinary_resolve.py executes full original49fe40 ordinary control flow with all numeric callees unhooked; only427550/49d7e0 are supplied callback boundaries, with identical flag, velocity and time mutations for PC/NXDK. Across2048 cases:1433 no-callback,492 contact-only,123 contact+response; all464 state bytes, remaining time and callback order match NXDK exactly. PC nonmatrix state matches exactly;193 matrix-residual cases have maximum4.385608375615706e-17 under1e-15 tolerance. Four callback-failure cases verify preceding mutations survive and subsequent stages do not run. Both builds and22 CTest checks pass. No new XEMU or full427550 gameplay implementation is claimed. Harness hooks are restricted to the four callback addresses to avoid per-instruction Python overhead.

### Actor-to-actor contact decision41a000 (2026-09-13)

rf_entity_actor_contact_decide reconstructs41a000 response and429790 destruction request with explicit resolved42a130 occupant predicate. Use-kind1 against42cca0 armor immunity responds immediately. Otherwise kind1 against mass<30 returns false and requests destruction when linear or angular velocity squared is positive. Class8000 with contact kind3 uses squared threshold0.5, with class200/occupant fallback. Other cases respond unless target object7c bit8 identifies a player. Finite reached numeric inputs are required; unused numeric fields are not validated. No destruction/damage is executed here and no heap allocation is introduced. The427550 caller must execute requested destruction before returning1+response.

verify_actor_contact.py executes full41a000 with real429990/486c90,42cca0,40cac0,4895d0 and40a180; supplies42a130 predicate and observes429790 destruction at its boundary. All2048 original AL response/destruction requests match PC/NXDK:541 no-response/no-destruction,916 response,591 destruction requests. Three reached-NaN guards preserve outputs. Both builds and22 CTest checks pass. No new XEMU/live collision claim. Full427550 surface/item/vehicle effects,429790 destruction and retained contact binding remain open.

### Contact destruction429790 ordering (2026-09-13)

rf_entity_contact_destroy issues4892c0 to the current handle with amount1000000, source-1, hit region-1, kind9, argument6=0, auxiliary-1 and force0. After damage, it rereads the backend-refreshed class174 sound selector, calls434da0, then plays the returned identifier without filtering at the current published position3c with volume1, default173c378 parameters and flags0. Owners remain alive; callbacks refresh mutable fields. Failure stops later effects without rollback. No heap allocation or extra retained scene state.

verify_contact_destroy.py executes complete429790 with explicit damage/selection/playback boundaries and compares1024 cases to PC/NXDK. Mutation cases replace the original class pointer during damage and update position again during selection, verifying the reads occur after callbacks. All damage arguments, selected identifier, position, volume/defaults/flags, final actor fields and ordered trace hashes match exactly. Six PC/NXDK callback failures preserve preceding effects and skip subsequent calls. Both builds and22 CTest checks pass. No new XEMU/live destruction claim; retained class174 selector and scene damage/audio binding remain open, together with other427550 contact branches.

### Retained class174 squash sounds (2026-09-13)

Original41cad9 reads string5955d8 "$Squash Sounds:";41cb03 calls434cb0 and41cb0b stores class174. Missing-tag41cb13 writes ESI=-1 (last assignment41c45e). New rf_entity_squash_sound_group_read shares the single-sound parser with impact-death groups but resets missing squash tags to-1; impact missing-tag preservation is unchanged. Scene startup owns one int32 per loaded class, releases it with audio owners and includes it in retained Foley memory. No duplicate impact/squash selector is inferred.

verify_pain_groups.py --squash passes all63 installed classes and6 synthetic/error cases on PC/NXDK against independently extracted labels and actual original434cb0 name resolution;20 classes declare nonmissing groups. The full original class parser is source-audited, not execution-verified here. Existing --impact regression and22 CTest checks pass. Initial native run caught the harness old52-byte per-class accounting; updated it to56 for the added int32. Replay-20260913-061623 passes180 frames with base67108864, plugged0 and Foley retained26620 bytes (three classes,12 added bytes). Both builds pass and normal Xbox image is restored/rebuilt. Damage/destruction callback and positional audio binding remain next; no new gameplay visual is claimed.

### Retained contact destruction and squash playback (2026-09-13)

rf_scene_npc_contact_destroy binds429790 to registered NPC damage, post-damage class174/published-position reload, shared Foley selection and positional playback. Supplied damage effects must retain lifetime and publish mutations synchronously. No continuous collision scheduling is added. The opt-in damage fixture uses L1S1 UID8456, temporarily raises health to2000000, compares the complete retained owner against a direct damage-service application, checks stale-handle preservation and published audio position, then restores the owner. Its notification backend suppresses downstream death/pain effects; this tests service binding and audio, not complete lethal gameplay.

Replay-20260913-063142 passes180 frames with base67108864 and plugged0. Contact test [2,1240736768,1232349419,0] represents health2000000 to1000078.6875 before restoration. Audio [1,1,1,1,22478,154,2745024,0,1249339151,...] matches PC including position, selected sample, loaded bytes and RNG. Both builds and22 CTest checks pass; normal Xbox image restored/rebuilt. Full427550 dispatcher and per-frame collision scheduling remain open.

L1S1 regression maintenance: visibility records had been appended to the collision-query hash, invalidating its independent archive-backed reference. Keep the original query hash and add a separately compared object-count/visibility-placement hash [168,2804948639]. Independent verify_clutter_scene_collision.py now passes with unchanged expected query hash2476729641. Door sound-bank expected loaded bytes now include existing impact2380 and new squash22478 loads; the measured total375768 matches the sum. Baseline L1S2 replay passed but did not enable the damage fixture; the above explicit L1S1 run is the evidence for the new binding.

### Registered NPC actor-contact dispatch (2026-09-13)

rf_scene_npc_actor_contact reads both registered NPC owners, class use-kind/flags, source contact material and linear/angular velocity, target mass/armor/814/object flags, then calls verified41a000 decision. Requested destruction runs the retained damage/squash service before publishing response. Stale handles and failed destruction preserve the output response; already-applied effects are not rolled back. No new allocation. The42a130(source,-1) predicate is source view810 bit16, not an occupant-list scan; verify_actor_contact.py now executes1024 original direct-flag cases in addition to2048 decision comparisons and3 guards.

L1S1 has no registered kind1 source for the light-target crush branch. The explicit fixture temporarily sets one source class use-kind1, target mass1/armor0 and high health, with stationary then moving source. It restores both NPC owners and the class value. Four checks cover stale source, stale target, stationary no-damage/no-audio, and moving contact routed through retained destruction. Death/pain downstream notifications remain fixture-suppressed, so this does not prove lethal gameplay. Replay-20260913-063755 passes180 frames at base67108864, plugged0; test [4,1240736768,1232348160,0] and audio [1,1,1,1,22478,154,2745024,0,1249339151,...] match PC. Both builds and22 CTests pass; normal Xbox image restored/rebuilt. Player targets and other object families, full427550 dispatcher and continuous collision scheduling remain open.

### Object-contact427550 dispatch branch (2026-09-13)

rf_entity_contact_object_dispatch implements427550 with actor1ec zero and contact1d4 nonzero. Lookup missing ->2. Type4 current-class match through410c70/class298 versus5afb78 ->1 BEFORE player check. Player object7c bit8 ->2. Type0 invokes actor contact and maps its LOW BYTE to1+boolean; type1 invokes pickup(target,source,1,0) and returns1; types3/4/7/9 return2; other types return1. Stable borrowed views and explicit lookup/current-class/actor/pickup callbacks; no allocation. Errors stop at the reached callback and preserve output decision. This is not the zero-inverse-mass surface branch or nonzero1ec branch.

verify_contact_object.py executes full original427550 for this entry condition with actual4895d0 and branch/switch logic, supplying40a0e0/410c70/41a000/459560 boundaries.2048 cases match PC/NXDK decisions and ordered callback arguments exactly:1615 return2,433 return1. Actor return values0/1/2/255/256/257 test low-byte mapping independently of object type. Four reached-callback failures preserve decision and skip later calls. Both builds and22 CTest checks pass. No new XEMU scene-binding claim. Object lookup and pickup ownership plus remaining surface/vehicle effects and continuous scheduling remain open.

### Surface collision sound4278e0 (2026-09-13)

rf_entity_contact_sound gates on velocity144 squared >= speed8c0 squared times float0.2, then queries voice1478 through505c00 (full-word active result). If inactive, read published forward60 and contact normal1c0 using Z+Y+X dot ordering; dot must be strictly below float-0.4. Select current global5cc4f8 group through434da0, play at supplied position with volume1/default173c378/flags0, and publish the returned voice ID without filtering. Callback-visible direction/position/group changes are retained; errors stop later calls and do not publish a new voice. No heap allocation or live scene voice owner added.

verify_contact_sound.py executes full original4278e0 with actual vector arithmetic and supplied playing/selection/playback boundaries.2048 PC/NXDK cases match exactly:354 skip before callbacks,1126 stop after voice query,568 play. Three callback failures preserve the previous voice, and three reached nonfinite-input failures preserve state. Both builds and22 CTest checks pass. Remaining work: retain actor1478, identify/retain global collision sound selector, bind voice lookup/playback and complete427550 surface dispatch. No XEMU gameplay claim is added here.

### Retained surface collision sound (2026-09-13)

Original41b7a7 loads the "Sub Hit" name at594e8c, calls434cb0 at41b7be and stores the group to5cc4f8 at41b7cb. Actor construction423ada initializes1478 to-1. The scene now resolves that global group once after Foley initialization and retains a signed voice ID per NPC (four additional bytes per slot). The prepared rf_scene_npc_contact_sound service reads movement.speed (8c0), body velocity, live published model forward and contact normal, calls verified4278e0, and publishes the returned device voice. It uses the existing generation-aware mixer lookup, Foley RNG selection, asset loading and positional playback. Existing pain callers still discard their returned voice.

The optional L1S1 damage fixture restores the NPC owner after checking stale-handle rejection, stationary suppression, one positional impact voice and suppression while that voice remains active. Replay-20260913-065157 passed180 frames on base67108864/plugged0 with contact test [4,3,2745024,0] and audio [3,1,1,1,73512,1033,2745024,0,4242392777,...], exactly matching PC. The sample adds73512 loaded bytes; allocated NPC body storage is84436 bytes for78 owners. A final replay after moving the global lookup outside the class loop is recorded below when complete. This service is not yet automatically scheduled: full427550 surface/vehicle dispatch and continuous NPC collision stepping remain required.

Final replay-20260913-065839 passes180 frames after global initialization correction, with the same four contact cases, one voice selection/play/load and exact PC telemetry on stock64MiB. Both builds and22 CTest checks pass; the harness restores and rebuilds the normal Xbox image.


### Surface-contact route gates (2026-09-13)

rf_entity_contact_surface_route reconstructs427550 entry with actor1ec=0 and contact1d4=0. Class724 bit200 selects the APC branch and prevents the bit400 Driller branch even if the APC gates fail. APC requires speed8c0 squared times float0.9 strictly less than velocity144 squared, then published forward60 dot contact normal1c0 strictly below float-0.9. Driller requires bit400, material1d0=8 and VELOCITY144 dot normal1c0 strictly below-3.0 (not forward dot). Dot order is Z+Y+X. Failed vehicle gates fall through to sound only for use-kind1; otherwise no effect. Reached nonfinite inputs preserve output and return RF_RANGE. This selector does not replace the required effect bodies or return a fake completed collision response.

verify_contact_surface.py executes original427550 and actual40cac0/40a270/429990/486c90/40a180/40a0b0, stopping at the selected effect boundary.4170 original/PC/NXDK cases match exactly:2302 no effect,774 sound,937 APC,157 Driller. Includes exact/adjacent dot thresholds, equal zero speed gate and combined class flags; six reached nonfinite guards pass compiled PC/NXDK. Both builds and22 CTest checks pass. No new native scene-binding or visual claim. Required next: APC bumper tag/transform and debris with75 damage, Driller geometry effect with100 damage and linked-player feedback, timed nonzero1ec branch, then full contact orchestration and continuous physics scheduling.


### Driller contact geometry/damage ordering (2026-09-13)

Assembly correction:40a490 is only a thiscall accessor returning object word0 (room pointer). Ghidra associates surrounding pushed arguments with that accessor incorrectly; those arguments remain for467020. The final467020 value0x10 (Driller) or0x1e (APC) is a flag field, not a duration. Fresh467020 analysis confirms seven arguments: scale, source handle, room pointer, position pointer, direction pointer, model, flags.

rf_entity_contact_driller_effects reconstructs42772b..42776a after successful Driller route selection. It emits467020(radius+radius,handle,room,contact1b4,normal1c0,0,0x10), then rereads the handle and emits4892c0(handle,100,handle,-1,0,0,-1,0). Geometry return is not a gameplay gate; adapters report transport/backend failure separately. Owner must remain alive and callback changes persist on error. Finite radius/overflow guards stop before effects. No allocation.

verify_contact_driller.py executes that original prefix with actual40a490 and observes the geometry and damage boundaries.1024 cases match PC and compiled NXDK argument words, ordering and refreshed handle after geometry; two callback failures and two radius guards pass. Both builds and22 CTest checks pass. Scope excludes467020 implementation, damage backend internals, the following linked-player feedback loop and native scene scheduling. Full geometry ownership and that feedback loop remain mandatory.


### Player contact direction and accumulation (2026-09-13)

rf_player_contact_direction reconstructs resolved4a5a20: missing player/actor ->0; world-normal X=Z=0 ->15 before orientation transform. Otherwise4fac60 projects normal onto actor48 rows, stores binary32 components, then selects Z when abs(X)<abs(Z), else X (ties included). Positive Z ->4, nonpositive Z ->1; positive X ->2, nonpositive X ->8. rf_player_contact_mark reconstructs4a5af0: OR (direction&15)<<13 into player10, preserving accumulated directions and unrelated bits; missing player is a no-op. Direction finite-input errors preserve output. No retained storage added.

verify_player_contact_direction.py executes complete original4a5a20/4a5af0 with actual copy/4fac60 numeric transform and supplied actor lookup.2048 original/PC/NXDK cases pass: missing player, vertical vectors, oblique matrices, ties, arbitrary direction high bits and existing player flags. Three reached nonfinite guards pass. Both builds and22 CTest checks pass. This implements the numeric/flag callees required after Driller contact; the player iteration, linked-handle check, camera effect ownership and live binding remain open.


### Driller linked-player feedback loop (2026-09-13)

rf_entity_contact_driller_feedback reconstructs42776a..4277e0 after geometry/self-damage. Iterate the signed global player count, resolve each player14 actor, compare actor200 to current source handle, then shake actor1430->c4 camera with strength bits3f7eb852 and duration3f000000. Reread the current list slot for4a5a20 direction using source normal1c0, then reread it for4a5af0 mark with the full returned word. Player count is reread only at iteration end; changing it to zero during shake does not skip direction/mark for that iteration. Stable borrowed owners/list, no allocation, bounded capacity. Callback errors preserve completed effects and stop later stages.

verify_driller_feedback.py executes the original tail at lookup/shake/direction/mark boundaries.1024 PC/NXDK comparisons match exact ordered arguments for signed empty counts, present/absent and linked/unlinked actors, arbitrary returned direction words and player count mutation during shake. Four reached callback failures and one capacity guard pass. Both builds and22 CTest checks pass. Numeric direction/mark callees were verified separately; retained scene player/actor/camera binding and composition after geometry/damage remain required. No new native gameplay claim.


### Retained Driller player-feedback service (2026-09-13)

rf_scene_npc_driller_feedback validates a registered NPC source and reads its retained contact normal. The stock single-player adapter supplies the registered player entity, resolves its current linked_handle, routes camera feedback through rf_scene_player_feedback into the rendered campaign_camera_effect, derives direction from the current player body orientation, and accumulates the owned player10 direction bits. These bits have a separate four-byte retained owner, reset on campaign initialization and included in the player memory telemetry; unrelated original player flags are not inferred. No heap allocation. Caller must establish Driller collision eligibility and invoke this after successful geometry/damage. Directional HUD consumption remains separate.

The optional damage fixture checks stale source rejection, no feedback when unlinked, linked vertical feedback (all four direction bits) and a further direction preserving accumulated bits. It restores the camera, player link, source normal and direction owner. Both PC and Xbox expose the eight-word fixture record for independent native memory comparison. Native result is recorded below after completion.

replay-20260913-071909 passes180 frames on base67108864/plugged0 with exact PC record [4,3,2,2,2,122880,600,0]: four cases, three lookups, two shakes/direction/mark calls, all direction bits, deadline600, no errors. Both builds and22 CTest checks pass; normal Xbox image restored/rebuilt. Full contact dispatch and geometry ownership remain open.


### APC surface-contact effects (2026-09-13)

rf_entity_contact_apc_effects reconstructs42764d..4276d8 after APC eligibility: lookup bumper_1 on current actor80 model; resolve Holey_APC.v3d; reread actor model and place the tag through5034f0 using published orientation48/position3c. Both temporary position and matrix start ZERO (actual409f90/4fbcd0); identity initialization was caught and corrected by original comparison. Then request467020(radius78,handle,room word0,placed position,published forward60,resolved model,0x1e), followed by4892c0(current handle,75,-1,0,-1,0,-1,0). Tagged placement orientation is an output but geometry direction is still actor forward. Backend errors stop later effects without undoing completed callback mutations; owners remain alive.

verify_apc_contact_effects.py executes original prefix with real zero constructors and40a490, supplying tag/model/placement/geometry/damage boundaries.1024 exact PC/NXDK cases verify asset names, initial placement outputs, full request arguments/order, model reread after tag selection and handle reread after geometry. Five callback-failure prefixes pass. Both builds and22 CTest checks pass. Actual asset/placement/geometry/damage services and live scene binding are excluded; these and the timed nonzero1ec branch remain needed for complete427550 orchestration.


### Timed nonzero1ec contact branch (2026-09-13)

rf_entity_timed_contact reconstructs427562..4275c6. Query timer1414 with verified4fa3f0; inactive/pending skips all effect reads. On expiry select global856854 when radius78<2.5, otherwise856858. Request4c16e0(effect,room word0,position3c,vector1b4,radius,-1,0,1), then set1414 to current clock plus2000 using verified4fa360. The contact vector is actor1b4, not normal1c0. Callback may change clock/deadline: setter reads the post-callback clock and overwrites its deadline mutation. Failures stop reset but preserve callback changes. Finite radius required only on the emitting path.

verify_timed_contact.py executes full original427550 with1ec nonzero, actual timer queries/setters and40a490 room accessor; only4c16e0 is supplied.2048 exact PC/NXDK cases include947 emissions, disabled timers, half-period/wrapped clock behavior, radius2.5 adjacent floats and callback clock changes. One emission failure preserves its completed mutation and skips reset. Both builds and22 CTest checks pass. Required next: identify/retain both effect globals, bind real4c16e0 emission and compose all427550 branches with retained contact ownership and continuous physics.


### Retained water-contact splash definitions (2026-09-13)

Original4c1332 pushes5a22fc water_splash_large, resolves4c1d00 and stores856854 at4c1346. Next5a2310 water_splash_huge resolves to856858 at4c1355. These are vclip slots43/44 in the installed table. The branch is water-splash handling; do not substitute a generic collision particle. Large uses WaterSplash01.VFX / Large Water Splash with authored VFX radius5; huge uses water_splash_huge.VFX / Huge Water Splash with radius10. Both have flag1 and no embedded particle emitter.

The scene retains both owned definitions plus vclip/Foley IDs after opening the catalogs:1016 bytes of static owner storage, exposed separately in memory telemetry. No VFX resource instance or sound playback is claimed. verify_contact_splash_assets.py executes original initialization plus actual4c1d00 against authored name slots; independent PC definition loads produce hash1070940602. replay-20260913-073303 passes180 frames on stock64MiB with retained record [43, 44, 483, 484, 1016, 1070940602, 1, 1] and the same definition hash. Both builds and22 CTest checks pass; normal Xbox image restored/rebuilt. Required next:4c16e0 VFX instance ownership/emission and composition with retained contact timers and full physics dispatch.


### Combined427550 contact routing (2026-09-13)

rf_entity_contact_dispatch composes the verified surface selector and object dispatcher with explicit timed/vehicle/sound services. Nonzero1ec takes precedence over all other contact fields. Otherwise finite inverse mass1d4 nonzero selects object lookup/response; zero (including negative zero) selects surface gates. No-effect surfaces and successful timed/vehicle/sound services produce response2. Object branch preserves its original1/2 result. Missing or failing reached services return an error without publishing a collision response; completed backend effects are not undone. No allocation or constant-success substitute for missing effects.

verify_contact_dispatch.py compares4096 original/PC/NXDK cases:1024 timed,1024 object,1000 no effect,443 APC,489 sound,116 Driller. Original numeric gates/class predicates execute; timed and surface effect bodies are explicit service boundaries, and object fixtures cover missing/type2 targets (other object types have their existing dedicated verifier). Five reached service failures preserve response; two nonfinite cases confirm that special-contact precedence bypasses inverse mass validation. Both builds and22 CTest checks pass. This is dispatcher composition, not completed backend implementation or live physics: retained timer/vehicle/object services and continuous scheduling remain open.


### Retained contact packet to dispatcher (2026-09-13)

rf_scene_npc_contact_dispatch validates the registered NPC/model owner, reads its retained collision packet through rf_collision_contact_read, and supplies special1ec, target1e4, inverse mass1d4, material1d0, class flags/use-kind, current speed/velocity and live published model forward to the combined dispatcher. The original actor view remains borrowed for object callbacks. Caller supplies timed/surface/object services and must establish that the contact packet is valid; this does not infer a contact from zero-initialized storage or schedule physics. No new retained allocation.
...
### Single-player pickup eligibility (2026-09-13)

rf_entity_pickup_prepare_sp reconstructs459560 through handler entry45977c. Original execution corrected an initial mistaken actor-kind assumption:4290d0 and429f90 each resolve actor200 through426fc0, rejecting linked kind1 and kind4 respectively. Both lookups are retained, including changed lookup results between calls. Item2bc bit1 precedes them; actor810 bit1 follows. Player lookup receives actor2c, then optional498e80 receives item3c, actor7d4,1,0. Visibility request and blocked result compare the low byte exactly to1. Rejected visibility retains the player token; earlier rejection returns zero token. Callback errors preserve caller outputs.

verify_pickup_gate.py executes actual4290d0/429f90/427020/486c90 in the original executable, intercepting only linked/player lookups and LOS and checking exact arguments/order.2048 original/PC/compiled-NXDK cases pass (680 eligible), including independently changing linked kinds, low-byte boundaries and absent players. Three callback-failure cases preserve both output sentinels. Both builds and22 CTest checks pass. This is compiled NXDK verification under Unicorn, not a new native XEMU replay. Grant handlers, NPC reaction, player pickup marking, item hide/retirement/timers, notifications and scene binding remain open.


### Single-player pickup completion ordering (2026-09-13)

rf_entity_pickup_finish_sp reconstructs45977c through45995c after eligibility. The cached class handler selects the custom grant or default45a3d0; nonzero grant result skips all completion work. Accepted NPC pickups call459490, while player pickups set the original item-indexed player byte. Negative item2ac retires through48ab40, zero leaves lifetime unchanged, positive hides through48a570 then rereads2ac and sets the timer at item2b0 through4fa360. The final459520 service is pickup audio: it resolves4594f0 and calls5056a0 with position, gain1, global audio state and0 when the resolved sound is nonnegative. It is not a generic mission notification.

verify_pickup_finish.py executes the original accepted path with actual4fa360 and player byte write, comparing1024 cases against PC and compiled NXDK under Unicorn. Both grant call signatures, default -1 output temporaries, arguments/order, changed delay after grant/hide, post-hide clock17, negative and wrapping timer offsets are covered. Six service-failure cases check the exact surviving mutations and call prefix while preserving completed output. Both builds,2048 eligibility regression cases and22 CTest checks pass. No native XEMU replay is claimed for these unbound services. Default weapon/ammo/general grants, custom handlers, NPC reaction implementation, player pickup array ownership, item hiding/retirement and pickup sound remain to bind before live scene pickup is complete.


### New-weapon inventory initialization (2026-09-13)

rf_weapon_acquire implements4030d0 over the existing bounded rf_weapon_inventory. A nonzero owned byte skips all work. Magazine size below1 adds quantity to a nonnegative ammo index and clamps reserve; positive magazine with quantity-1 fills loaded ammunition without touching reserve; otherwise loaded is min(quantity,magazine) and only excess goes to reserve. Addition wraps at32 bits before the signed40a520 clamp, whose lower-bound branch precedes the upper bound. Owned becomes1 before401470(inventory,1) notification. Invalid reached indices fail before mutation; notification failure keeps the completed inventory changes. No heap allocations or new scene storage.

verify_weapon_acquire.py executes complete original4030d0 with real40a510/40a520 and intercepts only401470, checking inventory identity/reason and ownership publication.2048 original/PC/compiled-NXDK cases match all448 inventory bytes, with840 notifications; five bounds guards and one callback-failure case also pass. Both builds and22 CTest checks pass. NXDK evidence is execution under Unicorn, not new XEMU gameplay.401470 notification internals,428d90 reserve addition,45a500 ammo pickup,45a6d0 weapon pickup and scene binding remain open. Analysis also identifies45a100 as pickup message formatting rather than a generic resource grant.


### Reserve ammunition addition (2026-09-13)

rf_weapon_add_ammo implements428d90 over existing inventory and borrowed current/pending/count state. A negative ammo type does nothing. Negative reserve is normalized to0, quantity is added with32-bit wrap, then425250 is queried. A nonzero low byte and matching current weapon add the full quantity to pending13ac before capacity clamps reserve. Actual4c86e0 requires weapon below active count and positive magazine; empty loaded ammunition with matching current requests425280(actor,0,0). Services may fail without undoing prior mutation. Definition capacity and state are read after the query, preserving borrowed-owner ordering.

verify_weapon_ammo.py executes full428d90 with actual4c86e0; query425250 and reload425280 are explicit service boundaries.2048 original/PC/compiled-NXDK cases match full inventory and current/pending state:676 absent-ammo no-ops,1243 query-only paths and129 reload requests. Two failure cases verify exact surviving mutations; three bounds checks preserve inventory. Both builds,2048 acquisition regression cases and22 CTest checks pass. No new native XEMU run or completed reload animation is claimed.425250 player-action/actor-flag query,425280 reload internals,45a500 difficulty scaling/acceptance and scene bindings remain open.


### Ammunition pickup quantity scaling (2026-09-13)

rf_weapon_pickup_amount reconstructs accepted SP45a5a0..45a69c quantity arithmetic. Authored difficulty factors at593e44 are1.5f,.9f,.8f,.6f;5893c0 is.5f and5897b0 is.01f. Ordinary scaling truncates quantity*factor+.5. Special IDs87243c/872468 scale in hundredths before truncation and multiply by100 with32-bit wrap. After min with signed-wrapping capacity-minus-reserve, special amounts have minimum100. Their displayed quantity truncates amount*.01f+.5 and has minimum1. Scale bypass omits only difficulty arithmetic.

The implementation represents products in96 integer bits and explicitly rounds to64 significant bits before and after adding.5, reproducing original x87 behavior without host-dependent long double precision. It returns both grant and message quantities; no resource mutation or allocation occurs. verify_pickup_amount.py executes the original accepted path through45a6c6 with actual ftol,40a510 and4895d0, recording428d90 and45a100 arguments.8192 cases match PC/compiled NXDK across difficulty, special/bypass paths, randomized signed32-bit inputs and integer limits; an invalid-difficulty guard preserves outputs. Both builds,2048 ammo-addition regressions and22 CTest checks pass. Native scene binding, special ID resolution, difficulty owner, acceptance, inventory/message consumers and mutations between those service calls remain open.


### Authored SP weapon supply audit (2026-09-13)

Scene inspection found weapon references but no retained rf_weapon_inventory owners, so connecting the numeric services first requires authored supply metadata and inventory creation. verify_weapon_supply_binding.py extracts44 weapons and16 ammo types from installed archives, executes actual4c22b0 name lookup over original-layout name objects, and executes full4c2a20 SP initialization. All mappings match, including -1 ammo for riot shield and Drill. These missing-ammo cases must remain explicit rather than assigning a guessed reserve slot.

4c2c80 stores Max Ammo pair at descriptor258/25c and optional Clip Size pair at80/84 (absent clips are0).4c2a20 copies SP258 to active260 and SP80 to active88, and copies requested difficulty593e58 to active593e54. Active magazine88 is global85cd90 for weapon0; it is not the MP84 field. For example Machine Pistol has SP magazine30 versus MP64, so selecting the wrong column would alter gameplay. Original lookup and active-field selection pass for all44 rows. The report is local artifacts/weapon-supply-binding.json; copyrighted tables remain untracked. This audit does not claim a compiled port reader, original full-parser execution, scene inventories or native XEMU playback. Next implement bounded ammo/weapons supply loading, then create player/NPC inventory owners through verified initialization. No runtime changes or rebuild were needed for this evidence-only audit.


### Bounded weapon supply reader (2026-09-13)

rf_weapon_supply_read parses the authored ammo-name order and weapon supply subset into a4872-byte catalog (existing weapon-name layout plus64 acquisition definitions). It retains first-match case-insensitive ammo indices, unknown ammo-1, first/SP Max Ammo and Clip Size values and optional clip0. Required fields and section structure, duplicate supply fields, numeric tokens and32/64 record bounds are checked before output publication. Read uses no heap; rf_weapon_supply_load bounds combined archive scratch storage and releases it on all completed reads. Installed scratch requirement is111834 bytes. No scene allocation has been added yet.

verify_weapon_supply_reader.py reruns the original lookup/SP-selection audit, then matches every name/index/SP field for44 weapons against PC and compiled NXDK. Eleven reader cases include malformed fields, missing ammo terminator, ammo overflow, optional clip, unknown ammo and duplicate-name first-match behavior; three PC archive-budget cases check exact capacity, one byte short and zero. Failure outputs remain unchanged. NXDK stack probing is supplied at its ABI boundary over pre-mapped fixture memory; kernel stack growth and native XEMU are excluded. Both builds and22 CTest checks pass. Next retain catalog with accounting in the scene and reconstruct inventory creation/notification before granting authored weapons.


### Retained scene supply catalog (2026-09-13)

Scene initialization loads the bounded weapon supply catalog from the opened tables archive using a128KiB scratch ceiling, before later resource construction. The static catalog adds4872 retained bytes and is cleared before each load. Its separate rf_scene_weapon_supply record publishes count, primary count, retained bytes and full catalog hash; PC replay output and native XEMU assertions compare that record. Load errors propagate instead of silently using empty definitions. Inventory owners and visible weapon grants are not introduced by this change.

replay-20260913-083746 passes180 frames for L1S1 with door/damage fixtures on stock64MiB XEMU (base memory67108864, plugged0). Native record [44,40,4872,3388863300] exactly matches PC and the independently serialized original supply audit. The existing gameplay/resource assertions pass, normal Xbox image is restored/rebuilt, and22 CTest checks pass. No new visual is claimed. Next create retained player/NPC inventories through original initialization and ownership notification, then connect the pickup services.


### Normal-SP acquisition notification resolved (2026-09-13)

401470 calls42cce0 before any selection/RNG. Actual42cce0 rejects players through4895d0, and also returns false whenever globals64ecb9 and6fc4d8 are both0. Thus the normal-SP acquisition notification has no effects; implementing alternate-mode AI selection is not a prerequisite for normal-SP inventory acquisition. rf_weapon_acquire_sp binds this proven no-op to the existing initializer. Its contract explicitly excludes alternate modes and does not equip/reset the weapon.

verify_weapon_acquire_sp.py executes complete original4030d0 with actual401470/42cce0/4895d0 and numeric helpers, observing but not replacing840 notifications across2048 cases. PC/NXDK inventories match every byte and five bounds cases preserve state. Both builds, the existing acquisition verifier and22 CTest checks pass. Constructor402c20 also confirms inventory current/secondary start-1 and ownership/reserve/loaded arrays start0, while its remaining AI/timer constructor is broader. Entity constructor422cb4 later grants class weapons and separately calls42ab20 for primary equip/reset; that separation must be preserved when adding retained inventory owners. No new XEMU run or visible equipment is claimed.


### Startup weapon grant sequencing (2026-09-13)

rf_weapon_startup_grant_sp reconstructs422cf5..422dbd after inventory construction. It uses normal-SP acquisition for primary, publishes primary before42ab20 equip/reset, rereads the authored primary afterward and fills that reserve to capacity. Secondary acquisition publishes secondary and fills its reserve; the extra weapon is acquired without a forced reserve fill. Repeated IDs preserve original already-owned handling. Missing ammo skips reserve fill. Errors preserve completed inventory/publication changes. Borrowed defaults may change during equip, and the reread is intentional.

verify_weapon_startup.py executes the original constructor slice with actual4030d0/401470/42cce0/4895d0; only42ab20 is supplied.1024 original/PC/NXDK cases compare entire inventories and primary/secondary/defaults, including816 equip calls, repeated/preowned weapons, absent ammo and changed primary definitions. One callback-failure case and three bounds checks verify surviving state. Both builds and22 CTest checks pass. A test-fixture type-name collision was corrected before the final PC build. Actual equip resources, earlier402c20 AI/inventory construction and scene owners remain open; this is compiled verification, not new native gameplay.


### Startup grant and animation-binding composition (2026-09-13)

Inspection corrects the earlier broad equip/reset description:42ab20 copies base animation records and overlays selected weapon records, including machine-pistol-special aliasing. It does not create a weapon model or reset firing effects (41ae70 is the distinct weapon-reset routine). rf_entity_startup_weapon_bindings_sp now connects the startup grant sequence directly to existing rf_entity_motion_selection_weapon using retained catalogs, removing the external42ab20 callback for this composition. It verifies matching supply/binding weapon IDs and preserves the original non-skeletal no-op. Playback and model creation remain separate.

The loaded L1S1 catalog probe exercises44 weapons on each of5 skeletal classes (220 cases), comparing complete selected mappings and45 sound pointers while independent original-audited supply records check loaded/reserve results. verify_startup_weapon_bindings.py records this composition evidence. Original startup and full42ab20 overlay verifiers each pass1024 PC/NXDK cases again; both builds and22 CTest checks pass. These are loaded PC composition tests and compiled primitive verification, not live native inventory owners. Next add per-actor inventories/selections and replace scene base-only animation consumers with retained selections.


### Optional default melee ownership metadata (2026-09-13)

Before live inventory creation, inspection found that the existing default reader omitted class90, the third startup grant. Original41c028..41c073 initializes it to-1 and optionally resolves $Default Melee through4c81f0. The class metadata now retains primary, secondary and melee; missing melee is-1, while primary/secondary remain required. Four of63 installed classes declare melee (Drone Smash, Tankbot Smash, Rock Snake Smash and Big Rock Snake Smash). Omitting it would remove authored attacks during startup inventory construction.

verify_default_melee.py executes the original optional-field block with actual string accessors/name lookup and supplied scanner tokens, then compares all63 complete default records with compiled NXDK and independently checked PC data. Updated default-reader tests cover missing/duplicate fields and preserve all12 output bytes on failure. Both builds,220 loaded startup-binding cases and22 CTest checks pass. Live NPC owner changes were deferred until this missing input was resolved; they remain the next integration step. No new native scene run is claimed.


### Retained NPC inventory and animation selection (2026-09-13)

Each registered skeletal NPC now owns a cleared448-byte inventory and its own base motion mapping plus45 borrowed action-sound pointers. AI animation reset/request, pain, death selection/playback, unholster recovery and the per-frame animation controller read the retained selection. Sound labels borrow the retained base catalog lifetime. Authored primary/secondary/melee grants are not enabled yet: armed death/recovery guards, corpse declaration views and firing-reset services remain integration work. No weapon attachment or firing is implied.

The residency fixture now initializes actor selections explicitly, including weapon=-1, and modifies actor-owned death/recovery mappings. Pain deliberately retains an action missing from the class mapping to catch a fallback to class storage. All22 CTests and both builds pass. The first native replay reached180 frames but correctly failed the running harness's old512KiB subsystem assertion; the updated640KiB ceiling accounts for the added storage without changing the stock64MiB machine.

Native replay-20260913-090737 passes180 door/damage frames for UID8456. PC and XEMU match78 inventory owners,71136 retained bytes, inventory hash1081661381 and mapping hash4180783186. NPC body subsystem peak is530212 bytes. QMP reports base-memory67108864 and plugged-memory0. The harness restores the normal Xbox image and exits0. This verifies ownership and existing behavior, not startup grants or complete gameplay.


### Armed retained animation consumers (2026-09-13)

Removed scene-only unarmed restrictions from death selection, death motion/playback, unholster recovery, AI animation reset and per-frame controller application. These consumers now validate the retained selection skeleton against the actor pose. Motion requests also validate that retained skeleton. The previously verified underlying routines consume actor-selected action/state records; possession of a weapon is not a rejection condition in those routines. Selection weapon IDs are not compared directly with current weapon IDs because42ab20 supports machine-pistol aliasing.

The scene death/recovery fixture runs twice, with primary/mapping weapon-1 and2. Both execute action selection, freeze/nonlooping behavior, bone clears, sound callback mutation/failure, recovery timers and RNG ordering using retained mappings. Incompatible retained skeletons still fail before playback or callback changes. The second fixture initially exposed persistent action15 from the first run; explicit initial unavailable actions corrected fixture isolation. Both builds and22 CTests pass. Original/PC/NXDK death-motion and AI-recovery verifiers each pass2048 cases, including their existing guard/failure checks. This turn adds no native XEMU evidence and does not enable startup grants, weapon rendering or firing. Corpse declaration views and firing-reset ownership remain before live armed startup.


### Selected action declarations and corpse lookup (2026-09-13)

Original42ab20 copies complete16-byte action records from base and replaces them only when the selected weapon record has a motion other than-1. Declaration/name availability therefore cannot be inferred from the final motion index or a union of authored base/weapon declarations. rf_entity_motion_selection now retains two declaration words alongside its motion/sound records. Base selection copies the base mask; an overridden action takes its declaration bit from the same weapon record as its motion and sound. Empty weapon motions preserve the entire base record.

Registered NPC corpse class sources and named-motion lookup now use the actor selection identified by the stable model slot. The NPC owner outlives model transfer during creation. Class-only callers without a registered NPC retain base lookup behavior. A source/selection class mismatch fails. The scene fixture exercises both armed and unarmed selection, actor-specific death motion/name, absent names and declared-but-empty names. Full corpse lifecycle dispatch remains open.

verify_base_action_sets.py independently derives selected declaration masks from authored records, including empty motions and machine-pistol aliasing. All484 weapon selections across L1S1/L1S2/L1S3 pass, together with existing mapping/sound/resource checks. Both builds and22 CTests pass. Retained NPC telemetry hashes the declaration words with the mapping so native comparison covers the new fields. Startup grants still await firing-reset services.

Native replay-20260913-091711 passes180 frames with door/damage UID8456. All78 retained owners match PC:71760 bytes, inventory hash1081661381 and combined mapping/declaration hash44603018. The added masks cost624 bytes; body subsystem peak530836 bytes stays within640KiB. QMP confirms67108864 base bytes and0 plugged memory. Normal Xbox image restoration completed and the harness exited0. This replay checks retained startup records and regressions; it does not demonstrate live armed deaths.


### Retained NPC firing reset owner (2026-09-13)

NPC bodies now retain rf_weapon_reset_state, including64 active bytes and sound/effect tokens. Original402c20 clears the active array;422360 initializes81c/820 and13d4 to-1. Registered skeletal NPCs have a character and no attached player. rf_scene_npc_weapon_reset resolves the registered actor/model and invokes the existing full41ae70 routine with caller-supplied descriptors and sound/effect services. It synchronizes shared actor flags before and after each callback and publishes preceding changes even on errors. Missing reached services remain RF_NOT_FOUND. Resource callbacks must keep the owners alive.

The new scene fixture verifies stale handles, idle reset without callbacks, active missing-service failure, complete stop/release/effect ordering, callback flag changes and failure after each service. The effect token intentionally stays unchanged after a successful stop, matching the original routine. Fixture seed/mapping counts were initialized explicitly after its first failure exposed omitted ownership metadata. All22 CTests pass. The existing original reset verifier passes2400 cases (1598 complete plus sound/effect/player service boundaries); it stops before unavailable original external operations and does not prove resource backend equivalence.

This is retained ownership and full routine integration, not live firing or automatic pain reset. The next connection needs authored weapon Flags/Flags2 and Stop Sound resolution, then concrete resource services. Current damage fixture still passes no pain reset callback and startup weapons remain ungranted.

Native replay-20260913-092348 passes180 frames with door/damage UID8456 and normal-image restoration. PC/XEMU match78 owners,78936 retained bytes, combined inventory/firing hash2955461549 and mapping/declaration hash44603018. Firing storage adds7176 bytes; body subsystem peak538012 bytes remains below640KiB. QMP reports67108864 base bytes and0 plugged memory. The native run verifies initialized owners and regressions, not invocation of live firing-reset resource callbacks.


### Authored weapon flag vocabularies (2026-09-13)

Recovered primary table5a24d8 (23 names) and secondary table5a2534 (11 names) directly from the original executable. Parser4c2dce passes23 entries to513020;4c2e06 passes11. Primary flags264 are ORed with the parsed primary mask; flags268 are cleared before the optional secondary field. Reset41ae70 mask6 means continuous_fire/alt_continuous_fire; secondary mask40 means flame. Stop Sound is optional at4c3992, defaults descriptor204 to-1, and resolves through434cb0 after string parsing. These facts guide descriptor composition; they are not yet a composed live descriptor loader.

rf_weapon_flags_read parses either vocabulary with the shared bounded lexer, case-insensitive names, duplicate OR behavior, comments and exact consumed byte reporting. It rejects unknown names, malformed syntax and invalid selector without changing outputs. verify_weapon_flags.py executes actual original513020 and its parser/CRT callees with the executable's own vocabularies, then compares PC and compiled NXDK. All1161 cases pass:88 installed lists,1062 individual/full/random masks and11 port guards. Original fatal diagnostic paths are not executed. The first verifier run reached the final selector guard but used the syntax-error expectation there; correcting that harness expectation completed verification. Both builds and22 CTests pass. No runtime scene change or new native XEMU run is claimed.


### Composed weapon reset catalog (2026-09-13)

rf_weapon_reset_catalog_read now retains all weapon names plus flags264/268 and resolved Stop Sound class IDs in4872 bytes. Required primary Flags OR an explicit64-entry initial_flags input; optional Flags2 starts0. Optional Stop Sound defaults-1 and resolves through retained Foley names, with first case-insensitive match and unknown/empty names remaining-1. Duplicate selected fields and malformed lists/strings fail transactionally. rf_weapon_reset_catalog_load bounds and releases its weapons.tbl scratch allocation (108890 installed bytes).

verify_weapon_reset_catalog.py compares complete PC/NXDK catalogs against actual original4c2dc2..4c2e1a, including mandatory/optional field parsing, actual513020 and descriptor stores; actual434cb0 resolves against497 independently extracted Foley names. All44 weapons pass with zero and nonzero initial flags. Twelve cases cover optional fields, duplicate sound names, unknown sound, primary OR and six malformed/duplicate guards; exact/one-byte-short/zero archive budgets pass. Original Stop Sound token extraction is independently supplied to434cb0; this does not execute the entire original weapon parser. Both builds and22 CTests pass.

Loader audit:4c67a0 dispatches primary4c2b60 and secondary4c4850, which both call4c2c80 and set their distinct descriptor+a8 marker afterward. The reset reader deliberately accepts initial flags rather than assuming constructor bits are zero. Confirm those startup seeds before retaining this catalog after scene Foley loading, then connect resource callbacks and pain dispatch. No new native scene run or startup grants are claimed.


### Fresh reset seeds and retained scene catalog (2026-09-13)

verify_weapon_reset_seeds.py executes original4c2990,5736fb and all64 complete4c9f60 constructors plus nested constructors without replacing their calls. The loader-mapped descriptor data starts with zero264/268 flags, and construction preserves them. A second run injects distinct flags into all64 descriptors and proves the constructors leave them unchanged. This validates zero seeds for a fresh catalog, not arbitrary in-place reload semantics. The synthetic thread SEH slot is mapped and its sentinel restored.

Scene audio startup now loads the4872-byte reset catalog after Foley creation and after freeing its table text. The108890-byte temporary weapons allocation is bounded by128KiB and released. All44 descriptors and7 Stop Sound IDs are retained; close clears the catalog before releasing Foley ownership. Telemetry includes count, resolved sound count, bytes and whole-catalog hash. The independent original parser/store and sound-lookup audit yields hash3940996059, which is checked explicitly by the native harness in addition to PC equality. Both builds and22 CTests pass.

Pain dispatch and startup grants remain the next connection. Fresh firing states have no active weapon/effect; the full reset routine can handle that path with the retained descriptors without invoking resource services. Actual active firing still needs sound/effect service bindings before it is enabled.

Native replay-20260913-093913 passes180 door/damage frames for UID8456. The retained catalog is exactly[44,7,4872,3940996059], matching PC and the independent original-derived hash. QMP confirms67108864 base bytes and0 plugged memory. Normal-image restoration completed and the driver exited0. This validates catalog residency and existing regressions; startup grants remain unenabled.


### Authored NPC startup grants and retained pain dispatch (2026-09-13)

NPC creation now calls the previously verified SP startup grant/binding composition with each class primary, secondary and optional melee defaults. It publishes selected weapons into registered views and retains owned slots, loaded/reserve ammunition, selected mappings, declarations and sound labels. Inventory/mapping telemetry is recorded after grants. In L1S1,60 of78 registered NPCs receive a primary weapon,0 receive a secondary, and60 total weapon slots are owned; selection hash3892424261. This does not create weapon models or implement active firing.

rf_scene_npc_pain_retained supplies retained reset descriptors and selected action audio to the existing pain routine; the damage fixture now uses it. The full reset routine handles fresh inactive firing states without reaching resource callbacks, while active resources still fail explicitly until bound. Controller-residency lookup now uses the actor selection after registration and base only before an actor selection exists. Unholster/AI-reset/motion-request fixtures use selected records.

The first180-frame stock64MiB door/damage replay-20260913-094343 passed with startup grants. Broader actor/death PC preflight exposed an incomplete AI-reset fixture expectation: stopping nonlooping animation also clears freeze, frozen and primary markers, which the newly reached pain path had populated. The fixture now verifies those side effects and restores actor/playback state. Temporary failure diagnostics were removed. All22 CTests pass; broader PC replay passes60 unholster cases,592 AI-reset cases and312 motion requests. Inventory/firing hash2983019725 and mapping/declaration hash1494918578 reflect the granted records.

Broader native replay-20260913-094951 passes180 frames with door, damage UID8456, actor-pair fixtures and death animation enabled. PC/XEMU startup, unholster, AI reset, motion-request and death records match; normal-image restoration completed and the driver exited0. The machine remains67108864 base bytes with0 plugged memory. This verifies inventory/animation integration and the exercised fixtures; continuous combat, weapon attachments and campaign completion remain open.


### Third-person model/tag accessors and attachment trace (2026-09-13)

Traced descriptor28 third-person filename,30 loaded model,34 muzzle cache and38 grip cache. Original4c84d0 returns a model only for weapon0..63 and a nonempty filename; it does not gate by current weapon count or load the model.4c8510 resolves grip_1 through503220 only when cache38 is-1.4c8560 does the same for muzzle_1/cache34. A returned-1 remains eligible for lookup next time; other signed cache values are retained.

rf_weapon_world_model_token and rf_weapon_world_tag implement that ownership boundary over loaded model tokens and explicit tag lookup services. Missing model returns tag-1. Port invalid-kind/missing-service/callback-failure guards preserve output and avoid assigning a cache result. verify_weapon_world_tags.py executes actual4c8510/4c8560 plus4c84d0/4ff490, supplying only503220 and validating model/name arguments.2048 cases include repeat calls, absent names/models, out-of-range weapons and unusual cached values;79 original lookup calls and3 port guards pass. Both builds and22 CTests pass. No model load, placement or rendering is claimed.

Attachment trace identifies418e60 (called by421c40): obtain hand count40a490, resolve hand tag42d970, transform actor tag5034f0 into position/basis, then compensate using weapon grip_1.421c40 applies visibility/state gates, optional player model override, multi-hand iteration and recoil rotation before503100 draw. These transforms/gates remain to be reconstructed and connected to actual model owners; copying a model to the hand position alone would omit the original grip correction. Raw Ghidra exports are local evidence and require instruction-level validation.
...
Instruction inspection corrected an initial test assumption:409f90 is a no-op constructor and4fbcd0 only invokes it for three rows; these do not initialize an identity matrix. The port initializes scratch defensively and requires successful transform services to produce complete output poses. No original scratch initialization is claimed. Actual5034f0 transforms, model loading and421c40 scene rendering remain unbound; this is compiled placement verification, not a new native gameplay or visual result.


### Third-person weapon filenames (2026-09-13)

Original4c3007..4c3024 optionally reads $3rd Person V3D into descriptor28. rf_weapon_model_names_read/load retains these authored names in stable primary/secondary weapon order, preserving absent/empty names and rejecting duplicate filename fields. The separate $3rd Person Muzzle Flash Glare field is ignored by this bounded filename subset. Output commits only after validation; archive scratch is freed and capped.

verify_weapon_model_names.py independently parses comments/quoted fields from installed weapons.tbl and compares all44 output slots against PC and compiled NXDK. Ten valid/malformed cases verify exact outputs and preservation on failure; three PC archive budgets check108890-byte scratch boundaries.20 entries reference18 distinct nonempty models. All exist in meshes.vpp under the compiled .v3m extension used by the existing model filename resolver; authored names remain unchanged. Catalog size4100 bytes. Both builds and22 CTests pass. This is metadata validation with traced original field semantics, not execution of the complete original parser or live weapon model residency/rendering. Next: budgeted selected-model ownership, authored hand tags and421c40 draw dispatch.


### Shared weapon geometry/tag ownership (2026-09-13)

rf_weapon_models_open loads selected weapon IDs through the existing compiled filename, static render resource and attachment tag owners. Case-insensitive compiled names share one retained item, while each weapon keeps its own grip/muzzle cache initialized to-1. Model tokens are local item index+1, not scene handles. All static LOD geometry, part/material records and first-submesh tags survive archive closure by ownership; textures remain separate. Publication is transactional, partial resources are closed on failure, and closing twice is safe. Budget includes the owner, allocated item capacity, retained arrays and temporary model directory, excluding caller archive, stack and allocator metadata.

All18 authored third-person models pass the existing static render loader. verify_weapon_model_owner.py checks48 PC selections (all44 single IDs, empty/full and alternating subsets), case-insensitive alias sharing, token/name/cache mapping and96 exact-peak/one-byte-short budget cases. Failure rollback and repeated close are asserted in the probe. Loading every referenced model retains207368 bytes and peaks at216160 bytes, with30 tags; this excludes textures. Both PC/NXDK builds, the10-case compiled filename reader verification and22 CTests pass. The new composed owner has not yet run natively in XEMU or through a compiled-NXDK file harness. It is not yet bound to live NPC residency/drawing. Next connect selected level inventories, authored hand tags and texture/draw ownership.


### Live NPC weapon model residency (2026-09-13)

After startup inventory grants, the level now collects owned weapon IDs and loads their shared third-person geometry/tag owners. NPC shutdown closes the shared owner. Filenames are temporary; the loading allowance is256KiB, including temporary filename storage. Reported peak135172 is a conservative bound covering the128KiB table scratch allowance plus4100 filename bytes, not a sampled allocation peak. Textures and drawing remain separate.
...
### Authored NPC hand/grip pose binding (2026-09-13)

Expanded verify_model_cached_tags.py to285 original primary_weapon_1..3 lookups over all95 installed character models. Actual51d5b0 selects the same bone/LOD attachment after file-view index translation; no cached CSPH name shadows these queries. This complements the96-case original two-entry capacity verification. The scene retains the first two sequential file tags, stopping at the first absent name, without substituting hand-bone names. File queries are immutable; this does not reproduce original loader cache side effects or excess-tag query calls beyond the two retained entries.

rf_scene_npc_weapon_placement resolves a registered NPC, reads its current published pose and primary weapon, evaluates the authored bone/LOD attachment from existing matrices, and supplies actual retained static weapon tags/transforms to rf_weapon_place_in_hand. Actor and weapon tokens belong to distinct owner spaces, so callback phase explicitly selects the correct owner. No animation is advanced. The corpse point helper now shares its full-pose implementation and preserves its existing point-only API. The startup probe evaluates both hand slots for each equipped actor and hashes successful outputs.

Native replay-20260913-102935 passes180 door/damage/actor-pair/death-animation frames. Initial placement telemetry [60,60,60,1612060727] matches PC:60 equipped actors,60 successful poses,60 absent second hands. Loaded-model telemetry remains[1,1,8200,135172,2,3661868426,8,0]; that hash is captured at model creation before grip cache resolution. QMP confirms67108864 base bytes,0 plugged memory. Normal-image restoration completed and the driver exited0. Both builds and22 CTests pass. This verifies the bound initial poses and regression replay, not per-frame held-weapon drawing or original-frame visual parity.

Next draw trace:421c40 clears810 bit200 before gating and sets it when a model exists after hand iteration.427020 reads810 bit0,428510 reads class724 bit3,429d20 checks attachment75c!=-1,428e60 combines429f90 with810 bit800,4290d0 classifies a linked200 object, and500f80 initializes draw state. These are raw trace notes awaiting composed verification and live binding; visibility/recoil, textures and503100 draw remain open.


### Weapon visibility/model prefix (2026-09-13)

rf_weapon_world_visibility reconstructs421c40 through421d47: clear810 bit200 first, then reject810 bit0, absent primary-1, missing class724 bit3, inventory7d0 bit200, attachment75c!=-1, the810 bit800 state unless linked class is4, and linked class1. Special player weapon gating uses1044 low byte; player103c low byte selects the override model after the ordinary loaded-model lookup. The prefix never sets bit200; the original does that after its hand loop. The input carries a stable resolved linked class, and this pure prefix does not reproduce mutable lookup services.

verify_weapon_visibility.py executes original421c40 to421d47 or its actual hidden return, including original predicates and486c90 linked-class dispatch. Only426fc0 handle lookup is supplied.4096 cases match complete flags and selected model on PC and compiled NXDK:342 models selected,2265 linked lookups. Three compiled argument guards preserve state/output. Both builds and22 CTests pass. Hand iteration, recoil/draw-state initialization, final bit publication and scene binding remain open. No new native or visual claim is made for this unbound prefix.


### Held-weapon draw-state setup (2026-09-13)

rf_weapon_draw_state_prepare reconstructs421df6..421e4f over the80-byte draw state: constructor defaults, flag80, optional special-view flag2 and color ff002000, actor1474 tint and the supplied post-recoil basis. Original50cc00 constructors are no-ops. State words3 and6 therefore remain caller-provided except when special-view low byte equals1 and word3 is replaced. The implementation preserves those bytes and supports an overlapping basis by copying it before state writes. No meanings beyond observed offsets are assigned to other fields.

verify_weapon_draw_state.py executes the original slice with every callee unhooked, including500f80/500fb0, color constructor/store, identity initialization and matrix copy.2048 cases match all80 bytes on PC and compiled NXDK, with arbitrary matrix bit patterns, low-byte special modes and poisoned initial state. Two null guards and one explicit alias case pass. Both builds and22 CTests pass. Recoil is an input to this stage; full hand-loop orchestration, textures, final draw submission and scene binding remain open.


### Held-weapon recoil basis (2026-09-13)

rf_weapon_recoil_basis reconstructs421d85..421df6: zero recoil preserves the basis; hand0 negates the angle and hand1 keeps it. Axis is basis[6..8], without normalization.4fbf30 uses half-angle sine/cosine, float-rounded axis products and intermediates, then40ea80 multiplies in its per-element accumulation order. Position is unchanged. The port rejects nonfinite inputs/invalid hand and preserves output on error; alias is supported.

verify_weapon_recoil.py executes original recoil, axis-angle and matrix-product callees unhooked.2048 nonnormalized-basis and signed-zero cases with recoil in[-pi,pi] match PC/NXDK output bytes exactly under x87 control0x27f; no tolerance was needed. Five invalid-input cases and one alias case pass. Both builds and22 CTests pass. Arbitrary huge-angle equivalence and live firing are not established. Next compose visibility, hand placement, recoil, draw state and final flag publication around submission, then bind textures/rendering.


### Composed held-weapon draw loop (2026-09-13)

rf_weapon_world_draw_run composes the verified visibility, recoil and state setup around explicit placement/submission services. Model selection stays captured, hand count is reread each iteration, and callback-updated recoil/view mode/tint are read at their corresponding stages. Missing placements skip their hand. The successful visible-model path publishes810 bit200 even with zero hands or all placements missing. Port errors preserve completed service effects and do not perform final flag publication. Scratch explicitly supplies constructor-preserved fields.

verify_weapon_world_draw.py executes full original421c40, supplying only linked-handle lookup,418e60 placement and503100 submission.1024 cases match PC and compiled NXDK complete draw state, callback records and final flags, with330 placements and160 submissions. Coverage includes hidden actors, player overrides, empty/missing hands, recoil, special-view state, preserved scratch, callback hand-count shrink/growth and flag mutation. Original placement math was independently verified and is already bound to scene poses; submission is still captured by this harness. Both builds and22 CTests pass. Dedicated callback-failure/missing-service coverage and actual renderer binding remain required; this is not a new native or visual result.


### Draw failure prefixes and live weapon materials (2026-09-13)

The composed draw verifier now covers ten port-specific cases: failure at either hand placement or submission, missing placement/submission services, zero-hand success, invalid hand count, and callback state mutations followed by errors. PC and compiled NXDK match completed records, counts, scratch and flags; no final bit200 is published on failures. Original1024-case comparison remains passing.
