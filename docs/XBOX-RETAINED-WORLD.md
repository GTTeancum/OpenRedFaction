...
The next structural pass adds retained GPU NPC skinning/projection; see [its implementation and validation limits](XBOX-RETAINED-MODELS.md). Rigid-model rendering, CPU pose evaluation and high draw-call counts remain. These changes do not solve all rendering or simulation costs.
