...
## Owned geometry body adapter

rf_geometry_collision_body_sweep uses owned mover flat faces and static room trees, mapping tree face indices back to source indices. Mover queries use committed pose.position and input_matrix, carry pose velocity and registered handles, and retain sphere/solid/room/face identity. Metadata resolution is an explicit callback for each accepted query; no runtime texture or material IDs are fabricated. Caller supplies 92 bytes of scratch per mover; no query-time allocation.

python tools/verify_geometry_body_sweep.py passes 11 PC integration fixtures based on the original499ed0 ordering corpus. Actual reconstructed flat-face and room-tree geometry are used, with synthetic surface metadata, world face remap to37, and deliberately incompatible ray pose fields. Tests verify exact contact/identity/velocity and metadata callback counts, including miss preservation. PC/NXDK builds and six CTests pass. Native adapter execution, real surface lookup, scene sweeps/support, cache equivalence and moving-platform physical response remain open.

## Body collision surface metadata

rf_geometry_body_surface is the metadata callback for owned body sweeps. Borrowed world-first geometry and shared texture mappings resolve source faces to port texture slots, while rf_surface_material_lookup resolves their names to authored material IDs. No-texture faces return UINT32_MAX/default0, matching468700. Port slots are not original RF.exe texture handles. No allocation, texture loading or runtime face mutation is implied.

python tools/verify_body_surfaces.py passes29 PC/NXDK machine-code checks covering world/mover slot selection, case/prefix boundaries, default material and invalid mapping outputs. python tools/verify_surface_materials.py passes67 original/PC/NXDK authored-prefix cases,5 malformed inputs and duplicate ordering. Both builds and six CTests pass. Scene palette/mapping lifetime and live body sweep/support integration remain open.

## Campaign body sweep wiring

Campaign actor_sweep now uses the shared owned-geometry body sweep with registered movers, committed poses, body orientation/radius, all authored spheres (bounded by the existing eight-entry class declaration limit), and real material-prefix/shared-slot metadata. Scene retains the palette, source pointer list and mover scratch until cleanup; queries allocate nothing. The full last contact remains in rf_scene_actor_body_contact, and rf_scene_actor_body_sweeps records queries/hits/mover hits/status/adapter bytes. Noncampaign diagnostic sweeps retain their prior static-only path. Ground queries and physical moving-support response remain open.

Stock64MiB XEMU L1S1 replay artifacts/xemu/replay-20260910-161005/report.json passes16 frames with exact PC telemetry:24 queries,1 hit,0 mover hits,3032 retained adapter bytes. L2S1 replay artifacts/xemu/replay-20260910-161027/report.json also passes. These startup paths do not establish a live mover contact; targeted door/platform approach coverage is still required. Both builds and six CTests pass; no new visual capture.

## Resolved mover support commit
...
## Campaign mover-aware ground queries

Campaign ground probes now query the shared body sweep with the selected lowest sphere and identity orientation. Per-frame full contacts are retained separately from legacy132-byte ground diagnostics; transient/stance queries carry explicit contact outputs. Material selection uses returned metadata, support commits use resolved mover height/handle rules, and ordinary run landing rebases velocity before its existing Y reset. Ground preparation consumes retained support Y velocity. The shared campaign geometry/metadata query setup is reused by body sweeps. Route fixtures preserve support state between independent routes.

Native stock64MiB replay artifacts/xemu/replay-20260910-161949/report.json (L1S1) matches PC:31 ground queries,12 hits,0 mover hits. L2S1 artifacts/xemu/replay-20260910-162033/report.json also passes. Both16-frame paths compare ground query counters/status and player state, not every full contact record. Both builds and six CTests pass. The new contact ring consumes5888 static bytes. Targeted live mover contact, per-tick platform velocity refresh, carry/response, support-loss ownership and full moving landing effects remain open; startup replays do not prove moving-platform gameplay.

## Authored-spawn mover approaches

Ran eight PC input routes from authored L1S1 spawn,480-2250 frames each. All completed without collision errors; none recorded a mover body/ground hit. tools/replay_mover_approaches.py preserves each command sequence and reports MOVER_NOT_REACHED rather than calling replay completion a contact success. The default door-direct case has2250 frames,2731 body sweeps/469 static hits and4499 ground queries/4423 static hits, ending(-58.42318,-7.43263,10.11554). Authored lower door movers8544/8543 are near(-51.564,-7.125,-19.515) and(-47.623,-7.125,-17.923). Straight continuations encounter static geometry before reaching them.

Reproduce with python tools/replay_mover_approaches.py --case door-direct; artifacts/mover-approach/door-direct-report.json includes executable/input hashes. This is PC route evidence only, not XEMU or correct campaign traversal proof. Next work is inspect the intervening route or explicitly stage a separate contact fixture; do not keep extending a blocked straight command and claim progress toward door contact. No source/build behavior changed in this step.

## Staged live lower-door contact

rf_scene_stage_door resolves authored L1S1 mover UIDs8544/8543, stages the player three units from their midpoint along the mover forward basis, and faces the pair. This is an explicit collision fixture, not the reconstructed player spawn or a route through the campaign. PC headless opt-in uses RF_REPLAY_DOOR_START; Xbox uses harness-owned campaign-door.flag, restored with other replay state. tools/replay_door_contact.py --native generates180 forward-command frames and runs both platforms.

PC and stock64MiB XEMU artifacts/xemu/replay-20260910-162838/report.json pass:225 body queries,112 hits,111 mover hits. Final92-byte contact/identity record matches exactly, selecting mover index0 (UID8544), face3, Metal material2. Ground queries359/hits270/mover hits0. First native run162700 also passed state/counter comparisons. This proves live player collision reaches authored mover geometry; no door activation/opening, platform carry, authored traversal or visual parity claim. Both builds and six CTests pass. Frame capture was inspected but is not a useful showcase, so no README image update.

## Campaign compact actor registration
...
Combined special lightmap grid (4f3500..4f3e23 and4f3e5d..4f3f4a):
- `python tools/verify_lightmap_special_grid.py` compares256 grids/6237 texels on original instructions, PC and compiled NXDK under Unicorn at x87 0x027f. Original coverage, selection, interpolation, actual4da8b0 and border copies execute unchanged; the harness supplies topology/normals and skips their allocation/collection/destruction.
...
Vertex adjacency construction evidence:
- Original4e0140 appends a corner before the head of the circular face-corner list (tail insertion), then calls4ce1e0 on vertex+0x20 with the face pointer. Thus corner traversal retains insertion order.
- `tools/verify_lightmap_adjacency_order.py` runs full original4ce1e0 and its actual4ce360 search/45ec40 append helpers using preallocated storage. All2048 sequences/66560 insertions preserve first-occurrence order and suppress41336 repeated insertions; unused slots remain unchanged. This is original behavior evidence, not a new C adjacency implementation.
- `tools/inspect_geometry.py` now inventories unique face membership per vertex across94 static sections. No repeated same-face corner membership appears in these authored sections. L1S1 has29110 links, maximum12 faces/vertex, and143076 bytes for uint32 offsets plus face IDs. Largest estimated table is L18S3:195852 bytes. Maximum degree across the inventory is24. These are data-size estimates excluding owner, scratch and allocator overhead, not native allocations.
- Original4ed520 may reject faces through4dfe20 and remove them via4ce2a0/4dfc70; retained topology must account for that before assuming every serialized face survives. Runtime deletion/GeoMod can also alter adjacency. Bounded ownership and native sampling binding remain open.

Bounded retained vertex/face adjacency owner:
- `rf_geometry_vertex_faces_open` creates one compact offsets/face-ID block from an explicit strictly increasing list of surviving serialized faces. Repeated corners add each face only once per vertex, preserving the original4ce1e0 insertion semantics. Three passes avoid temporary per-vertex arrays; budget covers owner and block. Output remains unchanged on failure, and close is idempotent.
- `python tools/verify_geometry_vertex_faces.py` loads actual archive/level/geometry owners and checks every ordered list against an independent raw-record traversal across94 levels:482975 vertices and1922603 links. Every level also passes a one-byte-under-budget rejection. L1S1 uses143096 bytes including the20-byte owner on this PC target.
- `rf_geometry_probe.exe --adjacency-fixture` checks a filtered face list, repeated corners, empty selection, invalid IDs/order, budget rejection and repeated close. Both PC and NXDK builds and24 CTests pass. Native owner execution has not yet been checked.
- Callers must supply loader-surviving faces; this API does not reconstruct rejection or removal decisions. This is a static snapshot and must be rebuilt after topology changes. Smoothed-normal binding, native residency and rendered lighting updates remain open.

Retained geometry special-sampler corner binding:
- `rf_geometry_lightmap_vertex` reads one retained corner UV/position and gathers its explicit surviving adjacency into caller scratch, then runs the verified original corner-normal calculation. It requires the base face to be present, returns NOT_FOUND for unmapped/excluded faces, and preserves output on errors. No allocation.
- `python tools/verify_geometry_lightmap_vertex.py` checks all28356 lightmapped L1S1 corners. Actual PC archive/geometry/adjacency binding matches raw UV/position records and original4f4192..4f4222 normal results; compiled NXDK binding reads the same geometry and adjacency and matches all32 output bytes. Original helper traversal is unhooked. Two NXDK index/capacity guards pass.
- Harness scratch capacity24 occupies480 bytes; L1S1 PC probe sizes its work to actual maximum degree12. Both builds and24 CTests pass. All serialized faces were supplied as survivors and serialized planes were used; original loader acceptance/plane repair, native owner residency and rendered updates remain excluded.
- Next integration needs per-mapping selected polygon grouping, bounded vertex scratch, retained RGB/mask resources and native rendering validation.

Retained mapping polygon grouping:
- `rf_geometry_lightmap_polygons` accepts explicitly surviving file-order face IDs, a mapping ID and room (-1 for whole solid), queries required polygon/vertex counts or fills caller scratch using the verified retained corner binding. It respects the original signed16 face mapping field, preserves face/corner order, and allocates nothing. Count outputs change only on success; scratch may change on later errors.
