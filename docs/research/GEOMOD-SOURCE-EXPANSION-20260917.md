...
### Longer continuation exposes empty-grid fall-through

Added `--settle` to `tools/check_paired_reset_checkpoint.py`: load the recut save and run300 neutral updates, comparing against uninterrupted1450-frame control. The5096-byte saves match exactly, but the explicit three-settled-fragments acceptance fails. This is an intentional outstanding gameplay regression, not a passing motion test. Log: `artifacts/connected-debris-settle-pc.log`; inputs/saves/logs: `artifacts/connected-reset-checkpoint/settle*`.

Fragment0 has serialized collision radius0. At the1150-frame checkpoint its position is(-5,-41.84996796,-2.79743099) and velocity(0,-29.40003014,0). After300 further updates it reaches Y=-311.35043335 with vertical velocity=-78.39985657. Fragments1/2 remain settled at Y=-0.80344826/-0.82953572. Thus the previous301-frame exact-PC acceptance proved cross-platform state, but did not prove valid collision for every fragment. The awake body was already below the world, not waiting to settle normally.

The thin piece has no sampled collision spheres. This follows the already-evidenced empty-grid case in original4d1700 and grid-to-sphere threshold; it must not be misdiagnosed as a save/restore or float-rounding regression. Extended `inspect_body_sweep_order.py` and `verify_body_sweep.py` with an empty-sphere body crossing three actual planes. Full original499ed0 emits no geometry queries and misses; shared PC and compiled NXDK composition match. All12 cases pass (`artifacts/empty-grid-body-sweep.log`). This qualifies only the zero-sphere sweep path, not a complete original fragment lifecycle or proof of how the retail game handles every thin fragment.
...
### Three-source save while a fragment is moving

`check_beam_continuation.py --both-posts --airborne` saves181 records into the second-blast replay (equivalent uninterrupted780 records), requires two fragments with fewer than two asleep and no motion error, then reloads and runs600 further updates. Both must finish asleep with finite centers above the room floor, and all7692 checkpoint bytes must equal uninterrupted1380-record playback. The PC run passes with one fragment moving at save and both settled afterward. Separate output directory `artifacts/triple-beam-airborne` preserves the previous longer-save evidence.

Native moving save `artifacts/xemu/render-20260917-191849` passes76 checks on stock64MiB. The7692-byte checkpoint exactly equals the uninterrupted PC780-record save;3018 available pages gives11.789MiB endpoint headroom. Native capture inspected: damaged joint, smoke and airborne wood are visible along with the existing room and weapon. This is an earlier point in the already tested real rocket sequence, not an injected fragment pose. Landing acceptance follows separately.


Native landing run `artifacts/xemu/render-20260917-192011` loads that Xbox-created moving save, advances601 input records, and passes76 checks. Its7692-byte checkpoint exactly equals uninterrupted1380-record PC control.3336 available pages gives13.031MiB endpoint headroom. The native final frame was inspected: detached wood rests on the floor, joint damage remains, smoke has cleared, and the room/weapon/HUD remain visible. The moving and final endpoint captures plus body-state checks establish this sequence; intermediate visual frames were not individually inspected, and audio quality was not tested. Logs: `triple-beam-airborne-pc.log`, `triple-beam-airborne-native.log`, and `triple-beam-airborne-land-native.log` under artifacts. Broader destruction behavior remains open; no further repetition of this same beam history is needed without a regression or changed implementation.


## Elevated render-only cap inspection

Added `rf_scene_inspection_camera(eye,target)` after gameplay/combat/audio updates and before visibility/render dispatch. It validates a finite nondegenerate look-at basis and preserves an existing camera on invalid requests; NULL/NULL disables it. The PC frontend accepts RF_REPLAY_INSPECTION_CAMERA only for headless DEV runs, formatted as six comma-separated eye/target floats. This is an inspection view, not a player teleport or a new playable flight mode. The camera is disabled by default, including on Xbox.

`tools/check_geomod_cap_views.py` loads the existing three-source Xbox save and runs121 neutral records with baseline, near-cap, far-cap and reverse-far-cap views. All four7692-byte gameplay checkpoints match exactly. PNGs under artifacts/geomod-cap-views were inspected directly: the elevated views reveal wood-textured post tops and cut beam-end surfaces. Caps are dark; a dark polygon behind the far joint in the first angle is not conclusively identified by the reverse view. Do not call this complete surface-fidelity acceptance or assume that patch is valid/invalid without tracing its render source. The reverse camera also places nearby lamp/roof geometry prominently in view; these are diagnostic camera positions, not proof of traversable player placement. No original screenshot comparison or GitHub image upload occurred.

All123 PC tests pass, including camera basis/invalid-input preservation controls, and the stock-profile NXDK build/link/XBE/ISO succeeds. Logs: artifacts/cap-inspection-{build,tests,pc,xbox}.log. There is no native elevated-camera run in this change. Remaining work is tracing the ambiguous patch, cap lighting/material quality, broader cut shapes, and native qualification as needed.


### Dark-polygon provenance resolved by depth and uncut control

The apparent dark polygon behind the far joint is pre-existing floor detail. The cap-view harness now exports the native PC raster depth buffers and renders an additional uncut-room control from the identical elevated camera. The inspected uncut image contains the same patch. In the unobstructed rectangle x360..409/y268..280, all650 RGB pixels and all650 depth values match the destroyed-room capture exactly. The neighboring uncut post occludes the left edge, so that occluded region is explicitly excluded rather than treated as a match.
