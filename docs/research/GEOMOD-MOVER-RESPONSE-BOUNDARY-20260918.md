...
## Timed fragment scheduler handoff

Added `rf_physics_fragment_step_timed` with a read-only callback receiving the actual remaining seconds for every speculative substep. This uses the same bounded fragment step implementation and original contact response; legacy solid/fragment entry points retain their old query gates. The timed variant queries active stationary bodies as well, which is necessary for a moving counterpart to cross a stationary piece. Inactive bodies still need explicit scene wake admission; this API does not invent that policy.

The new timing test exercises contacts at 0.25 and 0.5 followed by a miss, observing remaining times 0.125, 0.09375 and 0.046875 exactly. It checks full rollback after a second-query error, ten-query protection against zero-time contacts, no work for sleeping bodies or zero dt, and a rising polygon crossing a stationary body's point through the actual relative-query and thin-face routines. This last case proves query eligibility/detection only; it does not establish pushing/carry response. A moving-body control produces byte-identical body state, report, published pose and flags through legacy and timed fragment entry points.

All 126 PC tests pass (artifacts/fragment-timing-ctest.log); the final added differential control also passes separately after rebuild. Stock64MiB NXDK builds successfully (artifacts/fragment-timing-xbox-build.log). The new callback route is not yet selected by the scene and has not run in XEMU. Retaining and consuming mover motion intervals across scene commit, geometry candidate filtering, and wake admission remain the live integration work. Existing stationary-world gameplay remains on its prior path.

## Live frame interval retention

The scene now allocates one 104-byte `scene_mover_interval` per registered mover at level load, counts that storage in the mover registration ledger, and frees it on level teardown. `campaign_controller_commit` captures committed physics positions/bases before the controller position commit, then the resulting positions/bases after synchronization. It records runtime handle identity and separate translation/orientation change bits. Duration becomes valid only after the complete commit pass succeeds; it is invalidated at the next controller tick and on teardown. No per-frame allocation, checkpoint-format change or velocity extrapolation is introduced.

`scene_detached_sources_tests` exercises two movers, translation, rotation, reverse motion, a subsequent stationary frame, deliberately unrelated pending/public poses, a relative subinterval query built from the retained data, capacity failure, handle replacement and nonfinite input. Validation precedes writes across the entire array, so invalid later records cannot partially overwrite an earlier interval. All 126 PC tests pass (artifacts/mover-interval-ctest.log).

The current 180-frame lower-door replay opens both leaves and the player crosses the doorway (artifacts/mover-interval-door-open.log). The older 420-frame cycle fixture fails its close/reopen assertion with motion counters [419,419,8,0,2,2,0,0]. A control rebuilt from the previous HEAD scene.c produces exactly the same failure/counters (artifacts/mover-interval-baseline-door-cycle.log); current source was restored and rebuilt afterward. This is an existing fixture/door-cycle issue, not an accepted cycle test and not evidence against the snapshot regression. It remains in TO-DO.

These records now survive actual scene commits, but debris collision still uses the prior committed-surface path. Connecting the retained intervals to candidate queries and timed fragment scheduling, correctly filtering competing static mover candidates, and waking settled pieces remain unfinished. Rotating intervals are retained explicitly and must not be silently treated as pure translation.

Stock64MiB regression: artifacts/xemu/render-20260918-041523 passes all80 checks across600 frames with3314 free pages (12.95MiB), no plugged memory, and byte-exact5300-byte PC/Xbox checkpoint. Checkpoint SHA256 remains d27cfa99dae3f1595e695e85f32b777a8f8b856017c85b944cab34c241601aba. The framebuffer is byte-identical to the previously visually inspected render-20260918-020011 image (SHA256 ad18ceadb48c89b8c98548d169a7e9bc51b785197a4ac223c326bcc9bab41ca3); no duplicate screenshot was posted. The harness restored its disc and closed its owned emulator. This validates regression safety of the retained-motion allocation/lifecycle and current gameplay, not completed relative mover interaction.

Integration update: translating-mover queries now consume the retained intervals in live fragment scheduling, with native numerical acceptance. See GEOMOD-FRAGMENT-MOVING-SURFACES-20260918.md; this supersedes the earlier unconnected-query limitation, while settled wake, rotating sweeps and moving support response remain open.
