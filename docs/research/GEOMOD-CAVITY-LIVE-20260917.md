...
Native reload-and-second-shot evidence: `artifacts/xemu/render-20260917-235907` passed77 checks at301frames using the first PC wall save. Its8390-byte output checkpoint matches PC SHA256308d48325a90b763b4a01b7ff0c93e3ba94f1a042bc51cc48880a0eea2c3df47. The native framebuffer was inspected and shows the expanded crater.3943 free pages =15.40234375MiB at endpoint. Harness restored disc and closed its emulator. Xbox-created-save roundtrip and traversal remain separate follow-ups.


## Deeper wall cuts, 2026-09-18

Admission now accepts cutters wholly behind the original inward source plane when their projected footprint still fits one ordinary compiled window. It checks the AABB of the complete corridor from cutter to window against all other authored brush bounds, as well as the cutter itself. This prevents skipping through an intervening brush. Cross-window and neighboring geometry remain conservatively unsupported. Admission is a spatial ownership guard, not a proof of cutter connectivity; the sequential live impacts and collision probes below establish connectivity for the tested history.

The developer hardness patch grows from radius2 to radius4 around the same center at hardness65. The original radius2 correctly rejected the third impact atX-35.0759, because it was outside that test region. Installed data/default hardness remain untouched; the changed settings digest intentionally makes old radius2 fixture saves incompatible.

Core tests extend the original crater with centersX-34 andX-35. Rays toX-34.4 andX-35.4 are blocked before their respective cuts and clear afterward. Those distances are inside the actual template; an initial assumed0.8-unit depth exceeded its approximately0.5-unit reach on this axis. The full composed room also admits the deep ray, blocks theY7control, preserves656 unselected faces and retains the untouched window areas. Existing invalid-bounds, interior-only, brush-overlap and seam rejection tests remain in place.

The live replay keeps the same player aim and fires at frames240/420/600. Actual impacts are(-33,4,8),(-34.0371284,4.03803015,7.9977932),(-35.0759163,4.08295727,7.98823881); all three cuts commit and the player survives. The PC framebuffer was inspected: the original entrance remains open with a dark interior, while the surrounding wall/weapon/HUD remain intact. A distant frontal image alone does not establish depth; the impact positions and collision probes provide that evidence. Save/reload second-shot byte comparison still passes. All123 PC tests and the stock NXDK build pass, with a subsequent focused composition-depth test also passing.

Native three-shot evidence: `artifacts/xemu/render-20260918-000512` passed77 checks over800frames. Xbox and PC produced identical24076-byte saves, SHA25619861e70b435241288b75412d40b6cf335c2d251af72d1639937cf2345e5f882.3863 free pages =15.08984375MiB at endpoint. Native framebuffer inspected; harness restored disc and closed its emulator. The three-cut PC reload plus120 updates matches uninterrupted play byte-for-byte. Player traversal and Xbox-created-save reload remain unverified for this wall.
