...
## Collection shared journal restoration

Explicit RFDS3 adapters now import the shared admission state and rebuild the atlas journal in a private lighting stage. Legacy RFDS2 entry points retain their version gate. The collection journal verifies the directory source count matches the scene and the aggregate cuts equal the sum of local histories, with every local count bounded by the room serial. Existing packing, RNG, map containment, face binding and canonical substrate checks are reused.

The installed paired writer fixture imports its complete saved journal into a fresh private lighting clone, bakes/draws that clone, and captures scene digests. All512x512 packed atlas pixels, face-to-map bindings and three digests match the pre-save candidate exactly. A corrupted fourth-map seed rejects while the original empty live atlas owner remains unchanged. The test also reads the saved admission count/RNG; its admission list is empty, so nonempty collection admission coverage remains open.

The test must reacquire the active publication view after publication_finish: the prior pending collision view changes ownership on commit and is no longer the correct clone input. This was fixed in the harness rather than weakening lighting-stage validation. The new unused-by-gameplay adapters are static inline for NXDK warning-as-error compatibility.

All122 PC tests pass and stock NXDK produces the XBE/ISO after that build fix. Logs: artifacts/collection-journal-build.log, artifacts/collection-journal-focused.log, artifacts/collection-journal-tests.log and artifacts/collection-journal-xbox.log. No emulator session or HDD clone was created. Full collection orchestration still must combine private source reconstruction with shared publication/lighting, authenticate saved extension/digests, validate player support/placement, reserve the whole live/staged scene and commit atomically before live paired save/load is enabled.


## Combined private collection restoration

scene_authored_collection_stage.inc now orchestrates full RFDS3 reconstruction into a private scene context: header/source/settings checks, shared admission import, every source core/registry, combined pending publication/collision, shared atlas rebuild/bake/draw, face bindings and all three expected digests, followed by collection-extension comparison. Original live aliases stay unchanged. Discard releases lighting first, aborts pending publication and closes every reconstructed source. No player admission or live commit API is exposed yet.

The initial integration rejected under the13MiB destruction ceiling because the source helper reserved every registry's maximum reconstruction workspace concurrently. Reconstruction is sequential and completed owners are immutable during later factories. It now reserves the current full core/registry ceilings plus actual resident bytes of completed owners, updating the peak before each allocation. Lower-budget and later-source failure controls remain passing. The source-stage reservation in this fixture falls from6569948 to4204888 bytes without increasing any allocation ceiling.

The complete installed paired fixture reserves13546968 bytes (about12.92MiB), under the unchanged13631488-byte subsystem cap. It reconstructs both nonempty source registries and shared publication/atlas, and matches the pre-save digests and all atlas pixels. A damaged material-digest byte rejects only after reconstruction, discards the candidate and preserves active publication index/live core pointers. This is a conservative subsystem reservation on PC, not a whole-game64MiB peak measurement or native reload acceptance.

All122 PC tests pass and stock NXDK compilation succeeds. Validation logs are artifacts/collection-stage-build.log, artifacts/collection-stage-focused.log, artifacts/collection-stage-tests.log and artifacts/collection-stage-xbox.log. Player support/placement across staged source IDs, final atomic ownership transfer, nonempty collection admissions, reset reload, broader histories and native live continuation remain open. No emulator session or HDD clone was created.


## Player support and placement against staged collections

A collection checkpoint support provider now uses the existing all-source player-ground query, retaining its polygon-over-sphere tie rule and source-qualified batch resolution. It inspects the selected fragment's physics state and admits stable support only when sleeping with zero linear/angular velocity. A separate fit gate checks every source registry. The combined stage exposes standing admission against its full pending room collision and these privately restored registries; it normalizes overlap/unsupported results to invalid-save rejection.

Synthetic two-registry tests prove support can come from source1, matches the single-registry provider's fraction/normal, becomes unstable when that body moves, rejects a player overlapping the second registry, and preserves query outputs on a later-registry error. The installed combined restore admits a small0.1-radius test body above a downward-ray-measured floor and rejects it inside the second post. The initial guessed floor height was wrong; the test now derives the floor directly from reconstructed collision. These fixtures do not establish actual player-body standing on restored paired rubble or native continuation.

All122 PC tests pass and stock NXDK compilation succeeds. Validation logs: artifacts/collection-player-build.log, artifacts/collection-player-focused.log, artifacts/collection-player-tests.log and artifacts/collection-player-xbox.log. The admission gate remains separate from preparation so the eventual composed-player loader can supply its validated actual body. Gameplay collection save/load and final ownership transfer remain disabled; no emulator session or disk clone was created.


## Atomic collection ownership transfer

The prepared collection now has a synchronous commit operation. It validates source ownership and active selection, optionally runs the staged player gate, validates atlas image bounds, then asks publication_finish to validate/commit the shared render/collision state. Only after that succeeds does it copy prepared lighting, transfer every source core/registry, restore admission/RNG/serial state, repair the selected aliases, update source telemetry and close the old owners. The candidate's transferred pointers are cleared so discard cannot free live ownership. Repeated commit rejects.

The installed two-post fixture injects an invalid generated binding immediately before commit. Rejection preserves both original core/registry pointers and the active publication bank. Restoring the binding allows a successful commit: both owners change, the second source remains selected, and re-encoding the complete RFDS3 payload yields exactly the same4176 bytes. A player placed inside restored geometry also rejects at the commit gate without transfer. The successful scene-only fixture passes NULL for player; actual gameplay loading must provide validated actual-body placement.

All122 PC tests pass and stock NXDK compilation succeeds. Validation logs: artifacts/collection-commit-build.log, artifacts/collection-commit-focused.log, artifacts/collection-commit-tests.log and artifacts/collection-commit-xbox.log. This establishes a CPU scene save/reconstruct/commit/re-encode cycle, not game-facing checkpoint dispatch or native continuation. Those integrations, actual-body standing/rejection replays, reset checkpoints and broader histories remain open. No emulator session or disk clone was created.


## Paired gameplay checkpoint dispatch and PC continuation

Game-facing checkpoint capture now selects RFCP profile3/RFDS3 for an authored collection, retaining profile2 for single sources. Paired validate/restore dispatch uses complete private reconstruction, actual player-body standing/fit validation and atomic collection commit before player-state publication. Capture support and overlap checks visit every source registry. Clutter baseline identity remains anchored to collection slot0, independently of the selected source. Raw non-player collection capture remains rejected.

The first actual two-rocket capture was rejected because reconstruction was charged for lighting/digest scratch that did not exist yet: it requested3293148 bytes with3064700 left. The combined reservation now separates source reconstruction from atlas/digest preparation. Publication capacity and all live ownership remain charged in both phases. During reconstruction the current-source full factory ceilings plus completed retained owners are reserved; after that scratch is freed, the retained reconstructed cores/registries plus full atlas/digest work are checked separately. The maximum phase reservation must stay under the unchanged13MiB cap. This corrects overlapping lifetime accounting rather than weakening a cap.
...
## Paired reset checkpoint followed by another rocket

New tools/check_paired_reset_checkpoint.py extends the existing actual two-rocket/reset replay. It saves after both posts are restored, then loads that1192-byte RFCP into a fresh process and replays300 updates ending with another rocket into source93. The uninterrupted1150-frame control reaches exactly the same3016-byte composed checkpoint: source94 stays intact, source93 has one cut, serial advances from3 to4, and one66-triangle detached piece remains. The reset checkpoint has no cuts or detached pieces. Replay record0 is initialization, so the resumed input prepends a neutral record before uninterrupted records850..1149.

The initial harness compared the publication allocation peak as though it were persistent state. That counter correctly differs because the uninterrupted process previously performed two cuts. The harness now compares every other publication field and the complete composed checkpoint, retaining both peak values in raw logs. No runtime behavior was changed to make this test pass.

PC save/resume/control framebuffers were inspected individually: both posts are intact after reset; after the later rocket the left post is damaged and the right stays intact. The red prop partly obscures left-side rubble. Evidence is artifacts/paired-reset-checkpoint/report.json and the adjacent logs/checkpoints/native-format framebuffers. This does not cover unsafe reset, standing on paired rubble, or arbitrary campaign geometry.
...
## Tiny-fragment alpha collision and moving-save timing fix

The geometry body sweep now has an optional textured route for room trees, liquid-enabled room traversal and flat movers. It preserves the original small-body query0x100 bit and uses existing texture-aware face traversal; the old API retains its explicit unsupported-alpha gate. Scene tiny/alpha body queries build current terrain-room texture bindings in bounded retained scratch rather than indexing the original tree's bitmap order. Other alpha clients keep their original bindings. The extra backend array and one-room bitmap capacity are included in the existing128KiB binding budget; body queries do not allocate. The scene currently publishes one terrain room, and the adapter explicitly rejects multiple changed room bindings.

Core tests verify a radius0.004 body against transparent/opaque world faces and translated mover faces, correct contact fraction/object/material, and unchanged caller outputs on sampler failure. The legacy no-backend refusal remains tested. tools/check_tiny_fragment_collision.py reproduces both rocket cuts and the tiny fragment independently of the still-unfinished standing acceptance. The scene now completes instead of aborting at frame619.

This moving-body replay also exposed an extra physics step on checkpoint frame0. Player motion starts at frame1, but detached simulation advanced the restored body on frame0. Detached frame0 now publishes telemetry with zero elapsed time. The940-frame save and200-update continuation now produce exactly the same4624-byte RFCP as uninterrupted1140-frame control, including the tiny body's falling position and velocity. The second-source ordinary standing regression still passes. The tiny fragment starts below the floor and continues falling; out-of-world retirement and two-destroyed-source standing remain open and are not claimed by this crash fix.

Stock64MiB XEMU run artifacts/xemu/render-20260917-143718 completes the940-frame two-rocket reproduction and passes all76 checks. Its exported checkpoint matches PC exactly. Native framebuffer inspection shows the damaged post areas, retained right-side chunk partly obscured by the red prop, room and weapon/HUD. The tiny fragment is below the room and is not visible in this image; its state is verified through exported bytes. Endpoint free pages3653 (14.27MiB), not a minimum. Disc staging restored; no HDD copy or GitHub screenshot was created.


Native reload artifacts/xemu/render-20260917-143957 loads the Xbox-created checkpoint from the preceding run and executes201 replay frames (200 physics updates). All76 checks pass and its4624-byte export equals the uninterrupted PC control exactly, including falling-body position/velocity. The native endpoint image was inspected and retains the expected damaged room/weapon/HUD. Endpoint availability3799 pages (14.84MiB); disc staging restored. All122 PC tests pass, including the added transparent/opaque world/mover tests; stock NXDK compilation passes through both native harness runs. Validation logs: artifacts/tiny-alpha-{build,tests,continuation,standing-regression,native,native-reload}.log.


## Lifecycle correction and live standing/fit disagreement

The prior TODO directing below-world retirement was premature. GEOMOD-DETACHED-LIFETIME-20260917.md proves this producer disables the lifetime timer; GEOMOD-DEBRIS-OUTSIDE-ROOM-20260916.md finds no coordinate retirement in the separate examined decorative-debris paths. Neither proves the full generic terrain-fragment world-exit route, but neither justifies an invented kill height. No spatial deletion rule was added. That work is now an evidence question rather than an assumed implementation requirement.

The two-destroyed-source standing replay now aims the second rocket atY-0.3 instead of the tiny-fragmentY-0.5 case (the latter remains independently reproducible with check_tiny_fragment_collision.py). It destroys both posts and ordinary movement/jump leaves the player stationary at(-4.42032862,0.477345318,-2.50573325). Support telemetry resolves source slot1/batch0 (encoded batch16), sphere-mode contact, with both rubble bodies stopped. However composed save capture rejects the same position: the minimum sphere clearance against source94 is+4.04718421; against source93 it is-0.0249082726 (player sphere0, target sphere2). This exceeds the existing0.002 fit tolerance. An extra ten forward-input frames did not move the blocked player; that exploratory change was removed.

Error-only checkpoint diagnostics now measure/report every retained source rather than returning a generic collection-rubble error. No placement tolerance or collision response was weakened. The exact failure is artifacts/paired-both-support/saved.log and input saved.bin; the no-save run artifacts/paired-both-support/pose.log confirms the settled player/contact state. Next work must reconcile live contact/ground placement with save fit, including the original raw-normal ground gate and multi-sphere contact behavior. Paired standing is still NOT accepted; no native run was claimed for this case.

The finalized two-source input reproduces the same clearance values after removing the exploratory extra movement. The prior second-source standing/save/walk-away regression passes unchanged, and stock NXDK builds the diagnostic extension. Logs: artifacts/paired-both-fit-{build,regression,xbox}.log.


## Ground-support adjustment constrained by live rubble collision

A process-local trace isolates the first invalid fit at frame813. Ordinary movement ends at(-4.41333294,0.514354169,-2.50339007) with valid rubble clearance. Ground-support publication then lowers Y to0.474226892 and the same fit check rejects. The original-derived ground-sphere routine can reject a shallow raw normal and select deeper support; committing that numeric support proposal without a full-body collision check caused the penetration. Temporary frame-window tracing was removed after preserving artifacts/paired-both-support/fit-trace.log.

The scene now sweeps downward support adjustments through the same source-qualified rubble movement query used by ordinary movement, including all player spheres and both polygon/sphere fragment paths. It caps the proposed Y at the earliest contact and updates next position/bounds consistently. Original ground/contact math, source eligibility and the0.002 save-fit tolerance are unchanged. This is a reconstructed integration safeguard, not a claim that an identical extra support sweep has been recovered from the original executable. Scenes without rubble skip the additional query; no allocation is added.

The complete --both-destroyed PC standing test now passes: final standing position(-4.42144,0.518854,-2.505721), two live fragments, support from source93/slot1. Saved standing and walk-away both match uninterrupted RFCP bytes exactly, and the altered retired-support payload rejects. Saved/continued/retreat PC images were individually inspected; retreat visibly retains both tilted chunks and the damaged posts. The fragment under the standing camera is not visible in that close view, so standing is established by contact/position/checkpoint evidence.

Verification correction: the earlier tiny-alpha test run had used stale executables for two targets after unnoticed build errors. This turn fixes the mover fixture's flat input_matrix indexing and supplies the new elapsed-time argument in scene_detached_sources_tests. The translated-mover test also now supplies an empty-world texture backend, since tiny-body flags still require that route even when its world has no faces. A checked successful full PC build followed by all122 freshly built tests now passes. The prior single-source and second-source standing regressions pass. Logs: artifacts/paired-support-clamp-{build,tests,single,second}.log. Earlier native crash/reload runs remain independent evidence from their actual NXDK builds.


Stock64MiB native acceptance now passes both ends: artifacts/xemu/render-20260917-145648 executes940 frames, destroys both posts, jumps onto source93 rubble and creates a4430-byte RFCP identical to PC. artifacts/xemu/render-20260917-145845 loads that Xbox-created save and executes201 frames (200 updates) walking away; its4430-byte export exactly matches uninterrupted PC retreat. Each run passes76 comparisons. Endpoint availability is3653/3799 pages (14.27/14.84MiB), not minimum measurements. Both native images were inspected: the standing view is elevated beside the post; the retreat visibly retains both detached chunks, damaged posts and weapon/HUD. Both report restored disc staging. No HDD clone or GitHub upload was made. Larger bodies, moving support, stricter visual parity and broader histories remain open. The tiny-fragment moving-save regression also still passes (artifacts/paired-support-clamp-tiny.log).


## Larger-fragment support coverage (synthetic, not live acceptance)

Authored UID94 bounds are X[-5.25,-4.75], Y[-1.5,2], Z[2.25,2.75]. Ordinary rocket aim searches at Y0.5/1/1.5/2 produced respectively one radius0.355793 fragment or no retained fragments. Lower aims at Y-1.4/-1.2/-1.0 produced no retained fragments; all three process runs exited successfully. These outcomes do not establish why extraction was absent and are not large-body coverage. Reports: artifacts/larger-rubble-search/report.json and artifacts/larger-rubble-base-search/report.json. tools/find_large_rubble.py preserves the bounded process-local search with configurable heights/output.

Extended scene_detached_sources_tests with actual emitted cube geometry of half-extent0.8, whose computed body radius exceeds1 and therefore uses polygon collision. Eight cases cover both source slots, landing versus ongoing support, and obstructed versus clear descent. A radius0.6 player descending from Y2 must stop at Y1.4 (surface0.8 plus sphere0.6); outside the cube it must retain the original Y0.05 support proposal. Each case verifies pending position and bounds agree. The test calls the real actor_support_commit integration and would fail the obstructed cases if its collision clamp were removed. It neither changes production behavior nor claims original-binary equivalence for the added safeguard.

The target was rebuilt successfully and targeted CTest scene_detached_sources passed. Build log: artifacts/large-support-build.log. No Xbox build/run, native image, disk clone or additional GitHub screenshot was performed for this test-only extension. Real larger-chunk extraction/standing and moving support remain open; prior native smaller-rubble acceptance is unchanged.


## Valid post shot rejected after clipping: diagnosed and corrected

The aiming harness now records final GEOMOD/ROCKETS counters, signed edit status, committed cut count, retained radii and optional process-local impact/publication trace. Its report distinguishes rejected edits, committed edits without fragments, small fragments and large fragments. The report starts empty and updates after each completed case so an interrupted search cannot present an old complete report.

The low Y-1.2 candidate hits the actual floor at(-4.68967962,-1.25,2.5), returns RF_NOT_FOUND, and commits zero cuts. The nearby authored region UID11228 starts at Y-0.992641449 (center2.007358551 minus height6/2); the floor is outside it, and the level default hardness is100. The Y-0.9 control hits the post at(-4.75000048,-0.900497615,2.5) and commits a cut. A successful process exit is therefore insufficient destruction evidence.

A broader admitted-height search found a real defect at target Y0.3: impact(-4.75000048,0.299967051,2.5), core cut succeeds, but publication collision-face conversion rejects RF_FORMAT. Its source surface reference150 becomes a seven-corner face after clipping. The short edge from(-5.25,-1.24846113,2.25465441) to(-5.25,-1.25,2.25438333) gives an inward violation about-2.53e-5 after float rounding, exceeding the unchanged1e-5 convexity tolerance. Full vertices are retained in the regression test and artifacts/post-cut-invalid-face/0.log. No original screenshots were used.

Publication now routes final clipped polygons through the existing rf_geomod_partition_mesh before appending them. Already-valid polygons remain single faces. Invalid convex boundaries are partitioned using the terrain core's existing strict collision contract, retaining material and origin for every child. It uses existing publication result storage and capacity limits, adds no heap allocation, and rejects a failed partition before publishing live output. This is a reconstructed robustness fix, not a newly recovered original routine. Temporary per-face tracing was removed; compact trace-gated stage diagnostics remain.
