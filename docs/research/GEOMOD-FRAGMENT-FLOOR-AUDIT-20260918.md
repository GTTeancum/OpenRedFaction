...
## Fragment-specific partial-pose contact policy

Implemented `rf_physics_fragment_pose` / `rf_physics_fragment_step` as an explicit port policy. The original-derived `rf_physics_solid_advance` and legacy scheduler behavior remain available and unchanged for their callers. Fragment contacts now commit fractional translation and an orthonormalized interpolated orientation, refresh the world tensor, and publish that accepted basis. Pure angular motion also invokes the query. Existing contact response, material coefficients, mass, sphere generation, maximum ten response iterations and atomic scheduler publication remain in place. This is not claimed as a recovered retail angular-collision algorithm.

The scene query supplements the body-sphere query with unique mesh-corner sweeps across four pose intervals. It uses the existing finite-world/mover/material body-query route with zero-radius corner samples; no per-query allocation, host input, render-only vertex lift or unconditional floor snap is introduced. Two admission details were necessary:

- A shared birth corner can leave the back of a neighbor's face. The piece must have some mesh extent on the outward side of a corner-hit plane before that hit acts as an obstacle. Without this check the second west chunk stopped at birth against face140 near(-8.5,2,-4), rather than falling. The original +0.002 corner padding made this worse and is not used.
- Original occupancy-grid sphere radii can extend beyond the mesh. A sphere contact is ignored when the accepted mesh pose remains more than0.002units clear of its plane, allowing the later corner contact. Without this qualification the east fragments rested about0.14units above their floor despite passing a penetration-only gate.

`tools/check_fragment_support_audit.py` now supports `--max-clearance` for these known floor-resting fixtures. The joint acceptance command is `python -B tools/check_fragment_support_audit.py --max-penetration .005 --max-clearance .005`. All five pieces are sleeping, both output checkpoints preserve their untraced continuation, and measured finite-floor gaps are0,0.000100135803 for group92 and0,0,0 for group108. Their earlier west gaps were-0.106614828 and-0.061811209. Fresh full first/second-junction, protected-trim rejection, saved rejection, reload and uninterrupted comparisons pass for both groups. The older95 beam/both-posts/second-cut/settle replay also preserves exact continuation.

Tests add a rotation-only contact that must query and publish a partial basis, plus finite/atomic pose controls, and distinguish outward supporting mesh extent from a shared backside boundary. All123 PC tests pass (`artifacts/fragment-pose-final-tests.log`). The four contact-trace runs preserve their corresponding new baseline saves. The trace now measures accepted fractional rotation; the preceding historical section describes the earlier full-rotation diagnostic and is not the new trace's semantics.

Limits remain explicit: four angular intervals and corner samples are bounded approximations, not a continuous full polyhedron collision solver. Mesh-plane qualification does not prove every finite-face edge case. Broader thin obstacles, edge/edge contacts, moving supports, large angular steps, tightly confined birth poses and cumulative Xbox cost still need coverage. Existing sleeping saved poses are not retroactively depenetrated. These fixtures demonstrate corrected resting contact, not complete GeoMod fidelity.


Native validation: `artifacts/xemu/render-20260918-015509` ran group92's second junction from the first-cut save for501frames, passing77 checks on stock64MiB with3042pages free and exact7702-byte PC/Xbox checkpoints (SHA289b393de3dd45d04b15b78df6a297f79171c5b2dc7950f9ca091e5ca86d8211). This run preceded the final sphere-padding qualification; its checkpoint also matches the final PC result exactly. `artifacts/xemu/render-20260918-020011` then ran the final implementation through a fresh group108 first cut for600frames, passing77 checks with3332pages free (13.02MiB) and an exact5300-byte checkpoint (SHAd27cfa99dae3f1595e695e85f32b777a8f8b856017c85b944cab34c241601aba). Both native framebuffers were inspected: the level, weapon/HUD, cut beam and rubble are present. The second image has high visible wood shapes, so the first-cut endpoint was also audited rather than inferring suspension from the image: `--first-cut --max-penetration .005 --max-clearance .005` confirms all four first-cut fragments (one west, three east) sleep with measured floor gap0 and exact saved continuation. This does not identify every high visible polygon or prove every contact edge.

Both native runs restored the harness disc and closed their owned emulator. No GitHub images were uploaded. `tools/check_large_rubble_standing.py` also passes standing, saved neutral continuation and walk-away comparisons against its retained large-fragment checkpoint (`artifacts/fragment-pose-standing.log`). This checks existing saved support compatibility, not regeneration of that fixture under the new policy. The final native run and PC rest gates validate the implemented correction; performance benchmarking and broader shape coverage remain follow-up work.
