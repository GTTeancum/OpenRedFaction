...
enum rf_world_checkpoint_section {
    ...
    RF_WORLD_CAMPAIGN_HISTORY
};
enum {RF_WORLD_CHECKPOINT_SECTIONS=15,RF_WORLD_CHECKPOINT_HEADER=128,
    RF_WORLD_CHECKPOINT_DIRECTORY_ROW=12,RF_WORLD_CHECKPOINT_PREFIX=308};
...
/* Structural RFWC1 envelope only, capped at RF_CHECKPOINT_FILE_MAX unchanged.
 * Wire: 128-byte header (magic RFWC at0, u32 version1 at4, total at8,
 * FNV1a checksum at12, section count at16, reserved zero at20, identity at24,
 * level at56, reserved zero at120/124), then 15 directory rows, then payload.
 ...
 * includes the 308-byte prefix. Components, including campaign history, are
 * opaque: presence alone does not establish complete gameplay restore support.
 * Fixed canonical directory lists all 15 types once, sorted, with contiguous
