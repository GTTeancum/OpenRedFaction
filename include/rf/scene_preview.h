...
extern uint32_t rf_scene_player_form[8];
extern uint32_t rf_scene_player_model[4]; /* class: miner/suit/scientist, compiled bytes, bones, reserved */
...
/* Read-only diagnostic of live fragment mesh/sphere clearance to downward floor rays. */
...
extern uint32_t rf_scene_fragment_edge_audit[32];
extern uint32_t rf_scene_fragment_moving_audit[16];
extern uint32_t rf_scene_fragment_support_audit[16];
...
extern uint32_t rf_scene_npc_drive_test[4];
...
/* Caller has selected the ordinary angular branch. SP drive state comes from
 * the registered player/control lookup48aa30.
...
extern uint32_t rf_scene_burning[5];
extern uint32_t rf_scene_burning_visual[5];
extern uint32_t rf_scene_scanner[4];
extern float rf_scene_scope_projection; /* world-only magnification, default1 */
...
extern uint32_t rf_scene_rifle_alt[8];
...
/* Opt-in paired-post developer setup; collection checkpoint support pending. */
...
extern uint32_t rf_scene_fragment_platform_enabled,rf_scene_fragment_platform_audit[32];
#endif
