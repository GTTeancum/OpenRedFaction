...
for level in levels:
 ...
 rooms,faces,primary,children,retained,peak,queries,hits,errors,checksum=map(int,run.stdout.split())
 assert rooms==level['rooms'] and faces==level['faces'] and errors==0,(level['file'],errors)
 results.append(dict(file=level['file'],rooms=rooms,faces=faces,primary=primary,children=children,retained=retained,peak=peak,queries=queries,hits=hits,checksum=checksum))
