"""Verify deferred loop, non-loop and individual stops against original x86."""
...
for k in range(1800):
    ...
    weight=rng.choice([0,.25,.5,1]);restart=2+k%3;freeze=rng.choice([0,1,2,257,-255])
...
for k,(s,resources,mid,w,restart,freeze) in enumerate(cases):
    ...
    u.emu_start({2:0x51c340,3:0x51c390,4:0x51c3f0}[restart],stop,count=100000);assert u.reg_read(UC_X86_REG_EIP)==stop
...
report=dict(result='PASS',cases=len(cases),scope='Original 0x51c340, 0x51c390 and 0x51c3f0; exact flag tests, deferred removal and primary clearing; all state and references')
(root/'artifacts/motion-stop-verification.json').write_text(json.dumps(report,indent=2));print(report)
