"""Compare original override/cache evaluation with shared PC matrices and stamps."""
...
for name in ('ult2_stand.rfa','ult2_crouch.rfa'):
        ...
        for bone in (0,1,8,count-1):
            for weight in (0,.25,.5,1):
                ...
                for enabled in (1,2,127,128,255):
                    ...
                    commands.append(struct.pack('<ii9fB3xf',tick,bone,0,1,0,-1,0,0,0,0,1,enabled,weight))
                    expected.append(overridden+struct.pack('<'+'H'*count,*([1]*count)))
                    ...
                    expected.append(baseline+struct.pack('<'+'H'*count,*([2]*count)))
    ...
    run=subprocess.run([str(root/'build/pc/Release/rf_skeleton_probe.exe'),str(root/'Installed_Game'/entries['miner.v3c'][0]),str(root/'Installed_Game'/entries[name][0]),'miner.v3c',name],input=b''.join(commands),env=dict(os.environ,RF_PROBE_OVERRIDES='1'),capture_output=True,check=True)
    want=b''.join(expected)
    assert len(run.stdout)==len(want),(name,len(run.stdout),len(want))
    for i,(a,b) in enumerate(zip(run.stdout,want)):
        assert a==b,(name,'snapshot',i//(count*50),'byte',i%(count*50),a,b)
assert changed>0
report=dict(result='PASS',cases=cases,changed_poses=changed,bones=count,
 scope='Unhooked original 0x51b500: miner stand/crouch, four bones, four weights, all tested nonzero flag bytes; clear preserves current-generation cache and next generation restores baseline. Shared PC evaluator matches all matrices and generation stamps over all three passes; no Xbox runtime execution.')
(root/'artifacts/bone-override-evaluation.json').write_text(json.dumps(report,indent=2));print(report)
