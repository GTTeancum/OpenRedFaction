...
rng=random.Random(0x49d330);commands=[];expected=[]
...
if '--nxdk' in sys.argv:
 ...
 for case,command in enumerate(commands):
  ...
  native_cases+=1
report=dict(result='PASS',cases=len(expected),nxdk_cases=native_cases,scope='Original nonrigid stationary contact, explicit surface coefficients; no random-normal/vehicle/contact query/scheduler')
