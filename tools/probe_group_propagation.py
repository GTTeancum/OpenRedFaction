...
port_cases=[];port_expected=[];binding_cases=[]
def port_pose(blob):
...
for n in range(2000):
 ...
 contributions=[];dirty=False;port_contributions=[];binding_controllers=[]
 for i in range(count):
  ...
  binding_controllers.append(struct.pack('<6f3I',*first,*pending,control_flags,*member_handles))
 ...
 binding_cases.append(struct.pack('<2If',count,force,dt)+port_pose(initial)+b''.join(binding_controllers)+bytes(36*(2-count)))
