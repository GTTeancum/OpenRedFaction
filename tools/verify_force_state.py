...
for case in range(1024):
    ...
    u.mem_write(base+0x3290,w(51));u.reg_write(UC_X86_REG_ECX,base+0x3000)
    u.mem_write(stack,w(stop));u.reg_write(UC_X86_REG_ESP,stack)
    u.emu_start(0x4b9070 if action else 0x4b9f80,stop,count=100000)
...
report=dict(result='PASS',cases=1024,original_sha256=digest,scope='Original type51 dispatch4b9070/4b9f80 and handlers4b9330/4ba130 including45d6d0 UID lookup and list callees unchanged. PC/NXDK entire records agree; only first matching low activation byte changes. Empty collections/links, duplicates, missing IDs, zero and UINT32_MAX IDs, padded activation bytes. Event dispatch integration remains separate.')
