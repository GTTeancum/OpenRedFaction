    ...
    uint32_t fog_enabled,uint32_t fog_rgb)
{
    ...
    uint32_t i,j,base_mode=mode&~(31u<<20),depth_mode=(mode>>20)&31u,glow,corona=mode==0x06010c41u,solid=mode==0x18000u;
    int x,y,x0,x1,y0,y1;
    ...
    if((base_mode!=(RF_PARTICLE_NORMAL_MODE&~(31u<<20)) && base_mode!=(RF_PARTICLE_GLOW_MODE&~(31u<<20)) && !solid && !corona) || depth_mode>1)return RF_NOT_FOUND;
    ...
    for(y=y0;y<=y1;y++)for(x=x0;x<=x1;x++) {
        ...
        for(i=1;i+1<count;i++) {
            ...
            else sample(image,uv[0]/q,uv[1]/q,!corona,texel);
            for(k=0;k<3;k++) {
