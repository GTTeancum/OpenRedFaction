...
for i in range(4096):
 count=i%17;weighted=i%2;weighted_cases+=weighted;position=[rng.uniform(-5,5) for _ in range(3)];normal=[rng.uniform(-1,1) for _ in range(3)];initial=[rng.random()*.1 for _ in range(3)];sources=[]
 ...
 data=w(count,weighted)+f(*position,*normal,*initial,.25)+b''.join(sources)+weights;inputs.append(data);x.mem_write(B,data);x.mem_write(OUT,b'\xa5'*12)
 assert call('rf_vfx_light_accumulate',[B+8,B+20,B+32,read(x,B+44),B+48,count,B+1264 if weighted else 0,OUT])==0
 ...
 o.mem_write(STACK,w(STOP,OUT,OUT+4,OUT+8,B+8,B+20,MASK if weighted else 0,0,0));o.reg_write(UC_X86_REG_ESP,STACK);o.reg_write(UC_X86_REG_FPCW,0x27f);o.emu_start(0x4da8b0,STOP,count=1000000);assert o.reg_read(UC_X86_REG_EIP)==STOP
 ...
 x.mem_write(B+32,data[32:44]);assert call('rf_vfx_light_accumulate',[B+8,B+20,B+32,read(x,B+44),B+48,count,B+1264 if weighted else 0,B+32])==0;assert bytes(x.mem_read(B+32,12))==got
