...
for case,(name,heights,first,winner,fraction,height) in enumerate(fixtures):
 ...
 for i,z in enumerate(heights):
  ...
  u.mem_write(solid+0x70,struct.pack('<I',face if heights[i] is not None else 0));u.mem_write(solid+0x9c,struct.pack('<3I',1,1,array));u.mem_write(array,struct.pack('<I',room))
 ...
 if hit:
  ...
  expected.append(struct.pack('<iI',0,1)+raw[24:28]+raw[:24]+struct.pack('<4I',object_id,source if source<2 else 0xffffffff,0xffffffff,0))
...
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--solid-ray-flat'],input=b''.join(cases));assert len(raw)==len(cases)*52
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 ...
 for i,z in enumerate(heights):
  ...
  x.mem_write(solid+4,struct.pack('<I',0));x.mem_write(solid+148,struct.pack('<II',face,enabled[i]))
...
report=dict(result='PASS',cases=len(cases),hits=sum(struct.unpack_from('<I',v,4)[0] for v in expected),scope='Complete original 498e80 and all callees unchanged, two movers plus static solid, all using zero-room flat-face fallback, identity poses, null material. Exact geometric/object/index result on PC and NXDK. Original output fraction reuse and first-hit order retained; no live moving-solid extraction or actor response.')
(root/'artifacts/collision-solid-flat-verification.json').write_text(json.dumps(report,indent=2));print(report)
