...
actual=subprocess.check_output([str(root/'build/pc/Release/rf_entity_probe.exe'),'--ai-search-solid'],input=b''.join(commands))
...
entry=int(re.search(r'\s_rf_entity_navigation_search_solid\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
...
for i,((cfg,seeds,enabled),want) in enumerate(zip(records,expected)):
 ...
 x.mem_write(XQ,w(0,cfg['goal'],ALT if cfg['alternate'] else 0)+f(cfg['limit'],1,1)+w(0x12345678));x.mem_write(ROUTE,bytes(20));x.mem_write(OUT,w(99));x.mem_write(STACK,w(STOP,REFS,n,XQ,SCRATCH,8,WORLD,ALT,ROUTE,OUT));x.reg_write(UC_X86_REG_ESP,STACK);x.reg_write(UC_X86_REG_FPCW,0x27f)
