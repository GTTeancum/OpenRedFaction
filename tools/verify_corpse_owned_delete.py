...
backend=b+0xd000;effect=b+0xd100;sound_cb=b+0xd110;text=b+0xd200;stack=b+0x3e000;stop=b+0x3f000;base=19344
...
def hook(cpu,address,size,data):
 ...
 if address in (load_cb,motion_cb,create_effect_cb,emitter_cb):
  ...
  if address==load_cb:raise AssertionError('unexpected replacement model load')
   ...
   assert read(sp+8)==source;name=bytes(cpu.mem_read(read(sp+12),32)).split(b'\0',1)[0];create_trace.append(('motion',name));result=0xffffffff
   if abort_mode and ((mode==0 and name==b'death_front') or (mode==2 and name==b'corpse_drop') or (mode==3 and name==b'corpse_carry')):result=45
  elif address==emitter_cb:
   assert abort_mode and mode>=2;cpu.mem_write(emitters,w(0,88));result=emitters
  else:
 ...
 if address==malloc:
  ...
  sid=read(sp+8);assert a==123 and read(names+12)==0 and read(body+320)>0 and (abort_mode or read(names+4)>0)
  trace.append(('sound',sid));cpu.reg_write(UC_X86_REG_EAX,sound if case%2 and not abort_mode else 0)
 else:
  ...
  if op==0:assert (abort_mode or read(names+4)>0) and read(names+12)>0 and read(body+320)>0
  if op==2:assert read(names+12)==0 and read(body+320)>0 and read(counts)==1
  if op in (4,5):assert read(body+320)==0 and read(names+12)==0 and (abort_mode or read(names+4)>0) and read(counts)==0
  if op==5:
...
def memory(cpu,access,address,size,value,data):
 assert not (retired and address<c+640 and address+size>c),('owner read after recycle',hex(address))
...
for case in range(256):
 ...
 assert call(init,b,base+(24 if abort_mode and mode==1 else 128))==0;call(reginit,registry)
 ...
 if create_mode:
  create_trace.clear();u.mem_write(ch,w(ch,ch));u.mem_write(source,bytes(284));u.mem_write(source+12,w(77 if abort_mode or case%3 else 0));u.mem_write(source+36,w(2))
  u.mem_write(source+44,f(10,3));u.mem_write(source+68,f(1,100,1,0));u.mem_write(source+88,w(0xffffffff,0 if abort_mode and mode>=2 else 0xffffffff))
  ...
  if abort_mode:
   ...
   assert read(counts)==(1 if mode>=2 else 0) and read(counts+4)==1
   assert not any(item in create_trace for item in [('effect',4),('effect',5)]) or mode==4
   handle=read(c+116);labels.update({read(body+308):'body'})
   if read(names+12):labels[read(names+12)]='death'
   u.mem_write(backend,w(effect,sound_cb,123));args=(b,0,registry,counts,counts+4,1,backend)
   if mode==4:assert call(abort,*args)==0xfffffffc
   else:
    u.mem_write(c+116,w(handle^0x10000));assert call(abort,*args)==0xfffffffd and not trace
    u.mem_write(c+116,w(handle))
   deleting=True;assert call(delete if mode==4 else abort,*args)==0
   want=[('pairs',handle)] if mode==4 else []
   if mode>=2:want.append(('free','death'))
   if mode==4:want.append(('sound',1234))
   want.extend([('free','body'),('model',77)])
   if mode>=2:want.append(('emitter',88))
   want.extend([('recycle',handle),('registry_remove',handle)])
   assert trace==want,(case,trace,want)
   assert retired and not live and read(b+base-8)==base and read(registry+12292)==1024 and read(counts)==read(counts+4)==0
   assert read(c+36)==1234 and (mode!=0 or read(c+112)==0xdead0001)
   assert call(abort,*args)==0xfffffffc
   continue
  assert result==0 and read(out)==c
