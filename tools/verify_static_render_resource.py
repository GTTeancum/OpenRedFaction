...
for archive in json.loads((root/'artifacts/inventory.json').read_text())['files']:
 for e in archive.get('vpp',{}).get('entries',[]):
  ...
  sections=[s for s in inspect(raw)['sections'] if s['type']=='0x5355424d'];parts=bytearray();materials=bytearray();p_lines=[];g_lines=[];li=0;bytes_owned=44+48*len(sections);bad=False
  for s in sections:
  ...
  records.append(dict(model=e['name'],parts=len(sections),lods=li,bytes=bytes_owned))
report=dict(result='PASS',models=len(records),unsupported=unsupported,animated_rejections=animated,max_resource_bytes=max(r['bytes'] for r in records),lods=sum(r['lods'] for r in records),scope='PC actual archive composition; serialized parts/material/threshold/plane hashes, geometry probe agreement, exact/short budgets, outputs retained after archive close and repeated cleanup. Lower-level geometry decoding separately archive verified; no complete original loader or native XEMU claim.',records=records)
