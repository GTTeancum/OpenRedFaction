"""Original49ab00 with real geometry/vector helpers; actor velocity lookup supplied."""
import hashlib,json,math,random,re,struct,subprocess,sys
from pathlib import Path
import pefile
root=Path(__file__).resolve().parents[1];sys.path.insert(0,str(root/'local/python'))
from unicorn import Uc,UC_ARCH_X86,UC_MODE_32,UC_HOOK_CODE
from unicorn.x86_const import UC_X86_REG_ESP,UC_X86_REG_EIP,UC_X86_REG_EAX,UC_X86_REG_FPCW
b=0x30000000;stack=b+0xe000;stop=b+0xf000
w=lambda *v:struct.pack('<'+'I'*len(v),*(v&0xffffffff for v in v))
f=lambda v:struct.pack('<'+'f'*len(v),*v)
def machine(path):
 p=pefile.PE(str(path));im=p.get_memory_mapped_image();base=p.OPTIONAL_HEADER.ImageBase
 m=Uc(UC_ARCH_X86,UC_MODE_32);m.mem_map(base,(len(im)+4095)//4096*4096);m.mem_write(base,im);m.mem_map(b,65536);m.reg_write(UC_X86_REG_FPCW,0x27f);return m
exe=root/'Installed_Game/RF.exe';sha=hashlib.sha256(exe.read_bytes()).hexdigest()
assert sha=='b8fb9ab4c9bfc6f2868c30839d6cfc69f84b8c25d7e54eee1325f5b633c9b836'
u=machine(exe);x=machine(root/'build/xbox/main.exe');mapping=(root/'build/xbox/main.map').read_text()
entry=int(re.search(r'\s_rf_collision_actors_normal_response\s+([0-9a-fA-F]+)',mapping)[1],16)
trace=[];current=[]
def hook(m,address,size,native):
 if address!=(b+0xd000 if native else 0x426fc0):return
 sp=m.reg_read(UC_X86_REG_ESP);args=struct.unpack('<3I',m.mem_read(sp,12));handle=args[2] if native else args[1];trace.append(handle)
 result=0
 for i,row in enumerate(current):
  if struct.unpack('<I',row[64:68])[0]==handle and struct.unpack('<I',row[176:180])[0]:result=b+0x6000+i*0x1000+(0x8a0 if native else 0)
 m.reg_write(UC_X86_REG_EAX,result);m.reg_write(UC_X86_REG_ESP,sp+4);m.reg_write(UC_X86_REG_EIP,args[0])
u.hook_add(UC_HOOK_CODE,hook,False);x.hook_add(UC_HOOK_CODE,hook,True)
def run(m,address,args):
 trace.clear();m.mem_write(stack,w(stop,*args));m.reg_write(UC_X86_REG_ESP,stack);m.emu_start(address,stop,count=20000)
 assert m.reg_read(UC_X86_REG_EIP)==stop and m.reg_read(UC_X86_REG_FPCW)==0x27f
 return w(m.reg_read(UC_X86_REG_EAX)&255)+w(len(trace),*(trace+[0,0])[:2])
rng=random.Random(0x49ab00);commands=[];answers=[];counts=[0,0,0,0];lookups=0
for case in range(4096):
 rows=[]
 for i in range(2):
  pos=[rng.randrange(-16,17)/4 for _ in range(3)];nxt=[p+rng.randrange(-16,17)/4 for p in pos]
  radii=[rng.choice((0,.25,.5,1,2)) for _ in range(4)];count=rng.choice((-1,0,1,2,4));flags=rng.choice((0,0x40000000,0xc0000020));time=rng.choice((0,.01,.1,.5,1,2))
  bounds=[p-2 for p in pos]+[p+2 for p in pos]
  if case<2048:
   pos=[0,0,0] if i==0 else [0,case%3,2.5];nxt=pos.copy();nxt[2]+= (0,1,2,-1)[case//2%4] if i==0 else (-4,-2,-1,0)[case//8%4]
   radii=[1,.5,.25,0];count=case//32%5;bounds=[-10,-10,-10,10,10,10];time=(0,.05,.1,.5,1,2)[case//160%6] if i==0 else (0,.05,.1,.5,1,2)[case//27%6]
  velocity=[rng.randrange(-32,33)/8 for _ in range(3)];mass=rng.choice((.5,1,2,10))
  contact=f([11,12,13,14,15,16,time])+w(81)+f([19,20,21,22])+w(83,84,85,86,87)
  rows.append(f(bounds+pos+nxt+velocity+[mass])+w(101+i,31+i,flags,count)+f(radii)+contact+f([.125,-.25,.5])+w(case//(i+1)%2))
 current=rows;commands.append(b''.join(rows));before=[]
 u.mem_write(0x7c6a6c,w(1));u.mem_write(0x7c5a58,bytes(12))
 for i,row in enumerate(rows):
  obj=b+i*0x2000;native=b+0x8000+i*0x100;spheres=b+0x4000+i*0x100
  for m in (u,x):
   m.mem_write(spheres,b''.join(bytes(12)+row[80+j*4:84+j*4]+bytes(8) for j in range(4)))
   m.mem_write(b+0x68a0+i*0x1000,row[164:176])
  u.mem_write(obj,b'\xa5'*0x2000)
  for dst,src,n in ((0x190,0,24),(0xe4,24,24),(0x144,48,12),(0x98,60,4),(0x2c,64,4),(0x1fc,68,4),(0x1a8,72,4),(0x184,76,4),(0x1b4,96,68)):u.mem_write(obj+dst,row[src:src+n])
  u.mem_write(obj+0x188,w(4,spheres));before.append(bytes(u.mem_read(obj,0x2000)))
  x.mem_write(native,row[:80]+w(spheres)+row[96:164])
 expected=run(u,0x49ab00,[b,b+0x2000]);actual=run(x,entry,[b+0x8000,b+0x8100,b+0xd000,0]);lookups+=struct.unpack('<I',expected[4:8])[0]
 for i,row in enumerate(rows):
  obj=b+i*0x2000;native=b+0x8000+i*0x100
  expected+=row[:72]+bytes(u.mem_read(obj+0x1a8,4))+row[76:80]+bytes(u.mem_read(obj+0x1b4,68))
  actual+=bytes(x.mem_read(native,80))+bytes(x.mem_read(native+84,68))
  after=bytes(u.mem_read(obj,0x2000));preserved=bytearray(before[i]);preserved[0x1a8:0x1ac]=after[0x1a8:0x1ac];preserved[0x1b4:0x1f8]=after[0x1b4:0x1f8];assert bytes(preserved)==after
 assert expected==actual,(case,commands[-1].hex(),expected.hex(),actual.hex())
 result=struct.unpack('<I',expected[:4])[0];mode=0 if not result else 1 if expected[4:8]==w(2) else 2 if expected[88:92]!=rows[0][72:76] else 3;counts[mode]+=1
 answers.append(expected)
actual=subprocess.check_output([str(root/'build/pc/Release/rf_entity_probe.exe'),'--actor-response'],input=b''.join(commands))
for i,expected in enumerate(answers):assert actual[i*312:(i+1)*312]==expected,('PC',i,commands[i].hex(),expected.hex(),actual[i*312:(i+1)*312].hex())
assert all(counts)
report=dict(result='PASS',cases=len(commands),outcomes=counts,lookups=lookups,original_sha256=sha,scope='Full original49ab00 with actual46c340, sphere scans,508e40, normalization and vector helpers. Initialized static zero vector; pure426fc0 extra-velocity lookup supplied. Exact PC/NXDK return, lookup order, fields and preserved surrounding object bytes under027f. Finite geometry/positive masses; live response dispatch excluded.')
(root/'artifacts/actor-response.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
