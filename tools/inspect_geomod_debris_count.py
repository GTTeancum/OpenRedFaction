...
def run():
    ...
    for radius in (.1,.125,.5,1,3.75,5):
     configs=[(label,[entry]*14) for label,entry in [
        ('miss',(0,0,0,0)),('near',(1,1,0,radius*.25)),('far',(1,1,0,radius)),
        ('flag8',(1,1,8,0)),('no_face',(1,0,0,0)),('zero',(1,1,0,0)),
        ('hit2',(2,1,0,0)),('other_flags',(1,1,0xfffffff7,0))]]
     for selected in range(14):
        entries=[(0,0,0,0)]*14;entries[selected]=(1,1,0,radius*.33)
        configs.append(('only_'+str(selected),entries))
     configs.append(('mixed',[(i%3,i%2,(i%4)*4,radius*(i%5)/4) for i in range(14)]))
    ...
    print('PASS:',len(cases),'original/shared debris counts and14 probe endpoints; flags, misses, rounding and cap')
