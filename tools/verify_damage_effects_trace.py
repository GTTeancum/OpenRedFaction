...
mutation_hits={a:0 for a in (0x428740,0x4196f0,0x42e910,0x504e40,0x4089f0,0x4085f0,0x5056a0,0x4a7520,0x407fb0)}
...
def hook(m,address,size,context):
    ...
    if address==mutation[0]:
        mutation_hits[address]+=1
        m.mem_write(b+0x34,w(mutation[1]))
        for off,mask in zip((0x810,0x814,0x13d8,0x854),mutation[2:]):
            m.mem_write(b+off,w(struct.unpack('<I',m.mem_read(b+off,4))[0]^mask))
