...
actual=subprocess.check_output([str(root/'build/pc/Release/rf_physics_probe.exe'),'--accumulate'],input=b''.join(commands))
...
entry=int(re.search(r'_rf_physics_spheres_accumulate\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
...
(root/'artifacts/physics-sphere-mass-verification.json').write_text(json.dumps(report,indent=2));print(report)
