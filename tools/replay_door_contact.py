...
root=Path(__file__).resolve().parents[1];p=argparse.ArgumentParser();p.add_argument('--native',action='store_true');p.add_argument('--idle',action='store_true');p.add_argument('--frames',type=int,default=180);args=p.parse_args();assert args.frames>0
folder=root/'artifacts/door-contact';folder.mkdir(exist_ok=True);source=folder/'inputs.bin'
source.write_bytes(struct.pack('<5fI',0,0,0 if args.idle else 1,0,0,0)*args.frames)
env=dict(os.environ)
...
if not args.idle:assert sweeps[2]>0 and contact[17] in (0,1)
...
if (args.idle and args.frames>=180) or args.frames==180:
...
report=dict(live_motion=motion,door_positions=positions,live_activation=activation,trigger_contacts=triggers,result='PASS',frames=args.frames,idle=args.idle,staged=True,body_sweeps=sweeps,contact=contact,
