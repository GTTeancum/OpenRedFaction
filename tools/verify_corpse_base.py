...
commands=[];expected=[]
def observe(v):
 commands.append(f(10,v['mass'],*v['position'],*v['basis'],v['radius'])+w(v['flags'])+f(.25,.5,2)+w(v['count'],v['generation'],v['slot'])+v['spheres'])
 read=v['read'];a=v['actor'];u=v['u']
 expected.append(w(*[read(a+o) for o in (0x2c,0x34,0x7c,0x80,0x78,0x180,0x1a8,0x26c,0x1fc,0x2cc)])+bytes(u.mem_read(a+0x3c,48))+v['state']+w(v['used'])+v['owned'])
...
for case,(wire,want) in enumerate(zip(commands,expected)):
 values=struct.unpack('<22I',wire[:88]);used=values[19] or 1;budget=18144+used*24
 ...
 u.mem_write(seed,wire[:60]+w(source,values[19],values[15]));u.mem_write(source,wire[88:] or bytes(24))
 ...
 args=(b,registry,head,countp,seed,*values[16:19],out)
 ...
 handle=read(c+116);assert handle==struct.unpack_from('<I',want)[0]
 ...
 got=w(*[read(c+offset) for offset in (116,0,8,28,172,176,208,132,140,36)])+bytes(u.mem_read(c+40,48))+bytes(u.mem_read(body,308))+w(read(body+312))+bytes(u.mem_read(read(body+308),used*24))
...
report=dict(result='PASS',cases=len(commands),heap_failure_cases=len(commands),exhausted_registry_cases=len(commands),nxdk_sha256=hashlib.sha256((root/'build/xbox/main.exe').read_bytes()).hexdigest(),scope='Represented base fields/body bytes vs full original486da0; real shared owner allocation, registry generation and object list binding. PC/NXDK, no native XEMU or live room/string/model binding. Preserves incoming sound; negative seed radius is normalized in a local copy.')
