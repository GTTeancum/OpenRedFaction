...
results=[]
for frames in (59,180,600,1500,1501):
 ...
 state,actor,routes=words('SCRIPT_MOVE'),words('SCRIPT_ACTOR'),words('SCRIPT_ROUTES');assert state[7]==0 and routes[6]==0,(state,routes)
 if frames==59:assert state==[0]*8 and actor==[0]*8,(state,actor)
  ...
  if frames==180:assert state[1]==120 and state[3]==0 and math.dist(start,position)>2,(state,position)
  elif frames==600:assert state[1]==540 and state[3]==0 and position[0]>12 and routes[1]>=2 and routes[3]>=5,(state,routes,position)
  else:assert state[1]==638 and state[3]==frames-698,(state,position)
 results.append(dict(frames=frames,state=state,actor=actor,routes=routes))
assert results[-1]['actor']==results[-2]['actor'],'Blocked frame must preserve position'
report=dict(result='PASS',scope='Explicit authored Goto dispatch, delayed waypoint following beyond the first obstruction and later geometry stop; support/vertical motion, final route completion and locomotion animation remain open.',start=start,cases=results)
