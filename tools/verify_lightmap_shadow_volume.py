...
for at,value in [(0,f(float('nan'))),(16,f(float('inf'))),(28,f(float('nan'))),(40,data[16:28])]:
...
report=dict(result='PASS',original_pc_nxdk_volumes=len(inputs),planes=len(inputs)*6,pc_nxdk_guards=4,original_sha256=sha,x87_control_word='0x027f',scope='Original4f4c03..4f4daa with actual plane constructors, three-point normalization, orientation loop and final receiver-plane replacement. Axis-aligned and arbitrary supplied corners/normals/origins; exact96-byte outputs. Reuses reconstructed visibility plane math. Facing test, UV corner setup, face culling, complete masks and native rendering excluded.')
