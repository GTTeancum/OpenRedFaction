"""Ordinary side-beam92/108 rocket, saved second shot, uninterrupted comparison."""
...
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'artifacts'/('side-beam'+str(args.source));folder.mkdir(exist_ok=True)
...
env.update(RF_REPLAY_LEVEL='ctf06.rfl',RF_REPLAY_ARCHIVE='levelsm.vpp',RF_REPLAY_DEV_ROOM='1',RF_REPLAY_AUTHORED_SOURCE=str(args.source),RF_REPLAY_PLAYER_CHECKPOINT='1')
...
first=pitch_for(eye,[target_x,1.75,0]);commands,_=pitch_commands(0,first)
...
resume=bytearray(header+bytes(301*48));target=[target_x,1.75,2]
for offset,start,end in [(12,first,pitch_for(eye,target)),(16,yaw,math.atan2(target[0]-eye[0],target[2]-eye[2]))]:
...
report=dict(source=args.source,body=body,first_save_bytes=len(save),continued_save_bytes=len(resumed_bytes),continuation_equal=True,
