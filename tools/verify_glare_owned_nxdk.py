...
entry=sym('rf_glare_owned_open');close=sym('rf_glare_owned_close');C=D;SERV=D+0x800;CB=B+0x80000;WRONG=F+0x800
trace=[];mode=0
...
def service(cpu,address,size,context):
 ...
 if address==CB:
  trace.append('radius');assert arg(1)==0x3f000000 and arg(2)==0x3f800000
  if mode==1:status=0xffffffff
  else:x.mem_write(arg(3),w(0x7fc00000 if mode==4 else 0x3f400000))
 else:
  trace.append('pose');assert arg(1)==123 and arg(2)==7
  if mode==2:status=0xffffffff
  else:x.mem_write(arg(3),pose)
...
def create(slot,index=0,budget=528,flag=0):
 x.mem_write(slot,w(0));return call(entry,C,1,index,123,7,flag,R,L,F,UID,7,17,M,budget,SERV,slot)
for i in range(3):
 trace.clear();slot=O+i*4;assert create(slot,flag=i)==0;owner=r(slot);owners.append(owner);handles.append(r(owner+112))
 assert trace==['pose'] and r(F+8)==i+1 and r(L+8)==i+1 and r(R+12292)==1023-i
 assert r(owner)==123 and r(owner+4)==7 and r(owner+40)==0x5c9e98 and r(owner+48)==(2 if i else 0)
...
for mode,fail,budget,index in [(2,False,528,0),(0,True,528,0),(0,False,527,0),(0,False,528,-1),(0,False,528,1)]:
 ...
 assert trace==([] if index else ['pose'])
...
x.mem_write(R+12292,w(0));trace.clear();assert create(O)==0 and r(O)==0 and trace==['pose'] and not live;checks+=1
report=dict(result='PASS',checks=checks,scope='Actual compiled NXDK full owned factory/constructor/base/registry/list/physics code; only pose and heap supplied. Three simultaneous owners; state, ordered insertion, wrong-family close, pose/heap/short-budget failures, invalid classes, empty registry, middle/head/tail retirement, repeat close and no leaks. Component original equivalence checked separately; no native XEMU or campaign class binding.')
(root/'artifacts/glare-owned-nxdk.json').write_text(json.dumps(report,indent=2));print(report)
