...
entry=int(re.search(r'_rf_physics_static_land\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
for command,wanted in zip(commands,expected):
    ...
    assert bytes(x.mem_read(base,308))==wanted,'NXDK landing differs from original'
report=dict(status='PASS',cases=len(frames),nxdk_cases=len(frames),original_idle_cases=len(frames),frames=frames,scope='4a0b31..4a0bfa static support position/bounds; 419901..return ordinary run landing with actual callees and prepared descriptor. Original 49f646..49f8aa zero-input idle also checked. No damage/sound/AI state, zero support/contact velocity. Synthetic nonzero X/Z input retained in landing comparison.')
