...
for i in range(1024):
 ...
 o.mem_write(STACK,bytes(4096));o.mem_write(STACK+0x2f0,rawpoly);o.mem_write(STACK+0x40,w(nr,nr,B+0x1800));o.mem_write(STACK+0x70,w(amount));o.mem_write(OWNER,bytes(124));o.mem_write(OWNER+0x18,w(width,height));o.mem_write(OWNER+0x2c,f(*threshold));o.mem_write(B+0xf00c,w(OWNER));o.mem_write(B+0xf018,w(OUT));o.mem_write(OUT,initial)
 ...
 x.mem_write(B,data);x.mem_write(B+0xb000,bytes(512))
 ...
 inputs.append(data);responses.append(w(0,accepted)+expected);accepted_total+=accepted;changed+=sum(a!=b for a,b in zip(initial,expected))
assert subprocess.check_output([str(root/'build/pc/Release/rf_effect_probe.exe'),'--lightmap-shadow-filter'],input=b''.join(inputs))==b''.join(responses)
...
assert subprocess.check_output([str(root/'build/pc/Release/rf_effect_probe.exe'),'--lightmap-shadow-filter'],input=bytes(bad))==w(status,0xa5a5a5a5)+initial
report=dict(result='PASS',original_pc_nxdk_passes=len(inputs),accepted=accepted_total,rejected=len(inputs)-accepted_total,changed_mask_bytes=changed,pc_nxdk_empty_receiver_guards=1,original_sha256=sha,x87_control_word='0x027f',scope='Original4f53bd..4f5515 with actual receiver-list lookup,549f10,4f25a0,4f2100; only observation hook at raster entry. One to four receiver polygons, thresholds, full masks and one-fill/early-exit behavior exact. Original aliased subject/output suppresses final clipped-vertex copy; retained subject/tail area behavior reproduced with zero-initialized scratch per pass. Empty receiver original path references uninitialized raster scratch and is rejected by new API. Occluder selection/ownership and native rendering remain excluded.')
(root/'artifacts/lightmap-shadow-filter.json').write_text(json.dumps(report,indent=2));print(report)
