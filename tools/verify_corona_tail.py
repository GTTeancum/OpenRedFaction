...
original=[0x411e00,0x50cf80,0x50d060,0x515b40,0x515bd0];callbacks=[CB+i*16 for i in range(5)];traces={u:[],x:[]};case=None
...
def hook(cpu,address,size,context):
 ...
 if op==0:payload=[arg(i) for i in range(6)] if cpu is u else [r(cpu,arg(1)+i*4) for i in range(6)]
 elif op==1:payload=[arg(shift+i) for i in range(4)]
 elif op==2:payload=[arg(shift+i) for i in range(2)]
 elif op==3:
 ...
 traces[cpu].append([op+1,*payload,*([0]*(7-len(payload)))])
 result=-1 if cpu is x and case[10]==len(traces[cpu]) else 0
 cpu.reg_write(UC_X86_REG_EAX,result&0xffffffff);cpu.reg_write(UC_X86_REG_EIP,r(cpu,sp));cpu.reg_write(UC_X86_REG_ESP,sp+4+(24 if cpu is u and op==0 else 0))
...
base=len(cases)
for oriented,error in itertools.product((0,1),range(1,5)):cases.append([f(.7),f(3),f(.2),f(50),f(-1),f(1),97,1,1,oriented,error])
...
for n,case in enumerate(cases):
 ...
 x.mem_write(B,bytes(528));x.mem_write(B+144,w(radius));x.mem_write(B+24,samples);x.mem_write(B+76,bytes([oriented]));x.mem_write(TAIL,w(intensity,size,angular,distance,side,bitmap,view,draw));x.mem_write(BE,w(*callbacks,77));x.mem_write(S,w(STOP,B,TAIL,BE))
...
report={'result':'PASS','original_cases':base,'callback_error_cases':len(cases)-base,'original_sha256':sha,'scope':'Original414cef..414dfa actual ftol/max/dot arithmetic with graphics callbacks supplied versus PC/NXDK. Exact alpha, sample/radius publication, angle sign, state/texture sequence and billboard/oriented arguments. Upstream side-dot supplied to shared tail; original computes equivalent axial dot. No native drawing claim.'}
