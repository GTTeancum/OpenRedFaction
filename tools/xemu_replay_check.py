...
if args.goto_frame is not None and (args.goto_uid is None or not 0<args.goto_frame<0xffffffff):p.error('--goto-frame requires --goto-uid and a positive frame')
...
if args.goto_frame is not None:replay_env['RF_REPLAY_GOTO_FRAME']=str(args.goto_frame)
...
try:
 ...
 else:goto_file.write_bytes(args.goto_uid.to_bytes(4,'little')+(args.goto_frame.to_bytes(4,'little') if args.goto_frame is not None else b''))
 ...
 with (run/'stdout.log').open('wb') as out,(run/'stderr.log').open('wb') as err:
  ...
  while time.monotonic()<deadline:
   ...
   if d[2]==5:
    ...
    report['startup_inventory']=words(monitor,symbol('rf_scene_startup_inventory'),4)
    assert report['startup_inventory']==expected('STARTUP_INVENTORY')
