...
Type-specific submodel destructors and auxiliary element destructors are supplied
boundaries; scalar deleting wrappers, reverse array traversal and model pool
...
def code(cpu,address,size,data):
 ...
 if address==0x57360e:
  ...
  else:assert token==array-4;cpu.mem_write(token,b'\xdd'*(4+200*count))
 ...
 for present in (0,1):
  for attached in (0,1):
   for count in range(6):
    ...
    old_head=0 if cases%2 else b+0x8000;free_count=900+cases%50;live_count=1000-free_count
    ...
    if attached:expected.extend(('auxiliary',i) for i in reversed(range(count)));expected.append(('free',array-4));elements+=count
