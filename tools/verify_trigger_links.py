...
def original_effect(m,a,size,context):
 sp=m.reg_read(UC_X86_REG_ESP);ret,target,source_arg,actor_arg=struct.unpack('<4I',m.mem_read(sp,16))
 if a==0x4c0200:
  target=struct.unpack('<I',m.mem_read(target+0x2c,4))[0];source_arg=source;actor_arg=actor
 original_trace.append([5 if a==0x4c0200 else 8 if a==0x46aba0 else 6,target,source_arg,actor_arg])
 if a==0x4c0200:return  # Execute the original enable and inspect its flags below.
 m.reg_write(UC_X86_REG_EAX,0);m.reg_write(UC_X86_REG_ESP,sp+4);m.reg_write(UC_X86_REG_EIP,ret)
for addr in (0x4c0200,0x46aba0,0x4b6760):u.hook_add(UC_HOOK_CODE,original_effect,begin=addr,end=addr)
...
for kinds in itertools.product((0,5,6,8),repeat=4):
 for suppress in (0,1,256,257):
  for i,kind in enumerate(kinds):
   obj=b+0x2000+i*0x400;u.mem_write(0x7394cc+i*4,w(obj));u.mem_write(obj+0x24,w(kind));u.mem_write(obj+0x2c,w(handles[i]));u.mem_write(obj+0x2b0,w(0x5f))
  ...
  for i,kind in enumerate(kinds):assert struct.unpack('<I',u.mem_read(b+0x2000+i*0x400+0x2b0,4))[0]==(0x4f if kind==5 else 0x5f)
...
report=dict(result='PASS',cases=cases,scope='Original4c0320 SP routing with real handle/array callees; trigger-enable/controller/event effect boundaries recorded. PC/NXDK exact ordered kind/handle/source/actor calls including suppression low-byte behavior. Original trigger enable executes and its flags are checked. Synthetic registry; controller/event actions and live scene dispatch excluded.')
