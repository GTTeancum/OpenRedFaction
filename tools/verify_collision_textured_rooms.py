...
for n in range(5000):
 ...
 wire=b''.join(records)+struct.pack('<6I7fI',*primary,*children,*start,*delta,limit,flags);wire+=struct.pack('<f',radius)
 ...
 u.mem_write(query,bytes(128));u.mem_write(query+0x34,struct.pack('<6ffI',*start,*delta,radius,flags));u.mem_write(out,struct.pack('<If',0,limit)+bytes(32));u.mem_write(0xca06e0,bytes(8));u.mem_write(0xca06b0,struct.pack('<I',15));u.mem_write(0x1754525,b'\x03');u.mem_write(0x1754558,bytes(12));u.mem_write(0x1754488,bytes(12))
...
for offset,value,status in [(160,99,-4),(28,256,-4),(32,99,-4),(0,math.nan,-2),(208,math.nan,-2),(212,0x1000,-3),(216,-1.0,-2),(216,math.nan,-2)]:
 wire=bytearray(cases[0]);struct.pack_into('<f' if isinstance(value,float) else '<I',wire,offset,value);cases.append(bytes(wire));expected.append(struct.pack('<i',status)+bytes([0xa5])*48+tail([]))
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--sweep-rooms-textured'],input=b''.join(cases))
...
match=re.search(r'_rf_collision_sweep_rooms_textured\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match
entry=int(match.group(1),16);trees=base+32768;work=base+36864
backend=base+40000;callback=backend+256;bitmaps=backend+512
x.mem_write(bitmaps,struct.pack('<4I',0,1,2,3));x.hook_add(UC_HOOK_CODE,texture_hook,begin=callback,end=callback)
for n,(wire,want) in enumerate(zip(cases,expected)):
 extra=struct.unpack_from('<6I',wire,220);trace=[]
 ...
 x.mem_write(arrays,wire[160:184]);x.mem_write(query,wire[184:208]);x.mem_write(out,bytes([0xa5])*48)
 limit_bits,flags=struct.unpack_from('<2I',wire,208)
 x.mem_write(stack,struct.pack('<15I',stop,rooms,4,arrays,2,arrays+8,4,flags,query,query+12,struct.unpack_from('<I',wire,216)[0],limit_bits,backend,out,out+44));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
