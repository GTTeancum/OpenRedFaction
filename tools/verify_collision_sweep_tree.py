...
rng=random.Random(0x4deab0);cases=[];expected=[];hits=0;multiple=0;edge_hits=0
for n in range(5000):
 z=[rng.choice([-4,-2,0,2,4]) for _ in range(3)];flags=rng.choice([0x460,0x461]);start=[rng.choice([0,1,2.25,3]),0,8];delta=[rng.choice([0,-1,1]),0,-16];limit=rng.choice([.5,1])
 ...
 wire=b''.join(nodes)+struct.pack('<10fI',*z,*start,*delta,limit,flags);wire+=struct.pack('<4f',*normal_delta,radius);cases.append(wire)
...
for guard in range(5):
 ...
 cases.append(bytes(wire));expected.append(struct.pack('<iI',status,0xa5a5a5a5)+bytes([0xa5])*40)
actual=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--sweep-tree'],input=b''.join(cases))
assert len(actual)==len(expected)*48
for n,want in enumerate(expected):assert actual[n*48:(n+1)*48]==want,(n,actual[n*48:(n+1)*48].hex(),want.hex())
...
match=re.search(r'_rf_collision_sweep_tree\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 ...
 x.mem_write(stack,struct.pack('<15I',stop,base,3,faces,3,flags,query,query+12,query+24,struct.unpack_from('<I',wire,176)[0],limit_bits,base+26000,3,out,out+40));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
...
report=dict(result='PASS',original_cases=original_count,port_guards=5,nxdk_sha256=hashlib.sha256(xbox_path.read_bytes()).hexdigest(),cases=len(cases),hits=hits,multiple_updates=multiple,edge_hits=edge_hits,scope='Complete original 4deab0 with unchanged stack probe, radius-expanded bounds, linked face iteration and face queries. Three-node trees, child order swaps, empty lists, ties, closest/first-hit modes. Separate normal displacement, subthreshold radii and edge contacts. No world room selection or actor response.')
(root/'artifacts/collision-sweep-tree-verification.json').write_text(json.dumps(report,indent=2));print(report)
