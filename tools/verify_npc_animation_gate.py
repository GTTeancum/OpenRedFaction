...
# Process-local replacement distance boundary: fld binary64; ret.
u.mem_write(0x5182f0,b'\xdd\x05'+pack(base+0x7000)+b'\xc3')
...
for present,kind,desc,flag,flags,state,detail,pred,distance in itertools.product(
 ...
 cases.append(struct.pack('<5I2iId',present,kind,desc,flag,flags,state,detail,pred,distance))
...
for k,raw in enumerate(cases):
 present,kind,desc,flag,flags,state,detail,predicate,distance=struct.unpack('<5I2iId',raw)
 ...
 u.mem_write(base+0x294,pack(base+0x3000));u.mem_write(base+0x43b8,struct.pack('<i',detail));u.mem_write(base+0x7000,raw[-8:])
 ...
 expected=struct.pack('<I',advanced);assert pc[k*4:k*4+4]==expected,('PC',k,struct.unpack('<5I2iId',raw),advanced)
 ...
 count+=advanced
report=dict(result='PASS',cases=len(cases),advanced=count,original_sha256=digest,scope='Original41dbea..41dd49 with actual model-kind and descriptor getters; supplied5182f0 binary64 metric and427020 predicate;503360 advance dispatch intercepted. PC and NXDK decisions match. Excludes alternate-view distance calculation, earlier actor entry gates and live playback integration.')
