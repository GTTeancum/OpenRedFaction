"""Prepare non-DEV remote-charge acquisition, throw and detonator use.
...
PROFILES={'remote':('Remote Charges','remote charge',8)}
...
def prepare(folder,cycles):
    ...
    for kind,(item,weapon,slot) in PROFILES.items():
        ...
        event=U(910530)+S(b'Give_Item_To_Player')+F(*spawn)+S(b'remote_supply')
        ...
        for phase in ('acquired','planted','detonated'):
            base=folder/kind/phase
            frames=120 if phase=='acquired' else 280 if phase=='planted' else 360
            rows=[struct.pack('<5f7I',0,0,0,0,0,0,0,0,int((phase!='acquired' and f==120) or (phase=='detonated' and f==300)),0,int(f in range(30,30+4*cycles,4) or (phase=='detonated' and f==280)),0) for f in range(frames)]
            base.with_suffix('.bin').write_bytes(b'RFI6'+U(48)+b''.join(rows))
            jobs.append(dict(kind=kind,phase=phase,item=item,quantity=quantity,slot=slot,frames=frames,env=dict(RF_REPLAY_LEVEL='ctf06.rfl',RF_REPLAY_ARCHIVE='levelsm.vpp',RF_REPLAY_SETUP_UID='910530'),command=[str(ROOT/'build/pc/Release/rf_pc_play.exe'),'--spawn-replay',str(game),str(base.with_suffix('.bin')),str(base.with_suffix('.ppm'))]))
    report=dict(status='PREPARED_NOT_RUN',jobs=jobs,spawn=spawn,cycles=cycles,schedule='Give_Item0; ordinary cycle edges from30 every4frames; primary throw120, release141 after21ticks, cooldown45 ends186; planted endpoint279; cycle280 toslot9 and detonate300; finish359.',scope='Non-DEV real Remote Charges grant, finite supply, charge launch/attachment and owner-matched detonator activation; preserve original spawn/pickups/fullgeometry506props.',limitations='PC --spawn-replay calls set_campaign_spawn using original player section because DEV flag absent. Assumes only default handgun and granted target owned: onecycle selects8 then nextcycle selects9. Nearby original pickups could alter cycling or supply; --cycles allows explicit adjustment after live evidence. Existing item metadata can demand additional views without granting ownership. Primary release21ticks and postreleasecooldown45 verified in scene_remote_gameplay. Static attachment by279 is an explicit live expectation, not a preparation result. Resources and actual rendering require parent validation.')
...
def main():
    ...
    folder=ROOT/'artifacts/remote-pickup-live';folder.mkdir(parents=True,exist_ok=True);report=prepare(folder,args.cycles)
    ...
    for job in report['jobs']:
        ...
        text=base.with_suffix('.log').read_text(errors='replace');grant=words(text,'SCRIPT_GRANTS');selection=words(text,'WEAPON_SELECTION');ammo=words(text,'PLAYER_AMMO');combat=words(text,'COMBAT');remote=words(text,'REMOTE')
        assert grant[0]==1 and grant[7]==0 and selection[0]==(9 if phase=='detonated' else job['slot']) and ammo[7]==0,(grant,selection,ammo)
        assert remote[5]==0 and remote[6]==0 and remote[7]==0,remote
        if phase=='acquired':
            assert ammo[1]==3 and ammo[2]==3 and remote[:5]==[0,0,0,0,0],(ammo,remote)
            states[kind]={'acquired':dict(ammo=ammo,remote=remote)}
        else:
            baseline=states[kind]['acquired']['ammo']
            assert ammo[0]==baseline[0] and ammo[1]==baseline[1]-1 and ammo[2]==baseline[2]-1,(ammo,remote)
            assert remote[0:2]==[1,1],remote
            if phase=='planted':assert remote[2]==1 and remote[3:5]==[0,1],remote
            else:assert remote[2]==1 and remote[3:5]==[1,0],remote
            states[kind][phase]=dict(ammo=ammo,remote=remote,combat=combat)
