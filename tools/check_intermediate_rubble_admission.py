"""Player intermediate-fragment contact and checkpoint continuation; no host input."""
import json,os,struct,subprocess
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'artifacts/geomod-postedit-re/intermediate-rubble-admission';folder.mkdir(parents=True,exist_ok=True)
...
for name,recording in recordings.items():
 ...
 if name=='continued':assert contacts[1]==0,(name,contacts) # Already beyond chunk; idle after reload.
 else:assert contacts[1]>0 and contacts[2]==0xffffffff,(name,contacts) # Sphere route, not polygon.
...
report['scope']='Naturally extracted intermediate-radius post chunk produces player contacts with exact save continuation; native and visual acceptance remain separate.'
