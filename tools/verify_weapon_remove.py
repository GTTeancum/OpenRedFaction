...
report=dict(result='PASS',cases=len(commands),notifications=notifications,original_sha256=digest,nxdk_sha256=hashlib.sha256((root/'build/xbox/main.exe').read_bytes()).hexdigest(),scope='Full4031a0 with supplied player-to-inventory resolution and notification. Exact PC/NXDK owned byte, unchanged ammo, callback sequence and mutable player count/special index/notified handle; negative/out-of-range indices and absent players included. Notification implementation4a70e0 remains external.')
