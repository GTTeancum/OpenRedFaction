...
for case in range(256):
    ...
    end=0x49e9e6;entry=0x49e8b7
    if repeat:
        end=base+0xf000;entry=0x49e750
        u.mem_write(base+0x1a8,struct.pack('<I',0x1000000));u.mem_write(stack,struct.pack('<II',end,base))
    u.reg_write(UC_X86_REG_FPCW,0x37f);u.emu_start(entry,end,count=10000)
...
actual=subprocess.check_output([str(root/'build/pc/Release/rf_physics_probe.exe'),'--fall-repeat' if repeat else '--fall'],input=b''.join(commands))
...
for case,command in enumerate(commands):
    ...
    state[272:276]=struct.pack('<I',0x1000000 if repeat else 0)
...
report['repeat_pass']=repeat
(root/('artifacts/physics-fall-repeat-verification.json' if repeat else 'artifacts/physics-fall-verification.json')).write_text(json.dumps(report,indent=2));print(report)
