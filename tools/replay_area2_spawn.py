...
source.write_bytes(b'RFI6'+struct.pack('<I',48)+b''.join(
    struct.pack('<5f7I',move(i)[0],0,move(i)[1],0,0,0,int(i==1260),1,0,0,0,0) for i in range(2700)))
...
assert 'Completed 2700 frames' in run.stdout
...
report=dict(status='PASS',traces=traces,attack=attack,door=door,
    scope='Authored L2S2a spawn; movement, one jump and Use only. Reaches and completes the miner rescue encounter. No placement or event injection. Full section exit and combat beyond the rescue remain unverified.')
