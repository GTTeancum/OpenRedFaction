...
for i in range(4096):
 ...
 data=w(kind,0,0,0)+f(*position,*end,*axis,1,1,1,radius,1,.05,angle,1)+f(*lo,*hi)+f(*[rng.uniform(-5,5) for _ in range(3)],*[rng.uniform(-1,1) for _ in range(9)]);inputs.append(data);x.mem_write(B,data);x.mem_write(OUT,b'\xa5'*156)
 assert call('rf_visibility_light_volume_view',[B,B+108,B+120,OUT])==0;assert call('rf_visibility_light_bounds',[OUT,B+84,B+96,OUT+152])==0
 ...
 o.mem_write(0x1818a28,data[108:120]);o.mem_write(0x1818a38,data[120:156]);o.mem_write(0x1818b84,w(1))
 o.mem_write(STACK,w(STOP,P));o.reg_write(UC_X86_REG_ESP,STACK);o.emu_start(0x4d8480,STOP,count=100000);assert o.reg_read(UC_X86_REG_EIP)==STOP
 planes=bytes(120)
 if kind==3:
 ...
 volume=w(kind)+bytes(o.mem_read(P+0x5c,12))+(bytes(o.mem_read(P+0x68,12)) if kind==4 else bytes(12))+bytes(o.mem_read(P+0x3c,4))+planes
...
for offset,value in [(0,w(1)),(64,f(-1)),(16,f(float('nan')))]:
 ...
 status=call('rf_visibility_light_volume_view',[B,B+108,B+120,OUT]);assert status and bytes(x.mem_read(OUT,156))==b'\xa5'*156;responses.append(w(status)+b'\xa5'*156)
assert subprocess.check_output([str(root/'build/pc/Release/rf_effect_probe.exe'),'--light-volume-view'],input=b''.join(inputs))==b''.join(responses)
report=dict(result='PASS',original_pc_nxdk_cases=4096,guards=3,hits=hits,scope='Actual source constructors, spotlight width/planes and point/cone/segment box predicates without hooks; all152 prepared volume bytes and bounds decisions match PC/NXDK. Actual4d8480 with resolved view origin/matrix, including nonunit bases. View-stack push/pop and native owner composition excluded.')
(root/'artifacts/light-volume-view.json').write_text(json.dumps(report,indent=2));print(report)
