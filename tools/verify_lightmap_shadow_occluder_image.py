"""Replay shadow occluder selection with original static bitmap format lookup against PC/NXDK."""
...
o=machine(exe);x=machine(root/'build/xbox/main.exe');mp=(root/'build/xbox/main.map').read_text();entry=int(re.search(r'\s_rf_lightmap_shadow_occluder_image\s+([0-9a-fA-F]+)',mp)[1],16)
...
for i in range(4096):
 ...
 view=f(*lightmin,*lightmax,*mapmin,*mapmax,*mapplane,*[q for p in planes for q in p])+w(mapping);face=f(*faceplane,*minimum,*maximum)+w(flags,facemap&0xffffffff,portal&0xffffffff,excluded);present=int(i%5!=0);fmt=(i//16)%12;data=view+face+w(present,fmt)
 o.mem_write(OWNER,bytes(124));o.mem_write(OWNER,w(mapping));o.mem_write(OWNER+0x6c,f(*mapplane));o.mem_write(B+0x1000,bytes(80));o.mem_write(B+0x1000,f(*faceplane,*minimum,*maximum)+w(flags,0,0)+struct.pack('<hh',portal,facemap));o.mem_write(STACK,bytes(4096));o.mem_write(STACK+0x18,w(B+0x1000));o.mem_write(STACK+0x4c,f(*lightmin,*lightmax));o.mem_write(STACK+0xd4,f(*mapmin));o.mem_write(STACK+0xf8,f(*mapmax));o.mem_write(STACK+0x1a0,f(*[q for p in planes for q in p]));o.mem_write(B+0xf00c,w(OWNER));o.mem_write(B+0x1030,w(0 if present else 0xffffffff));o.mem_write(0x5a4554,w(32));o.mem_write(0x17c80c4,w(B+0x2000));o.mem_write(B+0x2000,bytes(108));o.mem_write(B+0x203c,w(fmt))
 ...
 x.mem_write(B,data);x.mem_write(B+0x3000,w(0,0,0,fmt,0));x.mem_write(OUT,w(0xa5a5a5a5));assert call(x,entry,[B,B+164,B+0x3000 if present else 0,OUT])==0;got=struct.unpack('<I',x.mem_read(OUT,4))[0];assert got==expected,(i,mode,got,expected)
 inputs.append(data);responses.append(w(0,expected));accepted_total+=expected
assert subprocess.check_output([str(root/'build/pc/Release/rf_effect_probe.exe'),'--lightmap-shadow-occluder-image'],input=b''.join(inputs))==b''.join(responses)
for at,value in [(0,f(float('nan'))),(64,f(float('inf'))),(164,f(float('nan'))),(164+44,w(32768)),(164+48,w(32768))]:
 bad=bytearray(data);bad[at:at+4]=value;x.mem_write(B,bytes(bad));x.mem_write(OUT,w(0xa5a5a5a5));status=call(x,entry,[B,B+164,B+0x3000 if present else 0,OUT]);assert status!=0 and bytes(x.mem_read(OUT,4))==w(0xa5a5a5a5)
 assert subprocess.check_output([str(root/'build/pc/Release/rf_effect_probe.exe'),'--lightmap-shadow-occluder-image'],input=bytes(bad))==w(status,0xa5a5a5a5)
report=dict(result='PASS',original_pc_nxdk_faces=len(inputs),accepted=accepted_total,rejected=len(inputs)-accepted_total,pc_nxdk_guards=5,original_sha256=sha,x87_control_word='0x027f',scope='Original4f4f15..4f5031 with actual bounds overlap, signed metadata, coplanar test, support corners and six plane distances. Actual510710/5106f0/50f440/50eee0 static bitmap lookup; metadata-only descriptors supplied, with missing bitmap and twelve formats. No texture-classification hook. Sixteen case groups include equality thresholds, signed IDs and unrelated flags. Selected retained image metadata consumed without pixel access. Loading/animated frame selection, face decoding and native rendering excluded.')
(root/'artifacts/lightmap-shadow-occluder-image.json').write_text(json.dumps(report,indent=2));print(report)
