...
for case in range(384):
    ...
    values=f(rng.choice([1/60,1/30,.001]),rng.choice([3,6]),20,rng.choice([.2,1,2]),rng.uniform(-1,1),0,rng.uniform(-1,1),*n,*[rng.uniform(-.5,.5) for _ in range(3)])
    u.mem_write(base,bytes(0x6000));u.mem_write(stack,bytes(256));u.mem_write(base+0x858,w(base+0x2000));u.mem_write(base+0x2000,w(1,1,2,0,2,1,3,0));u.mem_write(base+0xfc,f(1,0,0,0,1,0,0,0,1));u.mem_write(base+0x294,w(base+0x3000))
...
actual=subprocess.check_output([str(root/'build/pc/Release/rf_physics_probe.exe'),'--run'],input=b''.join(commands))
...
entry=int(re.search(r'_rf_physics_run_propose\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16);stop=base+0xf000
for i,command in enumerate(commands):
    x.mem_write(base,command[:308]);x.mem_write(base+0x3000,command[308:]);args=struct.unpack('<4I',command[308:324])
    x.mem_write(stack,w(stop,base,*args,base+0x3010,base+0x301c,base+0x3028));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x27f)
...
report=dict(status='PASS',pc_cases=len(expected),nxdk_cases=len(expected),scope='Original 49f646 selects 49e400 for descriptor 1 on every case; complete callees with prepared identity body/input and surface traction. Slopes, force, support and repeated passes included.')
(root/'artifacts/run-motion-verification.json').write_text(json.dumps(report,indent=2));print(report)
