...
pack=lambda model=b'',emitter=b'',life=-1,temp=0:struct.pack('<64s64sff',model,emitter,life,temp)
...
for i,m in enumerate(names):
 ...
 temperature=re.search(rb'\$Body Temperature\(F\):\s*([^\s]+)',body,re.I)
 value=pack(model[1] if model else b'',emitter[1] if emitter else b'',float(lifetime[1]) if lifetime else -1,float(temperature[1]) if temperature else 0)
...
for body,want in [
 (b'',pack()),
 (b'$Body Temperature(F): 90',pack(temp=90)),
 (b'$Body Temperature(F): -10 $Corpse V3D Filename: \"a\"',pack(b'a',temp=-10)),
 (b'$Body Temperature(F): nan',None),
 (b'$Body Temperature(F): 1 $Body Temperature(F): 2',None),
...
for body,name,want in cases:
 ...
 assert len(pc)==140
 x.mem_write(b,body);x.mem_write(b+0x10000,name+b'\0');x.mem_write(b+0x11000,b'\xa5'*136)
 ...
 assert w(x.reg_read(UC_X86_REG_EAX))+bytes(x.mem_read(b+0x11000,136))==pc,(name,'NXDK mismatch')
 if want is None:assert status!=0 and pc[4:]==b'\xa5'*136,(name,body)
...
for level in ('L1S1.rfl','L1S2.rfl','L1S3.rfl'):
 ...
 for _,name,model,emitter,life,temp in rows:assert pack(model.encode(),emitter.encode(),float(life),float(temp))==expected[name],(level,name)
 retained.append(dict(level=level,classes=len(rows),added_bytes=136*len(rows)))
report=dict(result='PASS',classes=len(names),cases=len(cases),replacements=replacements,retained_levels=retained,nxdk_sha256=hashlib.sha256(binary.read_bytes()).hexdigest(),scope='PC/NXDK metadata parsing agrees with independent installed-table inventory. Defaults and tags traced statically to41bb32..41bbb9 and body temperature41d4bc..41d4db; original parser not executed. PC class values survive archive closure and exact/short-budget checks. No live corpse model loading or emitter creation claimed.')
