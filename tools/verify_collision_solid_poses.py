...
import math,hashlib,json,struct,sys,random,re,subprocess
...
for n in range(4000):fixtures.append(('random '+str(n),[rng.choice([None,-4,-2,0,2,4,6]) for _ in range(3)],rng.randrange(2),None,None,None))
cases=[];expected=[]
results=[]
for case,(name,heights,first,winner,fraction,height) in enumerate(fixtures):
 poses=[];want_result=int(case<analytic_count or case%3!=0)
 for mover in range(2):
  z=heights[mover] or 0
  pose=struct.pack('<6f',-3,-3,z-.0001,3,3,z+.0001)+bytes(12)+identity+bytes(12)+identity
  if case>=analytic_count:
   def matrix():
    yaw=rng.uniform(-.6,.6);pitch=rng.uniform(-.6,.6);c,t,sn,tn=math.cos(yaw),math.cos(pitch),math.sin(yaw),math.sin(pitch)
    return [c*t,-sn,c*tn,sn*t,c,sn*tn,-tn,0,t]
   im=matrix();om=matrix();io=[rng.uniform(-2,2) for _ in range(3)];oo=[rng.uniform(-5,5) for _ in range(3)]
   if case%29==0 and not want_result:om[0]=math.nan
   pose=struct.pack('<30f',-20,-20,-20,20,20,20,*io,*im,*oo,*om)
  poses.append(pose)
 ...
 for i,z in enumerate(heights):
  ...
  if i<2:
   pose=poses[i];u.mem_write(block+0x190,pose[:24]);u.mem_write(block+0x3c,pose[24:72]);u.mem_write(block+0xe4,pose[72:84]);u.mem_write(block+0xfc,pose[84:120])
 ...
 u.mem_write(stack,struct.pack('<5I',stop,inputs,inputs+12,0x26|first,out if want_result else 0));u.reg_write(UC_X86_REG_ESP,stack);u.reg_write(UC_X86_REG_FPCW,0x37f)
 ...
 cases.append(struct.pack('<3f4I6f',*[z if z is not None else 0 for z in heights],*[int(z is not None) for z in heights],0x26|first,*segment)+b''.join(poses)+struct.pack('<I',want_result))
 if hit and want_result:
 ...
 else:expected.append(struct.pack('<iI',0,hit)+bytes([0xa5])*44)
 results.append(dict(name=name,hit=hit,output=raw.hex(),queries=list(trace)))
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--solid-ray-posed'],input=b''.join(cases));assert len(raw)==len(cases)*52
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 ...
 for i,z in enumerate(heights):
  ...
  if i<2:x.mem_write(solid+24,wire[52+i*120:172+i*120])
 ...
 x.mem_write(stack,struct.pack('<9I',stop,base,2,base+296,inputs,inputs+12,flags,out if struct.unpack_from('<I',wire,292)[0] else 0,out+44));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
...
visibility_hits=sum(not struct.unpack_from('<I',w,292)[0] and struct.unpack_from('<I',v,4)[0]!=0 for w,v in zip(cases,expected))
unused_nan_pose_hits=sum(not struct.unpack_from('<I',w,292)[0] and math.isnan(struct.unpack_from('<f',w,136)[0]) and struct.unpack_from('<I',v,4)[0]!=0 for w,v in zip(cases,expected))
assert visibility_hits>0 and unused_nan_pose_hits>0
report=dict(result='PASS',visibility_hits=visibility_hits,unused_nan_pose_hits=unused_nan_pose_hits,cases=len(cases),hits=sum(struct.unpack_from('<I',v,4)[0] for v in expected),visibility_only=sum(struct.unpack_from('<I',w,292)[0]==0 for w in cases),nxdk_sha256=hashlib.sha256((root/'build/xbox/main.exe').read_bytes()).hexdigest(),scope='Complete original 498e80 and all callees unchanged, two movers plus static hierarchy, independently translated/rotated input and output poses, optional output and unused NaN output matrices, null material. Exact geometric/object/index result on PC and NXDK. Original output fraction reuse and first-hit order retained; no live moving-solid extraction or actor response.')
(root/'artifacts/collision-solid-poses-verification.json').write_text(json.dumps(report,indent=2));print(report)
