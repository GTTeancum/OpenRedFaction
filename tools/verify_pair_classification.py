...
hooks={0x4290d0:16,0x427020:17,0x48c7f0:18,0x48c8e0:19};calls={hex(a):0 for a in [*hooks,0x40a0e0,0x48a840]}
views=[]
...
for case in range(32768):
 ...
 for i in range(2):
  ...
  view += [kind,10+i,rng.choice((100+i,11-i)),rng.randrange(2),rng.choice((0,0x20)),rng.choice((4,7,9)),fword(rng.choice((0,1,1.5,2,4))),rng.choice((0,2))]
  view += [rng.choice((0,0,1,256,257)) for _ in range(4)]
 ...
 for i,v in enumerate(views):
  ...
  for offset,index in [(0x24,8),(0x2c,9),(0x30,10),(0x78,14),(0x7c,0),(0x180,6),(0x1a8,1),(0x298,13),(0x2b0,15)]:u.mem_write(actor+offset,w(v[index]))
...
report=dict(result='PASS',cases=len(commands),outcomes=coverage,changed_flags_on_rejection=changed_rejections,predicate_calls=calls,original_sha256=sha,scope='Full48be00 branch routing and flag writes, all kind0..7 combinations, null/self and arbitrary initial flags; exact PC/NXDK. Actual actor/player/use/weapon/string helpers. External disabled/item-mode/projectile/trigger predicates and owner lookup/player predicate supplied as explicit boundaries. Their implementations, callback mutation and live scene binding are not verified by this gate.')
