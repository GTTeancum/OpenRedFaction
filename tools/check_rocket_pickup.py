"""Prepare non-DEV rocket-launcher event acquisition and ordinary firing.
...
PROFILES={'rocket':('rocket launcher','Rocket Launcher',4)}
...
def prepare(folder,cycles):
    ...
    for kind,(item,weapon,slot) in PROFILES.items():
        ...
        event=U(910510)+S(b'Give_Item_To_Player')+F(*spawn)+S(b'rocket_supply')
        ...
        for phase in ('acquired','fired'):
            ...
            rows=[struct.pack('<5f7I',0,0,0,0,0,0,0,0,int(phase=='fired' and f==120),0,int(f in range(30,30+4*cycles,4)),0) for f in range(180)]
            base.with_suffix('.bin').write_bytes(b'RFI6'+U(48)+b''.join(rows))
            jobs.append(dict(kind=kind,phase=phase,item=item,quantity=quantity,slot=slot,env=dict(RF_REPLAY_LEVEL='ctf06.rfl',RF_REPLAY_ARCHIVE='levelsm.vpp',RF_REPLAY_SETUP_UID='910510'),command=[str(ROOT/'build/pc/Release/rf_pc_play.exe'),'--spawn-replay',str(game),str(base.with_suffix('.bin')),str(base.with_suffix('.ppm'))]))
    report=dict(status='PREPARED_NOT_RUN',jobs=jobs,spawn=spawn,cycles=cycles,schedule='Give_Item0; ordinary cycle edges from30 every4frames; one primary request120; finish179.',scope='Non-DEV authored rocket grant and finite-ammo launch; preserve original spawn/pickups/fullgeometry506props. Impacts and terrain edits are reported only when observed, not prerequisites or implied by launch.',limitations='PC --spawn-replay calls set_campaign_spawn using original player section because DEV flag absent. Assumes only default handgun and granted target owned: onecycle selects4. Nearby original pickups could alter cycling or supply; --cycles allows explicit adjustment after live evidence. Existing item metadata can demand additional views without granting ownership. Resources and actual rendering require parent validation.')
...
def main():
    ...
    folder=ROOT/'artifacts/rocket-pickup-live';folder.mkdir(parents=True,exist_ok=True);report=prepare(folder,args.cycles)
    ...
    for job in report['jobs']:
        ...
        text=base.with_suffix('.log').read_text(errors='replace');grant=words(text,'SCRIPT_GRANTS');selection=words(text,'WEAPON_SELECTION');ammo=words(text,'PLAYER_AMMO');combat=words(text,'COMBAT');rockets=words(text,'ROCKETS')
        assert grant[0]==1 and grant[7]==0 and selection[0]==job['slot'] and ammo[7]==0,(grant,selection,ammo)
        assert rockets[6:8]==[0,0],rockets
        if phase=='acquired':
            assert ammo[2]>0 and combat[0]==0 and rockets[0]==0,(ammo,combat,rockets)
            states[kind]={'acquired':dict(ammo=ammo,combat=combat,rockets=rockets)}
        else:
            baseline=states[kind]['acquired']['ammo'];assert ammo[0]==baseline[0] and ammo[1]==baseline[1] and ammo[2]==baseline[2]-1 and combat[0]==1,(ammo,combat)
            assert rockets[0]==1,rockets
            states[kind]['fired']=dict(ammo=ammo,combat=combat,rockets=rockets,impact_observed=rockets[1]>0,terrain_edits_observed=rockets[4])
