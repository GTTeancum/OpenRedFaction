...
DATA=B+0x100000;MATERIALS=B+0x20000;SLOTS=B+0x21000;OFFSETS=SLOTS+0x1000;MAPPING=OFFSETS+0x100;results=[];total=uv_misses=bounds=guards=mapping_guards=missing_faces=0
levels=json.loads((root/'artifacts/geometry.json').read_text())[:1]
for level in levels:
 raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--level-material-samples',str(root/'Installed_Game'/level['archive']),level['file'],*[str(root/'Installed_Game'/name) for name in ('maps_en.vpp','maps1.vpp','maps2.vpp','maps3.vpp','maps4.vpp')]]);at=0;counted=0
 ...
 while at<len(raw):
  ...
  got=w(run(x,entry,(MAPPING,0,B,0,POINT,WORK,OUT)))+bytes(x.mem_read(OUT,4));assert got==want[16:],(level['file'],index,'NXDK',got.hex(),want[16:].hex());assert bytes(x.mem_read(VERT,len(data)))==data
  if index==0:
...
report=dict(result='PASS',faces=total,images=image_count,loaded_images=sum(im[4]==0 for im in images),missing_images=sum(im[4]!=0 for im in images),missing_faces=missing_faces,uv_misses=uv_misses,bounds_rejections=bounds,nxdk_workspace_guards=guards,nxdk_mapping_guards=mapping_guards,levels=results,original_sha256=sha,scope='Opening level authored geometry and actual retained material textures. Full original4e1ad0 and55cfa0 with only lock/release supplied, source channel precision restored from normalized decoded images. PC material mapping adapter and compiled NXDK complete adapter with distinct swizzled image allocations match. Static current images only; no live collision traversal or animation-selection claim.')
(root/'artifacts/geometry-material-sample.json').write_text(json.dumps(report,indent=2));print(report)
