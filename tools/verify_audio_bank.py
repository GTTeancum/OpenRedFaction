...
report=dict(result='PASS',retained_bytes=93434,post_close_pcm_hash=hash_,scope='PC VPP bank: metadata-only declarations with deferred PCM, fixed-budget registration, duplicate precedence, selective loading, exact/one-byte-short budgets, case-insensitive deduplication with first-registration spatial parameters, missing file and output preservation, playback data after archive close, invalid sample index, repeat bank close, explicit unload/reload with preserved parameters/index, exact byte reclamation, closed-archive and one-byte-short reload rejection, and restored PCM after archive reopen/close. Idle-release bridge rejects active borrowers without device calls, preserves bank/mixer on device failure, and clears completed borrowers before freeing PCM. Shared pressure retry reclaims an eligible idle sample and completes a previously over-budget load with stable indices. First256 stereo frames match independent Python wave/integer interpolation. NXDK build only for ownership; no live device output or native campaign budget-pressure coverage.')
