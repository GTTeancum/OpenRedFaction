...
assert off.get('item_uid') == on.get('item_uid'), 'Different staged pickup'
assert all(off.get(k)==on.get(k) for k in ('exit_start_uid','exit_uid','return_exit_uid','goal_uid','level','archive')), 'Different exits'
