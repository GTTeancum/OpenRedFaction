...
ops={0x48cc10:0,0x48bb00:1,0x48bb90:2,0x49ab00:3,0x49a420:4,0x49afe0:5,0x49b570:6}
trace=[];rows=[]
def hook(m,address,size,native):
 ...
 if native and address==getview:value=views+((a[2]-objects)//768)*16
 else:
  if native:op,first,second=a[2:5]
  else:op=ops[address];first=a[1];second=0 if op<3 else a[2]
  trace.extend((op,first,second));value=0
  if op<3:
   index=(first-nodes)//16;assert 0<=index<8;r=rows[index];value=r[4+op]
   if op==2 and r[8]:m.mem_write(first+12,w(r[7]))
...
rng=random.Random(0x48ca60);commands=[];answers=[];counts=[0]*7
...
for case in range(2048):
 ...
 rows=[[links[i],rng.randrange(16),rng.randrange(16),rng.getrandbits(6)]+[rng.choice((0,0,1,2,255,256,257)) for _ in range(3)]+[rng.getrandbits(6),rng.randrange(2)] for i in range(8)]
 actors=[[rng.choice((0,2,3,4,5,7)),rng.choice((0,0x40000000,0x40000000)),rng.choice((0,0,1)),rng.choice((1,1,3))] for _ in range(16)]
 ...
 words=heads+sum(rows,[])+sum(actors,[]);assert len(words)==140;commands.append(w(*words))
 ...
 for i,(kind,body,model,mode) in enumerate(actors):
  obj=objects+i*768;u.mem_write(obj+0x24,w(kind));u.mem_write(obj+0x1a8,w(body));u.mem_write(obj+0x80,w(model));u.mem_write(obj+0x858,w(b+0xa000+i*8));u.mem_write(b+0xa004+i*8,w(mode))
 ...
 for i in range(8):
  link,first,second,flags=struct.unpack('<4I',result[i*16:i*16+16]);out[4+i*9]=idx(link);out[7+i*9]=flags
...
report=dict(result='PASS',cases=2048,operation_counts=counts,original_sha256=sha,scope='Full original48ca60 control flow and real list helpers versus PC/NXDK. Expiration, kind5/bounds queries and response functions supplied. Exact traces, ordered arguments, list heads/links/counts, flags and endpoints. Empty/full/permuted lists, consecutive removals, counter wrap, query low bytes and bounds callback flag mutation. Actual collision response effects and live scene scheduling excluded.')
