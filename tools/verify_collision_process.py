...
ops={0x48cc10:0,0x4bfc60:1,0x49ab00:2,0x49a420:3,0x49afe0:4,0x49b570:5}
trace=[];rows=[]
def hook(m,address,size,native):
 ...
 if native and address==getview:value=views+((a[2]-objects)//768)*40
 else:
  if native:op,first,second=a[2:5]
  else:op=ops[address];first=a[1];second=0 if op==0 else a[2]
  if not native and op==1:assert a[3]==0
...
for case in range(2048):
 ...
 rows=[[links[i],rng.randrange(16),rng.randrange(16),rng.getrandbits(6)]+[rng.choice((0,0,1,2,255,256,257)) for _ in range(1)] for i in range(8)]
 actors=[[rng.choice((0,2,3,4,5,7)),rng.choice((0,0x40000000,0x40000000)),rng.choice((0,0,1)),rng.choice((1,1,3)),0x100+i,rng.choice((0xffffffff,0x100+rng.randrange(16))),rng.choice((0,0,0,0x4000)),rng.choice((0,2,2,3)),rng.choice((-1,0,1,2,8))]+[rng.choice((0xffffffff,0x100+rng.randrange(16))) for _ in range(8)] for i in range(16)]
 ...
 words=heads+sum(rows,[])+sum(actors,[]);assert len(words)==316;commands.append(w(*words))
 ...
 for i,fields in enumerate(actors):
  ...
  allowed=b+0xb000+i*32;x.mem_write(views+i*40,w(*fields[:9],allowed));x.mem_write(allowed,w(*fields[9:]));u.mem_write(allowed,w(*fields[9:]))
  obj=objects+i*768;u.mem_write(obj+0x2c4,w(filter_value));u.mem_write(obj+0x2d4,w(allowed_count,8,allowed));u.mem_write(obj+0x24,w(kind));u.mem_write(obj+0x2c,w(handle));u.mem_write(obj+0x200,w(parent));u.mem_write(obj+0x7c,w(object_flags));u.mem_write(obj+0x1a8,w(body));u.mem_write(obj+0x80,w(model));u.mem_write(obj+0x858,w(b+0xa000+i*8));u.mem_write(b+0xa004+i*8,w(mode))
 ...
 for i in range(8):
  link,first,second,flags=struct.unpack('<4I',result[i*16:i*16+16]);out[4+i*5]=idx(link);out[7+i*5]=flags
...
report=dict(result='PASS',cases=2048,operation_counts=counts,original_sha256=sha,scope='Full original48ca60 control flow and real list helpers versus PC/NXDK. Expiration, trigger contact4bfc60 and response effects supplied; actual48bb00 membership routing and48bb90/40a110 filter execute. Exact traces, ordered arguments, list heads/links/counts, flags and endpoints. Empty/full/permuted lists, consecutive removals, counter wrap, query low bytes, parent matches, visibility flags and trigger allowlists. Actual collision response effects and live scene scheduling excluded.')
