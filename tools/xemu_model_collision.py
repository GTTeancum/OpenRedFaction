"""Isolated stock-64-MiB XEMU static model collision tests; no host input or desktop capture."""
...
run=root/'artifacts/xemu'/('model-collision-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir(parents=True)
flag=root/'build/xbox/disc/model-collision-test.bin';saved=flag.read_bytes() if flag.exists() else None
process=monitor=None;report={'result':'FAIL','scope':'Native stock64MiB archive-loaded owned static collision resources versus original-game answers and PC:427 models/6900 queries. No live scene residency or visual claim.'}
...
try:
 flag.write_bytes((root/'artifacts/model-authored-trace.bin').read_bytes());build();mapping=(root/'build/xbox/main.map').read_text()
 address=int(re.search(r'_rf_model_collision_test\s+([0-9a-fA-F]+)',mapping)[1],16)
...
''')
 ...
 with (run/'stdout.log').open('wb') as out,(run/'stderr.log').open('wb') as err:
  ...
  expected=[int(v) for v in subprocess.check_output([str(root/'build/pc/Release/rf_model_file_probe.exe'),'--collision-fixture',str(root/'artifacts/model-authored-trace.bin'),str(root/'Installed_Game')+'/'],text=True).split()]
  report['pc_state']=expected
  assert state==expected and state[2:5]==[427,6900,6242],(state,expected)
  report['result']='PASS'
finally:
 if monitor:
