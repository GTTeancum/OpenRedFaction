...
folder=ROOT/'artifacts/clipped-post-continuation';folder.mkdir(parents=True,exist_ok=True)
...
commands,_=pitch_commands(0,pitch_for(eye,[-4.699,.3,2.5]))
...
inputs={'save':save,'resume':source[:8]+bytes(121*48),'control':save+bytes(120*48)}
...
for name,data in inputs.items():
 ...
 if name!='resume':assert values('AUTHORED_SOURCE_CUTS')[:2]==[94,1]
 ...
 assert (version,size,count)==(2,344,1) and bank+size==len(checkpoint)
 radius=struct.unpack_from('<f',checkpoint,bank+16+256)[0]
 assert abs(radius-.434212536)<1e-6,(name,radius)
 if name!='resume':assert values('GEOMOD')[5:8]==[0,1,1]
 report[name]=dict(bytes=len(checkpoint),radius=radius,geomod=geomod)
assert (folder/'resume.rfcp').read_bytes()==(folder/'control.rfcp').read_bytes(),'partitioned publication continuation differs'
report.update(result='PASS',scope='Actual previously rejected post cut, one retained fragment, exact PC player/destruction save continuation; Xbox and visual acceptance separate')
