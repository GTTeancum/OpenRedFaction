...
import pefile
death='--death' in sys.argv;count=3 if death else 2;sentinels=(123,456,789)[:count]
...
entry=int(re.search(r'\s_rf_entity_'+('damage_sound' if death else 'pain')+r'_groups_read\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
...
folder=root/('artifacts/damage-sound-groups' if death else 'artifacts/pain-groups');folder.mkdir(exist_ok=True);path=folder/'entity.tbl';path.write_bytes(entity)
def original(name):
...
def check(text,name,expected):
 ...
 pc=subprocess.check_output([str(root/'build/pc/Release/rf_entity_assets_probe.exe'),'--damage-sound-groups' if death else '--pain-groups',str(path),name.decode()],input=wire)
...
if death:
 for text in (b'$Name: "fixture"\n$DeathSnd: 3',b'$Name: "fixture"\n$DeathSnd: "a"\n$DeathSnd: "b"'):
  check(text,b'fixture',None)
report=dict(result='PASS',installed_classes=len(rows),foley_groups=len(names),synthetic_cases=7 if death else 5,rows=rows,
