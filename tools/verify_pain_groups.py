...
entry=int(re.search(r'\s_rf_entity_pain_groups_read\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
...
folder=root/'artifacts/pain-groups';folder.mkdir(exist_ok=True);path=folder/'entity.tbl';path.write_bytes(entity)
def original(name):
...
def check(text,name,expected):
 ...
 pc=subprocess.check_output([str(root/'build/pc/Release/rf_entity_assets_probe.exe'),'--pain-groups',str(path),name.decode()],input=wire)
 x.mem_write(b+0x10000,text);x.mem_write(b+0xe000,name+b'\0');x.mem_write(b+0xf000,w(123,456))
 ...
 result=w(x.reg_read(UC_X86_REG_EAX))+bytes(x.mem_read(b+0xf000,8));assert result==pc
 if expected is None:assert pc[:4]!=bytes(4) and pc[4:]==w(123,456)
...
for i,match in enumerate(classes):
 ...
 expected=list(map(original,labels));check(entity,match[1],expected)
 rows.append(dict(name=match[1].decode(),groups=expected))
for text in (b'$Name: "fixture"\n#End',b'$Name: "fixture"\n$Low_Pain Sounds: "unknown"\n$Med_Pain Sounds: ""\n#End'):
 check(text,b'fixture',[0xffffffff,0xffffffff])
for text in (b'$Name: "fixture"\n$Low_Pain Sounds: 3',b'$Name: "fixture"\n$Low_Pain Sounds: "a"\n$Low_Pain Sounds: "b"',b'$Name: "fixture"\n$Med_Pain bad "a"'):
 check(text,b'fixture',None)
report=dict(result='PASS',installed_classes=len(rows),foley_groups=len(names),synthetic_cases=5,rows=rows,
 scope='PC and linked NXDK metadata reader agree with independent installed label extraction and actual original434cb0/57c130 resolution. Full original class parser not executed. Missing/empty/unknown resolve-1; malformed/duplicate guards preserve output. Playback/voice ownership remain separate.')
