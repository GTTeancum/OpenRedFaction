...
import pefile
squash='--squash' in sys.argv;impact='--impact' in sys.argv or squash;death='--death' in sys.argv;count=1 if impact else 3 if death else 2;sentinels=(123,456,789)[:count]
...
entry=int(re.search(r'\s_rf_entity_'+('squash_sound_group_read' if squash else 'impact_sound_group_read' if impact else ('damage_sound' if death else 'pain')+'_groups_read')+r'\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
...
folder=root/('artifacts/squash-sound-group' if squash else 'artifacts/impact-sound-group' if impact else 'artifacts/damage-sound-groups' if death else 'artifacts/pain-groups');folder.mkdir(exist_ok=True);path=folder/'entity.tbl';path.write_bytes(entity)
def original(name):
...
def check(text,name,expected):
 if squash and text!=entity:text=text.replace(b'$Impact Death Sound:',b'$Squash Sounds:').replace(b'$Impact Death bad',b'$Squash bad')
 path.write_bytes(text)
 pc=subprocess.check_output([str(root/'build/pc/Release/rf_entity_assets_probe.exe'),'--squash-sound-group' if squash else '--impact-sound-group' if impact else '--damage-sound-groups' if death else '--pain-groups',str(path),name.decode()],input=wire)
...
for i,match in enumerate(classes):
 ...
 if impact:
  tag=b'Squash Sounds:' if squash else b'Impact Death Sound:'
  found=re.findall(rb'^\s*\$'+tag+rb'\s*"([^"\r\n]*)"',block,re.M|re.I)
  assert len(found)<=1;expected=[original(found[0])] if found else [0xffffffff if squash else 123]
 check(entity,match[1],expected)
 rows.append(dict(name=match[1].decode(),groups=expected))
if impact:
 check(b'$Name: "fixture"\n#End',b'fixture',[0xffffffff if squash else 123])
...
report=dict(result='PASS',installed_classes=len(rows),foley_groups=len(names),synthetic_cases=6 if impact else 7 if death else 5,rows=rows,
 scope='PC and linked NXDK metadata reader agree with independent installed label extraction and actual original434cb0/57c130 resolution. Full original class parser not executed. Missing squash fields reset to-1; missing impact fields preserve input; missing pain/death and explicit empty/unknown resolve-1; malformed/duplicate guards preserve output. Playback/voice ownership remain separate.')
