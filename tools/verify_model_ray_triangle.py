...
entry=int(re.search(r'\s_rf_collision_model_ray_triangle\s+([0-9a-fA-F]+)',mapping)[1],16)
rng=random.Random(0x54dd10);commands=[];answers=[];hits=0;flipped=0
for case in range(8192):
    ...
    command=f(start+delta+plane+[v for row in triangle for v in row])+w(token,two)+initial;assert len(command)==116;commands.append(command)
    ...
    u.mem_write(query,b'\xa5'*104);u.mem_write(query+0x50,f(start+delta));u.mem_write(hit,initial)
    ...
    u.emu_start(0x54dd10,stop,count=10000)
    ...
    elif expected[20:32]!=f(plane[:3]):flipped+=1
    x.mem_write(b,command+b'\xa5'*16);x.mem_write(stack,w(stop,b+24,b,b+12,two,b+84));x.reg_write(UC_X86_REG_ESP,stack)
    ...
    assert bytes(x.mem_read(b,84))==command[:84] and bytes(x.mem_read(b+116,16))==b'\xa5'*16
actual=subprocess.check_output([str(root/'build/pc/Release/rf_physics_probe.exe'),'--model-ray-triangle'],input=b''.join(commands))
assert actual==b''.join(answers),'PC mismatch'
report=dict(result='PASS',cases=len(commands),hits=hits,flipped_hits=flipped,original_sha256=sha,
    scope='Complete54dd10 with all plane, ray, fan containment and vector callees unchanged; no hooks. Exact PC/NXDK return and32-byte hit, one/two-sided behavior, strict limit, batch index selection, signed vertex indices resolved to contiguous vertices, preserved query/geometry and rejected output. Original triangle-record pointers represented by caller tokens. No swept sphere, full part query or XEMU integration.')
(root/'artifacts/model-ray-triangle.json').write_text(json.dumps(report,indent=2));print(report)
