"""Normal restore must calculate speed after standing effects."""
...
entry=int(re.search(r'\s_rf_player_climb_exit\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text())[1],16)
...
cases=[];expected=[]
for walk,crouch,force,blocked,enabled,forced,network in itertools.product((0,1),(0,0x400),(0,),(0,1),(0,1,256,257),(-1,4),(0,1)):
 calls=xcalls=0;raw=pack(walk,crouch,force,blocked,enabled,forced&0xffffffff,network);cases.append(raw)
 u.mem_write(base,bytes(0x6000));u.mem_write(base+0x294,pack(base+0x3000));u.mem_write(base+0x3724,pack(walk));u.mem_write(base+0x3050,f(3.5,.5,0,1));u.mem_write(base+0x13ec,pack(0x12345678,0x23456789));u.mem_write(base+0x3034,pack(base+0x5800));u.mem_write(base+0x5800,b'run\0')
 ...
 u.mem_write(stack,pack(stop,base,force));u.reg_write(UC_X86_REG_ESP,stack);u.emu_start(0x4280b0,stop,count=10000);assert u.reg_read(UC_X86_REG_EIP)==stop
 previous=int(not(walk and not(crouch and blocked)))
 assert bytes(u.mem_read(base+0x13ec,8))==pack(0x12345678 if previous else 0,0x23456789)
 ptr=struct.unpack('<I',u.mem_read(base+0x858,4))[0];selected=struct.unpack('<I',u.mem_read(0x630050,4))[0]
 want=pack(0,previous,1,(ptr-0x62fe50)//32,int(struct.unpack('<I',u.mem_read(base+0x85c,4))[0]==0x73a858))+bytes(u.mem_read(base+0x810,4))+pack(selected,calls)+bytes(u.mem_read(base+0x8c,4))+bytes(u.mem_read(base+0x8c0,8))+bytes(u.mem_read(base+0x148,4));expected.append(want)
 ...
 nx.mem_write(base+0x1000,pack(base+0x3000,base+0x4000,base+0x5000,1,forced&0xffffffff)+f(1)+pack(int(bool(crouch)),network));nx.mem_write(stack,pack(stop,base,base+0x1000,base+0x2010,callback,0));nx.reg_write(UC_X86_REG_ESP,stack);nx.emu_start(entry,stop,count=10000)
...
pc=subprocess.check_output([str(root/'build/pc/Release/rf_entity_probe.exe'),'--normal-enter'],input=b''.join(cases));assert pc==b''.join(expected)
report=dict(result='PASS',cases=len(cases),original_sha256=sha,scope='Complete4280b0 with real class/crouch predicates,427450 speed and433a00 named descriptor lookup; standing428a60 supplied at one boundary. Callback changes response to9, including blocked attempts. Exact PC/NXDK final state and callback count verify speed reads post-callback state; blocked exits keep callback effects and skip restore. Does not implement clearance/sphere/support callbacks or live NPC landing.')
(root/'artifacts/player-normal-enter.json').write_text(json.dumps(report,indent=2));print(report)
