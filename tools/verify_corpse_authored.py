...
report=dict(result='PASS',pc_sha256=hashlib.sha256(exe.read_bytes()).hexdigest(),levels=rows,scope='PC owned corpse constructor/update/destructor with retained class metadata and startup class spheres. Authored model/clip transfer, names/body/registry/list ownership,120 frames per selected skeleton, transition and item-pose callback, temperature decay and complete resource retirement. Dynamic actor fields/material coefficients are fixture inputs; collision/source effects observed only. No item creation, original-frame comparison, live death dispatch, rendered visual or XEMU claim.')
