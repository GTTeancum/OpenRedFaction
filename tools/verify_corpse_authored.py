...
report=dict(result='PASS',pc_sha256=hashlib.sha256(exe.read_bytes()).hexdigest(),levels=rows,scope='PC scene model detach, reset/start/duration,120 shared corpse-update/evaluation/followpoint frames per unique skeleton with death_generic and final reference retirement. Original actor matrices poisoned after transfer. Authored clip loading under bounded residency. Missing sound backend rejection and negative-health early-return checked. Transition-bit8 seek/pose, authored sphere definitions with fixture radii, timer emitter disable, fade-expiry continuation and sound callback position/failure checked. No audible playback, original-frame comparison, full corpse constructor, live death dispatch, rendered visual or XEMU claim.')
