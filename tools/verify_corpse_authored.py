...
report=dict(result='PASS',pc_sha256=hashlib.sha256(exe.read_bytes()).hexdigest(),levels=rows,scope='PC scene model detach, reset/start/duration,120 shared corpse-update/evaluation/followpoint frames per unique skeleton with death_generic and final reference retirement. Original actor matrices poisoned after transfer. Authored clip loading under bounded residency. Missing item backend rejection and negative-health early-return checked. Transition-bit8 seek/pose, authored class spheres built from startup poses with preserved class radii, timer emitter disable, fade-expiry continuation and item callback position/failure and actual48a230 world-pose assignment checked. No item creation, original-frame comparison, full corpse constructor, live death dispatch, rendered visual or XEMU claim.')
