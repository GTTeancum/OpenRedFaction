...
for bones in range(1,51):
 for count in (0,8,16):
  ...
  assert take_pose(budget-1)==0xfffffffc and not calls and bytes(u.mem_read(source,300))==before and bytes(u.mem_read(owned,308))==bytes(308)
  failed=True;assert take_pose(budget)==0xfffffffc and calls==[(malloc,bones*50)] and not live
  ...
  failed=False;calls.clear();assert take_pose(budget)==0 and calls==[(malloc,bones*50)]
...
(root/('artifacts/registered-pose-verification.json' if registered else 'artifacts/owned-pose-verification.json')).write_text(json.dumps(report,indent=2)+'\n');print(report)
