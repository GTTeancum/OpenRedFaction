    ...
    for entry in archive.get('vpp',{}).get('entries',[]):
        ...
        for section in inspect(raw)['sections']:
            for lod in section.get('lods',[]):
                if not lod['flags']&2:lod_index+=1;continue
                budget=16+lod['data_bytes']+lod['batches']*16;peak=max(peak,budget);owned_lods+=1
                header=len(expected);expected.append('');max_vertices=0
                ...
                for i in range(lod['batches']):
                    ...
                    vh=hash_bytes(raw[fields[5]:fields[5]+v*12]);ph=hash_bytes(raw[fields[17]:fields[17]+v*8]);th=hash_bytes(raw[fields[11]:fields[11]+t*8])
                    expected.append('D '+' '.join(map(str,(lod_index,i,v,t,vh,ph,th))))
                    formats[str(fmt)]=formats.get(str(fmt),0)+1
                expected[header]=f'C {lod_index} {lod["batches"]} {budget} {max_vertices}'
                assert start+relative==lod['attachment_offset']
                lod_index+=1
            material_base+=section.get('materials',0)
        output=subprocess.check_output([str(root/'build/pc/Release/rf_model_file_probe.exe'),str(path),entry['name'],'--skin-geometry'],text=True)
...
report=dict(result='PASS',models=models,lods=lod_count,batches=batches,owned_lods=owned_lods,peak_owned_bytes=peak,formats=formats,
    scope='All installed skeletal LODs: exact owned positions/links/records and maximum scratch vertex count, budget-minus-one rejection, exact-budget loading, minimum active-bone capacity and one-under rejection, malformed attachment cleanup and repeated close; static missing-skeletal-flag rejection. PC file I/O; native build only, no XEMU or scene binding.')
(root/'artifacts/model-skin-geometry.json').write_text(json.dumps(report,indent=2));print(report)
