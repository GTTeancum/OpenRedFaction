...
def main():
    ...
    parser.add_argument('--vehicle-class', choices=('driller','apc','jeep','sub','fighter'), default='driller', help='Vehicle class for --vehicle-test')
    ...
    fighter=args.vehicle_test and args.vehicle_class=='fighter'
    if fighter:
    ...
    if args.firearms_test and (not args.dev_room or args.level!='ctf06.rfl' or args.fusion_test or args.player_shield_test or args.npc_shield_test or args.npc_rocket_test or args.npc_grenade_test or args.npc_rubble_test):parser.error('--firearms-test requires enemy-free ctf06 DEV without other weapon/NPC fixtures')
    ...
    report['vehicle_test']=args.vehicle_test
    report['vehicle_class']=args.vehicle_class
    ...
    if args.vehicle_test:env['RF_REPLAY_VEHICLE']=args.vehicle_class if args.vehicle_class!='driller' else '1'
...
''')
        ...
        with (run / 'stdout.log').open('wb') as out, (run / 'stderr.log').open('wb') as err:
            ...
            for name, label, count in [('rf_scene_campaign_countdown', 'CAMPAIGN_COUNTDOWN', 3), ('rf_scene_script_sound', 'SCRIPT_SOUND_STATE', 8), ('rf_scene_endgame', 'CAMPAIGN_ENDGAME', 6), ('rf_scene_endgame_clear', 'ENDGAME_CLEAR', 4), ('rf_scene_authored_identity', 'AUTHORED_IDENTITY', 10), ('rf_scene_terrain_publication', 'TERRAIN_PUBLICATION', 8), ('rf_scene_debris_audio', 'DEBRIS_AUDIO', 14), ('rf_scene_player_checkpoint_state', 'PLAYER_CHECKPOINT', 8), ('rf_scene_liquid_damage', 'LIQUID_DAMAGE', 8), ('rf_scene_player_swim', 'PLAYER_SWIM', 12), ('scene_actor_body', 'PC_PLAY_BODY', 77),
                ...
                if label=='VEHICLE':indices=list(range(6))+list(range(12,16))
                equal=all(actual[i]==expected[i] for i in indices)
                if label=='VEHICLE':
                    pc_pose=struct.unpack('<6f',struct.pack('<6I',*expected[6:12]))
                    xbox_pose=struct.unpack('<6f',struct.pack('<6I',*actual[6:12]))
                    equal=equal and all(abs(a-b)<.01 for a,b in zip(pc_pose,xbox_pose))
                ...
                if label=='AUTHORED_IDENTITY' and args.dev_room and args.level=='ctf06.rfl':
                    assert actual[9]==1 and any(actual[:8]) and 0<actual[8]<=(3 if args.vehicle_test else 2)*1024*1024,'Authored identity capture missing/overbudget'
