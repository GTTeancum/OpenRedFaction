"""Prepare non-DEV sniper/rail event acquisition and ordinary firing.
...
PROFILES={'sniper':('Sniper Rifle','Sniper Rifle',6),'rail':('rail gun','rail_gun',7)}
...
def prepare(folder,cycles):
    ...
    for kind,(item,weapon,slot) in PROFILES.items():
        ...
        event=U(910500)+S(b'Give_Item_To_Player')+F(*spawn)+S(b'precision_supply')
        ...
        for phase in ('acquired','fired'):
            ...
            jobs.append(dict(kind=kind,phase=phase,item=item,quantity=quantity,slot=slot,env=dict(RF_REPLAY_LEVEL='ctf06.rfl',RF_REPLAY_ARCHIVE='levelsm.vpp',RF_REPLAY_SETUP_UID='910500'),command=[str(ROOT/'build/pc/Release/rf_pc_play.exe'),'--spawn-replay',str(game),str(base.with_suffix('.bin')),str(base.with_suffix('.ppm'))]))
    report=dict(status='PREPARED_NOT_RUN',jobs=jobs,spawn=spawn,cycles=cycles,schedule='Give_Item0; ordinary cycle edges from30 every4frames; one primary request120; finish179.',scope='Non-DEV authored sniper/rail grants and finite-ammo fire; preserve original spawn/pickups/fullgeometry506props.',limitations='PC --spawn-replay calls set_campaign_spawn using original player section because DEV flag absent. Assumes only default handgun and granted target owned: onecycle selects6/7. Nearby original pickups could alter cycling or supply; --cycles allows explicit adjustment after live evidence. Existing item metadata can demand additional views without granting ownership. Resources and actual rendering require parent validation.')
...
def main():
    ...
    folder=ROOT/'artifacts/precision-pickup-live';folder.mkdir(parents=True,exist_ok=True);report=prepare(folder,args.cycles)
    ...
    for job in report['jobs']:
        ...
        text=base.with_suffix('.log').read_text(errors='replace');grant=words(text,'SCRIPT_GRANTS');selection=words(text,'WEAPON_SELECTION');ammo=words(text,'PLAYER_AMMO');combat=words(text,'COMBAT')
        assert grant[0]==1 and grant[7]==0 and selection[0]==job['slot'] and ammo[7]==0,(grant,selection,ammo)
        if phase=='acquired':assert ammo[2]>0 and combat[0]==0;states[kind]={'acquired':dict(ammo=ammo,combat=combat)}
        else:
            ...
            states[kind]['fired']=dict(ammo=ammo,combat=combat)
