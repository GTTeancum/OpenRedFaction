"""Verify preferred-face early return followed by full transformed room traversal."""
...
for n in range(2000):
 ...
 wire=b''.join(records)+struct.pack('<6I7fI',*primary,*children,*start,*delta,limit,flags);wire+=struct.pack('<13f',radius,*origin,*matrix)+struct.pack('<I',preferred);cases.append(wire)
...
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--preferred-rooms'],input=b''.join(cases))
...
match=re.search(r'_rf_collision_solid_preferred\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 ...
 preferred=struct.unpack_from('<I',wire,268)[0];view=base+40000;pref=view+200
 ...
 x.mem_write(stack,struct.pack('<10I',stop,view,pref if preferred!=0xffffffff else 0,flags,query,query+12,struct.unpack_from('<I',wire,216)[0],limit_bits,out,out+44));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
...
report=dict(result='PASS',original_cases=2000,preferred_hits=preferred_hits,hits=hits,multiple_updates=multiple,edge_hits=edge_hits,scope='Full original4df1c0 preferred face plus transformed room traversal, no supplied callees. PC and actual NXDK exact geometric outputs, hit counts and room/face indices. Four single-face trees, room skips/child order, swept radius, transformed/direct inputs, preferred faces bypassing room skip/bounds, accepted preference vs fallback. No global cache-list/special1000 mode or native scene binding.')
(root/'artifacts/collision-preferred-rooms.json').write_text(json.dumps(report,indent=2));print(report)
