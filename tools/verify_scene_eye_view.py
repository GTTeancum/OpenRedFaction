...
turn_rows=[list(map(int,line.split()[1:])) for line in args.trace.read_text().splitlines() if line.startswith('TURN_TENSOR ')]
if turn_rows:assert len(turn_rows)==len(rows) and look_rows
...
for frame,row in enumerate(rows):
    ...
    if look_rows:
        if not turn_rows:assert struct.unpack("<9f",data[12:48])==(1,0,0,0,1,0,0,0,1), ("pitch fixture body orientation",frame)
        look=look_rows[frame];assert len(look)==33 and look[0]==frame
        command=(.25 if frame%180<90 else -.25) if frame else 0
        yaw=(.2 if frame%240<120 else -.2) if turn_rows and frame else 0
        before=struct.pack('<2f',command,yaw)+look_state[8:]
        if turn_rows:
            tr=turn_rows[frame];assert len(tr)==19 and tr[0]==frame
            u.mem_write(entity+0x9c,struct.pack('<9I',*tr[1:10]))
        ...
        end=stop if turn_rows else 0x49e02c
        u.emu_start(0x49de50,end,count=10000);assert u.reg_read(UC_X86_REG_EIP)==end
        if turn_rows:
            assert bytes(u.mem_read(entity+0xc0,36))==struct.pack('<9I',*tr[10:]),('world inertia',frame)
            assert bytes(u.mem_read(entity+0x48,36))==data[12:48],('bound body orientation',frame)
...
if turn_rows:report['scope']='All 664 complete original 49de50 updates including 49cd30 world inertia match turn trace, followed by 4194e0 and camera copy. Body/eye orientations match live scene inputs. Scripted pitch/yaw; no original caller scheduling or interactive input claim.'
(root/('artifacts/scene-turn-view-verification.json' if turn_rows else 'artifacts/scene-look-view-verification.json' if look_rows else 'artifacts/scene-eye-view-verification.json')).write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
