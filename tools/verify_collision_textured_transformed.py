...
for n in range(5000):
 ...
 wire=b''.join(records)+struct.pack('<6I7fI',*primary,*children,*start,*delta,limit,flags);wire+=struct.pack('<13f',radius,*origin,*matrix)
...
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--transformed-rooms-textured'],input=b''.join(cases))
...
match=re.search(r'_rf_collision_transformed_rooms_textured\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 extra=struct.unpack_from('<6I',wire,268);trace=[]
 ...
 x.mem_write(stack,struct.pack('<17I',stop,rooms,4,arrays,2,arrays+8,4,flags,query,query+12,query+24,query+36,struct.unpack_from('<I',wire,216)[0],limit_bits,backend,out,out+44));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
...
(root/'artifacts/collision-textured-transformed.json').write_text(json.dumps(report,indent=2));print(report)
