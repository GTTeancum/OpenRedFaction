...
report=dict(status='PASS',original_pc_nxdk_transform_cases=256,authored_descriptors=descriptors,scope='Complete original 433a50 with unchanged callees. Parser bindings independently compared with installed table and original reference/name arrays; original file parser not executed.')
(root/'artifacts/movement-transform-verification.json').write_text(json.dumps(report,indent=2));print('PASS: 256 original/PC/NXDK transforms and 16 authored descriptors')
