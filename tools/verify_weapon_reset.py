...
boundaries={0x505a40:'stop_sound',0x41af0a:'release_sound',0x48f130:'stop_effect',0x48aa90:'local_release',0x4a6f10:'player_reset'}
...
for k in range(2400):
    ...
    state=active+struct.pack('<IIiiiIII',rng.getrandbits(32),rng.choice([0,1]),rng.choice([-1,-1,99]),rng.randrange(-1,99),rng.choice([-1,-1,9]),rng.randrange(2),rng.randrange(2),rng.randrange(2))
    descriptors=b''.join(struct.pack('<IIi',rng.randrange(8),rng.choice([0,64]),rng.choice([-2,-1,-1,0])) for _ in range(64))
    context=struct.pack('<II5i',rng.choice([0,1,2,256]),rng.choice([0,32,64]),weapon,*rng.sample(range(64),4))
    ...
    cases.append(struct.pack('<i',weapon)+state+descriptors+context+playback+resources)
run=subprocess.run([str(root/'build/pc/Release/rf_weapon_probe.exe')],input=b''.join(cases),capture_output=True,check=True)
assert len(run.stdout)==len(cases)*488
...
for k,wire in enumerate(cases):
    weapon,=struct.unpack_from('<i',wire);state=wire[4:100];descriptors=wire[100:868];context=wire[868:896];s=wire[896:1156];resources=wire[1156:]
    ...
    player,local=struct.unpack_from('<II',state,88);put(entity+0x7c,'<I',8 if player else 0);put(entity+0x1430,'<I',obj+0xb000 if player else 0)
    ...
    disabled,n,*special=struct.unpack('<II5i',context);put(0x64ecbb,'<B',disabled&255);put(0x872448,'<I',n)
    for a,v in zip([0x872110,0x872464,0x872444,0x85cd04,0x85ccfc],special):put(a,'<i',v)
    ...
    assert run.stdout[k*488:(k+1)*488]==expected,(k,hex(end))
assert all(coverage.values()),coverage
report=dict(result='PASS',cases=len(cases),coverage=coverage,scope='Original reset with valid resolved type-zero entity, full state/playback/reference comparison, unmodified callees; observation stops before unavailable sound/effect/player operations and matches explicit RF_NOT_FOUND; successful external adapters remain unverified')
