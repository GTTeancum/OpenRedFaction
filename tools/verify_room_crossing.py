...
suite=globals().get('suite','room-crossing');custom=globals().get('query_specs')
ev=runpy.run_path(str(root/'tools/verify_room_crossing_trace.py'),init_globals={'query_specs':custom,'suite':suite});w=ev['w'];f=ev['f']
...
for row in ev['results']:
 start=row['from_point'];end=row['to_point'];commands.append(f(*start,*end)+w(row['flags'],row['cached'],row['tree']));expected.append(w(0,0 if row['hit'] else -1,row['face'] if row['hit'] else -1))
...
report=dict(result='PASS',original_cases=len(ev['results']),port_guards=2,original_sha256=ev['digest'],nxdk_sha256=hashlib.sha256((root/'build/xbox/main.exe').read_bytes()).hexdigest(),scope='Shared room crossing vs original supplied cube queries, exact first room/face selection on PC and actual NXDK; two nonfinite guards. One world root and one/three-node trees; no oblique planes, multiple roots or real levels.',suite=suite)
(root/'artifacts'/(suite+'.json')).write_text(json.dumps(report,indent=2)+'\n');print(report)
