...
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--level-initial' if initial else '--level',str(root/'Installed_Game/levels1.vpp'),'L1S1.rfl'])
...
while at<len(raw):
 metadata=raw[at:at+24] if initial else struct.pack('<IIiIII',0x461,0,0,0,0,0)
 if initial:at+=24
 query_flags,face_flags,portal,present,kind,state=struct.unpack('<IIiIII',metadata)
 ...
 u.mem_write(face,head);u.mem_write(face+0x28,bytes(16));u.mem_write(face+0x28,struct.pack('<I',face_flags));u.mem_write(face+0x34,struct.pack('<h',portal));u.mem_write(face+0x30,struct.pack('<i',-1));u.mem_write(face+0x40,struct.pack('<II',edges,base+32768 if present else 0));u.mem_write(verts,vertices)
 u.mem_write(base+32768,bytes([kind]));u.mem_write(base+32768+0x98,bytes([state]))
 ...
 u.mem_write(query+0x4c,struct.pack('<fI',0,query_flags));u.mem_write(query+0x54,ray);u.mem_write(out,struct.pack('<If',0,1)+bytes(32));u.mem_write(0xca06b0,struct.pack('<I',15))
 ...
 x.mem_write(face,head+struct.pack('<II',verts,count)+metadata);x.mem_write(verts,vertices);x.mem_write(query,ray);x.mem_write(out,bytes([0xa5])*32)
...
assert at==len(raw)
report=dict(result='PASS',nxdk_sha256=hashlib.sha256(xbox_path.read_bytes()).hexdigest(),initial_metadata=initial,level='L1S1.rfl',faces=cases,original_bounds_cases=cases,hits=hits,max_corners=max_corners,capacity_guards=cases,scope='Every loaded face bound to caller scratch and tested against complete original 4dec10. Filter values are file/room initial views with --initial, otherwise clear diagnostic face/owner attributes; later runtime changes excluded. Rays use vertex-average plus normal and -2*normal. No world traversal or nearest world result.')
(root/('artifacts/collision-level-initial-verification.json' if initial else 'artifacts/collision-level-verification.json')).write_text(json.dumps(report,indent=2));print(report)
