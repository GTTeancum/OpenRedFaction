...
records=subprocess.check_output([str(root/'build/pc/Release/rf_geometry_probe.exe'),str(path),'L1S1.rfl','--shadow-receivers']);receiver_values={};cursor=0
while cursor<len(records):
 face,n=struct.unpack_from('<2I',records,cursor);cursor+=8;receiver_values[face]=records[cursor:cursor+n*8];cursor+=n*8
...
for mapping in checks:
 ...
 x.mem_write(GROUP_WORK,w(POLYGONS,VERTICES,np,nv));x.mem_write(VIEW,w(128,128,raw[g['mapping_offset']+mapping*96+4],raw[g['mapping_offset']+mapping*96+5])+bytes(40));x.mem_write(OUT,bytes([165])*8)
...
report=dict(result='PASS',pc_mapping_groups=g['mappings'],pc_vertices=total,nxdk_groups=len(checks),nxdk_guards=3,nxdk_empty_queries=1,peak_polygon_vertex_scratch_bytes=peak,scope='L1S1 retained grouping compared to file-order mapping/room filtering and individually verified receiver UV outputs with supplied 128x128 images. PC count queries and one-vertex-short guards checked; compiled NXDK sampled mapping groups match pointers/counts/contents. Loader survival and native rendering remain outside scope.')
