...
boundaries={0x4a4cf5:'message',0x4aa0b0:'apply',0x4ad8a0:'followup',0x4a4db4:'complete'}
...
for k in range(4000):
    ...
    inp=struct.pack('<6i5I',requested,first,second,rng.choice([0,16,32,64]),primary,secondary,
        rng.choice([0,0xffffffff,1,0x80000000]),rng.choice([0,16,255]),
        rng.choice([0,1,256]),rng.choice([0,1,256]),rng.choice([0,1,255,256]))
    owned=bytes(rng.choice([0,0,1,255]) for _ in range(64))
    flags=struct.pack('<64I',*[rng.choice([0,0,0x40000]) for _ in range(64)])
    cases.append(struct.pack('<2i',pending,deadline)+inp+owned+flags+struct.pack('<I',rng.choice([0,32,64])))
...
coverage={name:0 for name in boundaries.values()};queued=0;unowned_followup=0
def put(a,fmt,*v):u.mem_write(a,struct.pack(fmt,*v))
for k,wire in enumerate(cases):
    pending,deadline=struct.unpack_from('<2i',wire)
    requested,first,second,split,primary,secondary,mask,pflags,force,defer,followup=struct.unpack_from('<6i5I',wire,8)
    before=bytearray([0xa5])*0x1000
    struct.pack_into('<i',before,0xf80,pending);struct.pack_into('<i',before,0xb8,deadline)
    struct.pack_into('<I',before,0x10,pflags);before[0xf94]=followup&255
    u.mem_write(player,bytes(before));put(entity+0x1428,'<I',mask)
    put(inventory+4,'<ii',primary,secondary);u.mem_write(inventory+0x18c,wire[52:116])
    for i in range(64):u.mem_write(0x85cf6c+i*1360,wire[116+4*i:120+4*i])
    u.mem_write(0x872448,wire[372:376]);put(0x85ccd8,'<i',first);put(0x85cd00,'<i',second);put(0x87211c,'<i',split)
    ...
    assert struct.unpack_from('<3i',out,k*12)==(expected_status,*actual),(k,hex(end),actual,struct.unpack_from('<3i',out,k*12))
    ...
    unowned_followup+=end==0x4ad8a0 and actual==(pending,deadline)
assert all(coverage.values()) and queued and unowned_followup
report=dict(result='PASS',cases=len(cases),coverage=coverage,queued=queued,followup_without_queue=unowned_followup,
    scope='4a4c91..4a4db4 with unchanged paired-bit, ownership, descriptor, local-player, queue/timer and followup-byte callees; observation stops before message/apply/followup; earlier selection gates and successful external operations excluded')
