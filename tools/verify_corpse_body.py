"""Original49ec90 corpse physics seed versus owned PC/NXDK body allocation."""
...
for case in range(640):
 count=(0,1,2,4,8)[case%5];mass=(0,-1,3,100)[(case//5)%4];radius=1.25;flags=0x73 if case%2 else 0x33
 response=(10,2.5)[case%2];material=(.25,.5,2);position=[rng.randrange(-80,81)*.25 for _ in range(3)]
 ...
 raw=bytes(u.mem_read(b,0x170));used=read(u,b+0xfc);assert used==(count or 1)
 ...
 if not count:owned=owned[:20]+w(0) # original fallback opaque word is undefined
 else:assert owned==spheres
 expected.append(w(0)+state+w(used)+owned)
pc=subprocess.check_output([str(root/'build/pc/Release/rf_entity_probe.exe'),'--corpse-body'],input=b''.join(commands));assert pc==b''.join(expected),'PC body mismatch'
mapping=(root/'build/xbox/main.map').read_text();symbol=lambda name:int(re.search(r'\s_'+name+r'\s+([0-9a-fA-F]+)',mapping)[1],16)
entry=symbol('rf_corpse_body_open');close=symbol('rf_physics_body_close');malloc=symbol('malloc');free=symbol('free');heap_calls=[];fail_heap=False
...
for case,(wire,want) in enumerate(zip(commands,expected)):
 values=struct.unpack('<20I',wire[:80]);count=values[19];used=count or 1;budget=324+used*24
 ...
 assert call(entry,args)==0 and heap_calls==[(malloc,used*24)]
 ...
 heap_calls.clear();call(close,[b]);assert heap_calls==[(free,b+0x5000)] and bytes(x.mem_read(b,324))==bytes(324)
 heap_calls.clear();call(close,[b]);assert all(a==free and arg==0 for a,arg in heap_calls)
report=dict(result='PASS',cases=len(commands),heap_failures=len(commands),budget_failures=len(commands),original_sha256=digest,nxdk_sha256=hashlib.sha256((root/'build/xbox/main.exe').read_bytes()).hexdigest(),scope='Complete original49ec90/49f010 with no geometric model vs owned PC/NXDK corpse bodies. Generated/inherited mass,0..8 spheres,33/73 flags, default material. Original heap supplied; fallback opaque word normalized to zero. Exact Xbox budgets, one owned allocation, failure preservation and close verified. No live scene/XEMU binding.')
(root/'artifacts/corpse-body-verification.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
