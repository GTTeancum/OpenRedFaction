...
for n in range(5000):
 z=[rng.choice([-4,-2,0,2,4,6]) for _ in range(4)];count=rng.randrange(5);flags=rng.choice([0,1,4,5,0x1004,0x1005])|rng.choice([0,0x80,0x100,0x180])
 ...
 wire=struct.pack('<4f2I20f',*z,count,flags,*start,*delta,radius,limit,*origin,*matrix)
 ...
 samples+=len(trace)
 expected.append((struct.pack('<iI',0,hit)+value if not status else struct.pack('<i',status)+b'\xa5'*44)+tail(kept))
for offset,value in [(48,-1),(48,math.nan),(52,math.nan),(24,math.inf),(56,math.nan),(68,math.nan)]:
 wire=bytearray(cases[1]);struct.pack_into('<I',wire,20,0x460);struct.pack_into('<f',wire,offset,value);cases.append(bytes(wire));expected.append(struct.pack('<i',-2)+bytes([0xa5])*44+tail([]))
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--flat-query-textured'],input=b''.join(cases));assert len(raw)==len(cases)*56
for n,want in enumerate(expected):assert raw[n*56:(n+1)*56]==want,('PC',n,raw[n*56:(n+1)*56].hex(),want.hex())
...
match=re.search(r'_rf_collision_flat_faces_textured\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match;entry=int(match[1],16)
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 extra=struct.unpack_from('<6I',wire,104);trace=[];x.mem_write(backend,struct.pack('<3I',bitmaps,0 if extra[5] else callback,77))
 ...
 x.mem_write(stack,struct.pack('<13I',stop,faces,count,flags,query+24,query+36,query+56,query+68,struct.unpack_from('<I',wire,48)[0],struct.unpack_from('<I',wire,52)[0],backend,out,out+40));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
 x.emu_start(entry,stop,count=1000000);assert x.reg_read(UC_X86_REG_EIP)==stop
 got=struct.pack('<I',x.reg_read(UC_X86_REG_EAX))+bytes(x.mem_read(out+40,4))+bytes(x.mem_read(out,40))+tail(trace);assert got==want,('NXDK',n,got.hex(),want.hex())
assert samples>0 and callback_errors>0 and missing_errors>0 and late_errors>0
report=dict(result='PASS',samples=samples,callback_errors=callback_errors,missing_backend_errors=missing_errors,late_errors=late_errors,original_cases=5000,port_guards=6,hits=hits,multiple_updates=multiple,edge_hits=edge_hits,nxdk_sha256=hashlib.sha256((root/'build/xbox/main.exe').read_bytes()).hexdigest(),scope='Complete original 4df1c0 uncached zero-room fallback, only UV/bitmap services supplied; geometry callees unchanged. Ordered linked faces, input transforms, thin/swept radii, bit-0 non-early-exit, ties and empty lists. PC and NXDK exact geometric/index/count and sampler order/contact hash comparison with output preservation on errors; no original preferred face, cache or loaded mover binding.')
(root/'artifacts/collision-textured-flat.json').write_text(json.dumps(report,indent=2));print(report)
