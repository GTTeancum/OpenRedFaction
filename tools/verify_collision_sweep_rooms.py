...
for n in range(5000):
 ...
 wire=b''.join(records)+struct.pack('<6I7fI',*primary,*children,*start,*delta,limit,flags);wire+=struct.pack('<f',radius);cases.append(wire)
...
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--sweep-rooms'],input=b''.join(cases))
...
match=re.search(r'_rf_collision_sweep_rooms\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match
...
for n,(wire,want) in enumerate(zip(cases,expected)):
 ...
 x.mem_write(stack,struct.pack('<14I',stop,rooms,4,arrays,2,arrays+8,4,flags,query,query+12,struct.unpack_from('<I',wire,216)[0],limit_bits,out,out+44));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
