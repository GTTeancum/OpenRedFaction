"""Verify integrated selected-turn effects through unmodified original callees."""
...
for k in range(2400):
    ...
    cases.append((state,resources,actions,sounds,effects,context,x))
wire=b''.join(s+e+c+struct.pack('<f',x)+b''.join(r)+struct.pack('<90i',*a,*n) for s,r,a,n,e,c,x in cases)
run=subprocess.run([str(root/'build/pc/Release/rf_turn_probe.exe')],input=wire,capture_output=True,check=True)
assert len(run.stdout)==len(cases)*440
entity=obj+0x6000;wrapper=obj+0x4000;info=obj+0x9000
for k,(s,resources,actions,sounds,effects,context,x) in enumerate(cases):
    ...
    u.emu_start(0x41fbdc,stop,count=100000);assert u.reg_read(UC_X86_REG_EIP)==stop
...
s,r,a,n,e,c,x=cases[0]
base=s+e+c+struct.pack('<f',x)+b''.join(r)+struct.pack('<90i',*a,*n)
bad=[]
for offset,fmt,value in [(344,'<i',1072800001),(348,'<f',float('nan'))]:
    raw=bytearray(base);struct.pack_into(fmt,raw,offset,value);bad.append(bytes(raw))
out=subprocess.run([str(root/'build/pc/Release/rf_turn_probe.exe')],input=b''.join(bad),capture_output=True,check=True).stdout
assert len(out)==len(bad)*440
for i in range(len(bad)):
    assert struct.unpack_from('<i',out,i*440)[0]!=0
    assert out[i*440+4:(i+1)*440]==s+e+struct.pack('<i',-99)+b''.join(v[32:36] for v in r)
report=dict(result='PASS',cases=len(cases),rejection_cases=len(bad),scope='Original 0x41fbdc..0x41fc83 and unmodified action restart, absent-sound resolver, timer and movement callees; all playback/references, candidates, turn flag, deadlines and movement fields; preceding decisions/reset and valid-sound playback excluded')
(root/'artifacts/turn-effects-verification.json').write_text(json.dumps(report,indent=2));print(report)
