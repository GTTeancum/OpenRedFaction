"""Original preferred-face early return plus complete no-room fallback versus PC/NXDK."""
...
for n in range(2400):
 z=[rng.choice([-4,-2,0,2,4,6]) for _ in range(4)];count=rng.randrange(5);flags=rng.choice([4,5,0x460,0x461,0x464,0x465])
 ...
 wire=struct.pack('<4f2I20f',*z,count,flags,*start,*delta,radius,limit,*origin,*matrix)+struct.pack('<I',preferred);cases.append(wire)
 ...
 u.mem_write(query,bytes(128));u.mem_write(query,struct.pack('<I',faces+128*preferred if preferred!=0xffffffff else 0));u.mem_write(query+4,wire[56:104]);u.mem_write(query+0x34,wire[24:52]+struct.pack('<I',flags));u.mem_write(out,struct.pack('<If',0,limit)+bytes(32));u.mem_write(0xca06e0,bytes(8));u.mem_write(0xca06b0,struct.pack('<I',15));u.mem_write(0x1754525,b'\x03');u.mem_write(0x1754558,bytes(12));u.mem_write(0x1754488,bytes(12))
 ...
 if hit:
  ...
  value=bytes(u.mem_read(out+4,28))+struct.pack('<4I',index,updates,edge,555 if early else 0xffffffff)
...
for offset,value in [(48,-1),(48,math.nan),(52,math.nan),(24,math.inf),(56,math.nan),(68,math.nan)]:
 wire=bytearray(cases[1]);struct.pack_into('<I',wire,20,0x460);struct.pack_into('<f',wire,offset,value);cases.append(bytes(wire));expected.append(struct.pack('<i',-2)+bytes([0xa5])*48)
raw=subprocess.check_output([str(root/'build/pc/Release/rf_collision_probe.exe'),'--preferred-flat-query'],input=b''.join(cases));assert len(raw)==len(cases)*52
...
match=re.search(r'_rf_collision_solid_preferred\s+([0-9a-fA-F]+)',(root/'build/xbox/main.map').read_text());assert match;entry=int(match[1],16)
for n,(wire,want) in enumerate(zip(cases,expected)):
 ...
 preferred=struct.unpack_from('<I',wire,104)[0];view=base+0x6000;pref=view+0x100
 x.mem_write(view,bytes(156));x.mem_write(view+48,wire[56:104]);x.mem_write(view+148,struct.pack('<II',faces,count))
 x.mem_write(pref,struct.pack('<III',faces+preferred*72 if preferred!=0xffffffff else 0,555,preferred))
 x.mem_write(query,wire);x.mem_write(out,bytes([0xa5])*48)
 x.mem_write(stack,struct.pack('<10I',stop,view,pref if preferred!=0xffffffff else 0,flags,query+24,query+36,struct.unpack_from('<I',wire,48)[0],struct.unpack_from('<I',wire,52)[0],out,out+44));x.reg_write(UC_X86_REG_ESP,stack);x.reg_write(UC_X86_REG_FPCW,0x37f)
...
assert preferred_hits and hits>preferred_hits and multiple
report=dict(result='PASS',original_cases=2400,port_guards=6,preferred_hits=preferred_hits,hits=hits,multiple_updates=multiple,edge_hits=edge_hits,scope='Complete original4df1c0 with actual preferred4dec10 and unmodified no-room fallback versus PC/compiled NXDK. Cached faces may be outside active flat list; flags bit0 enables early return. Miss/disabled preference, transformed/direct inputs, zero motion, swept radius, retained limits and ties. Original calls observed only, none supplied. Hierarchical preferred fallback and native scene binding remain separate.')
(root/'artifacts/collision-preferred-verification.json').write_text(json.dumps(report,indent=2));print(report)
