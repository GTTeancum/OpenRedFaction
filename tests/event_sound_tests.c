...
static uint32_t starts,stops,black_starts,black_stops,blast_starts,blast_stops;
...
int main(void)
{
    rf_runtime_event items[4]={{0}};rf_level_owned_event authored[4]={{0}};
    ...
    rf_object_registry_init(&registry);events.registry=triggers.registry=&registry;events.items=items;events.count=4;
    for(i=0;i<4;i++){items[i].object_kind=6;items[i].authored=authored+i;items[i].state.deadline=-1;
    ...
    puts("PASS scripted sound, blackout and explode dispatch");return 0;
}
