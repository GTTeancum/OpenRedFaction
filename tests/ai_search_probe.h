...
static int ai_search_probe(uint32_t retained)
{
 ...
 while(fread(&wire,sizeof(wire),1,stdin)==1) {
  ...
  out[1]=99;
  if(retained==4){
   rf_collision_solid_view solid={0};rf_collision_face face={0};float vertices[4][3]={{-100,-100,0},{100,-100,0},{100,100,0},{-100,100,0}};
   ...
   if(wire.visible[6]){face.plane[2]=1;face.vertices=vertices;face.count=4;face.minimum[0]=face.minimum[1]=-100.0001f;face.maximum[0]=face.maximum[1]=100.0001f;face.minimum[2]=-.0001f;face.maximum[2]=.0001f;solid.flat_faces=&face;solid.flat_count=1;request.solid=&solid;}
