...
#include "../src/diagnostic/scene_ai_ammo_fallback.inc"
#define CHECK(x) do{if(!(x)){fprintf(stderr,"AI fallback line%d: %s\n",__LINE__,#x);return 1;}}while(0)
static int adapter_case(void)
{
    ...
    owner.view.weapons[0]=0;owner.inventory.owned[0]=owner.inventory.owned[1]=1;owner.inventory.loaded[1]=2;
    ...
    CHECK(!campaign_enemy_ammo_fallback(0,&changed) && changed==1);
...
int main(void)
{
    rf_weapon_inventory inventory={0},saved;rf_weapon_acquire_definition ammo[8]={0};
    int32_t ids[8]={0,1,2,3,4,5,6,7};uint32_t i;
    for(i=0;i<8;i++){ammo[i].ammo_type=i;ammo[i].magazine=6;}
    inventory.owned[0]=inventory.owned[1]=inventory.owned[2]=1;
    inventory.reserve[1]=4;saved=inventory;
    CHECK(campaign_enemy_fallback_pick(&inventory,ammo,8,ids,0,0)==1);
    CHECK(campaign_enemy_fallback_pick(&inventory,ammo,8,ids,0,1)==-1);
    CHECK(campaign_enemy_fallback_pick(&inventory,ammo,8,ids,0,2)==1);
    CHECK(!memcmp(&inventory,&saved,sizeof(saved)));
    inventory.reserve[1]=0;
    CHECK(campaign_enemy_fallback_pick(&inventory,ammo,8,ids,0,0)==2);
    inventory.owned[2]=0;inventory.owned[6]=1;inventory.loaded[6]=1;
    CHECK(campaign_enemy_fallback_pick(&inventory,ammo,8,ids,0,0)==6);
    inventory.loaded[6]=0;inventory.owned[4]=1;inventory.loaded[4]=5;
    CHECK(campaign_enemy_fallback_pick(&inventory,ammo,8,ids,0,0)==-1);
    CHECK(!adapter_case());
    puts("NPC fallback selects finite owned ammo, preserves Attack and excludes unsupported weapons");return 0;
}
