...
{world_draw_context *c=user;++c->out->placed;if(c->out->placed==1 && (c->in->missing&256))c->in->draw.hand_count=0;if(c->in->missing&(1u<<hand))return RF_NOT_FOUND;*pose=c->in->poses[hand];return RF_OK;}
static int world_draw_submit(void *user,uint32_t model,const rf_weapon_hand_placement *pose,const uint32_t state[20])
{world_draw_context *c=user;uint32_t n=c->out->submitted++;if(n==0 && (c->in->missing&512)){c->in->draw.hand_count=2;c->in->draw.view.flags_810^=256;}if(n>=2)return RF_RANGE;c->out->records[n].model=model;c->out->records[n].pose=*pose;memcpy(c->out->records[n].state,state,80);return RF_OK;}
static int world_draw_probe(void)
{
 ...
 while(fread(&in,sizeof(in),1,stdin)==1){memset(&out,0,sizeof(out));out.status=rf_weapon_world_draw_run(&in.draw,in.models,in.scratch,&ops,&c);out.draw=in.draw;memcpy(out.scratch,in.scratch,80);if(fwrite(&out,sizeof(out),1,stdout)!=1)return 1;}return ferror(stdin)?1:0;
}
