...
int main(int argc,char **argv)
{
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-rotation-keys")) {
        uint32_t input[2];unsigned char *data;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,8,1,stdin)==1) {
            float out[4];int32_t status,time;uint32_t bytes;
            ...
            status=rf_vfx_rotation_key_sample(data,bytes,input[0],time,out);free(data);fwrite(&status,4,1,stdout);fwrite(out,16,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-transform")) {
        float input[13],out[3];int32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,sizeof(input),1,stdin)==1) {
            memset(out,0xa5,sizeof(out));status=rf_vfx_transform_point(input,input+10,out);
            fwrite(&status,4,1,stdout);fwrite(out,sizeof(out),1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    if(argc==2 && !strcmp(argv[1],"--vfx-transform-sample")) {
        unsigned char input[130];
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,sizeof(input),1,stdin)==1) {
            float frame[8],a[10],b[10],t;uint32_t mode,flags;rf_vfx_morph_sample out;int32_t status;
            memcpy(frame,input,32);memcpy(a,input+38,40);memcpy(b,input+78,40);memcpy(&t,input+118,4);memcpy(&mode,input+122,4);memcpy(&flags,input+126,4);memset(&out,0xa5,sizeof(out));
            status=rf_vfx_transform_sample(frame,input+32,a,b,t,(int)mode,flags,&out);fwrite(&status,4,1,stdout);fwrite(&out,sizeof(out),1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-key-time")) {
        float input[2];int32_t status,out;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,sizeof(input),1,stdin)==1) {
            out=0x12345678;status=rf_vfx_key_time(input[0],input[1],&out);fwrite(&status,4,1,stdout);fwrite(&out,4,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-parent")) {
        float input[20];rf_vfx_morph_sample sample,out;int32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,sizeof(input),1,stdin)==1) {
            memcpy(&sample,input,32);memset(&out,0xa5,sizeof(out));status=rf_vfx_parent_sample(&sample,input+8,&out);
            fwrite(&status,4,1,stdout);fwrite(&out,sizeof(out),1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-face-normal")) {
        float input[9],out[3];int32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,sizeof(input),1,stdin)==1) {
            memset(out,0xa5,sizeof(out));status=rf_vfx_face_normal(input,out);fwrite(&status,4,1,stdout);fwrite(out,12,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-face-append")) {
        uint32_t header[6];float depths[3];rf_vfx_sort_record records[8];int32_t material,status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(header,24,1,stdin)==1) {
            if(fread(records,128,1,stdin)!=1 || header[1]>8)return 2;
            memcpy(&material,header+2,4);memcpy(depths,header+3,12);
            status=rf_vfx_face_append(depths,material,0x30002000,records,header[1],header);
            fwrite(&status,4,1,stdout);fwrite(header,4,1,stdout);fwrite(records,128,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    if(argc==2 && !strcmp(argv[1],"--vfx-face-prepare")) {
        rf_vfx_face_input input;rf_vfx_face_output out;int32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-point-light")) {
        float v[10],out[2];uint32_t soften;int32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(v,sizeof(v),1,stdin)==1) {
            if(fread(&soften,4,1,stdin)!=1)return 2;memset(out,0xa5,8);
            status=rf_vfx_point_light(v,v+3,v+6,v[9],soften,out);fwrite(&status,4,1,stdout);fwrite(out,8,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-point-lighting")) {
        uint32_t count;float v[9];rf_vfx_point_source lights[32];unsigned char out[3];int32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&count,4,1,stdin)==1) {
            ...
            status=rf_vfx_point_lighting(v,v+3,v+6,lights,count,out);fwrite(&status,4,1,stdout);fwrite(out,3,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-cached-lighting")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&count,4,1,stdin)==1) {
            if(count>32 || fread(bounds,sizeof(bounds),1,stdin)!=1 || fread(&query,sizeof(query),1,stdin)!=1 ||
                fread(v,sizeof(v),1,stdin)!=1 || fread(candidates,sizeof(*candidates),count,stdin)!=count)return 2;
            memset(&cache,0,sizeof(cache));cache.capacity=32;cache.indices=indices;selected=0;memset(rgb,0xa5,3);
            status=rf_vfx_light_cache_refresh(&cache,1,candidates,count,bounds,bounds+3);
            if(!status)status=rf_vfx_lights_prepare(candidates,count,&cache,&query,lights,32,&selected);
            if(!status)status=rf_vfx_lighting(v,v+3,v+6,v[9],lights,selected,rgb);
            fwrite(&status,4,1,stdout);fwrite(&selected,4,1,stdout);fwrite(rgb,3,1,stdout);
        }return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-lights-box")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&count,4,1,stdin)==1) {
            if(count>32 || fread(query,4,6,stdin)!=6 || fread(flags,4,2,stdin)!=2 || fread(lights,sizeof(*lights),count,stdin)!=count)return 2;
            memset(indices,0xa5,sizeof(indices));selected=0xa5a5a5a5u;
            status=rf_vfx_lights_box(lights,count,query,query+3,flags[0],flags[1],indices,32,&selected);
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-material-track")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(header,12,1,stdin)==1) {
            if(header[0]>128 || fread(samples,4,header[0],stdin)!=header[0])return 2;
            memcpy(&rate,header+1,4);memcpy(&time,header+2,4);memset(&out,0xa5,4);status=rf_vfx_material_track(samples,header[0],rate,time,&out);
            fwrite(&status,4,1,stdout);fwrite(&out,4,1,stdout);
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--vfx-instance")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(input,16,1,stdin)==1) {
            rf_vfx_mesh *mesh=NULL;rf_vfx_instance *instance=NULL;
            if(fread(legacy,28,1,stdin)!=1 || input[3]>1048576)return 2;
            data=malloc(input[3]?input[3]:1);if(!data)return 2;
            if(fread(data,1,input[3],stdin)!=input[3]){free(data);return 2;}
            status=rf_vfx_mesh_open(data,input[3],input[0],input[1],legacy,input[2],&mesh);free(data);if(status)return 3;
            status=rf_vfx_instance_open(mesh,1048576,&instance);if(status)return 4;
            fwrite(&instance->allocated_bytes,4,1,stdout);
            for(i=0;i<4;++i) {
    ...
    if((argc==2 && !strcmp(argv[1],"--projectile-models")) ||
       (argc==4 && !strcmp(argv[1],"--projectile-model-load"))) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        if(argc==4) {
            rf_vpp a;memset(&out,0xa5,sizeof(out));code=rf_vpp_open(&a,argv[2]);
            if(!code){code=rf_projectile_model_catalog_load(&a,(uint32_t)strtoul(argv[3],NULL,10),&out);rf_vpp_close(&a);}
            fwrite(&code,4,1,stdout);fwrite(&out,sizeof(out),1,stdout);return 0;
        }
        while(fread(&size,4,1,stdin)==1) {
            if(size>1048576)return 2;text=malloc(size?size:1);if(!text)return 2;
            if(fread(text,1,size,stdin)!=size)return 2;memset(&out,0xa5,sizeof(out));
            code=rf_projectile_model_catalog_read(text,size,&out);free(text);
            if(fwrite(&code,4,1,stdout)!=1 || fwrite(&out,sizeof(out),1,stdout)!=1)return 2;
        }
        return ferror(stdin)?1:0;
    }
    if(argc==2 && !strcmp(argv[1],"--weapon-model-names")) {
        ...
        while(fread(&size,4,1,stdin)==1) {
            if(size>1048576)return 2;text=malloc(size?size:1);if(!text)return 2;
            if(fread(text,1,size,stdin)!=size)return 2;memset(&out,0xa5,sizeof(out));
            code=rf_weapon_model_names_read(text,size,&out);free(text);
            if(fwrite(&code,4,1,stdout)!=1 || fwrite(&out,sizeof(out),1,stdout)!=1)return 2;
        }
        return ferror(stdin)?1:0;
    }
    if(argc==4 && !strcmp(argv[1],"--weapon-model-load")) {
        rf_vpp a;rf_weapon_model_names out;int32_t code;memset(&out,0xa5,sizeof(out));
        code=rf_vpp_open(&a,argv[2]);if(!code){code=rf_weapon_model_names_load(&a,(uint32_t)strtoul(argv[3],NULL,10),&out);rf_vpp_close(&a);}
        _setmode(_fileno(stdout),_O_BINARY);fwrite(&code,4,1,stdout);fwrite(&out,sizeof(out),1,stdout);return 0;
    }
    ...
    if(argc==4 && !strcmp(argv[1],"--clutter-load"))return clutter_load_probe(argv[2],(uint32_t)strtoul(argv[3],NULL,10));
    ...
    if(argc==4 && !strcmp(argv[1],"--glare-definition")) {
        ...
        status=rf_glare_definition_read(text,(uint32_t)size,argv[3],&value);free(text);
    ...
    if(argc==5 && !strcmp(argv[1],"--clutter-skin-assets")) {
        FILE *f=fopen(argv[2],"rb");long size;rf_entity_assets before;char glare[64],old_glare[64];uint32_t present=0xa5a5a5a5;
        if(!f)return 2;fseek(f,0,SEEK_END);size=ftell(f);rewind(f);
        if(size<0 || size>1024*1024){fclose(f);return 2;}
        text=malloc((size_t)size+1);if(!text){fclose(f);return 2;}
        if(fread(text,1,(size_t)size,f)!=(size_t)size){free(text);fclose(f);return 2;}fclose(f);
        memset(&assets,0xa5,sizeof(assets));before=assets;memset(glare,0xa5,64);memcpy(old_glare,glare,64);
        status=rf_clutter_skin_assets_read(text,(uint32_t)size,argv[3],argv[4],&assets,glare,&present);free(text);
        if(status && (memcmp(&assets,&before,sizeof(assets)) || memcmp(glare,old_glare,64) || present!=0xa5a5a5a5))return 3;
        _setmode(_fileno(stdout),_O_BINARY);fwrite(&status,4,1,stdout);fwrite(&assets,sizeof(assets),1,stdout);
        fwrite(glare,64,1,stdout);fwrite(&present,4,1,stdout);return 0;
    }
    ...
    if(argc==4 && (!strcmp(argv[1],"--pain-groups") || !strcmp(argv[1],"--damage-sound-groups") || !strcmp(argv[1],"--impact-sound-group") || !strcmp(argv[1],"--squash-sound-group"))) {
        uint32_t count=(!strcmp(argv[1],"--impact-sound-group") || !strcmp(argv[1],"--squash-sound-group"))?1:!strcmp(argv[1],"--pain-groups")?2:3;
        ...
        status=!strcmp(argv[1],"--squash-sound-group")?rf_entity_squash_sound_group_read(text,(uint32_t)size,argv[3],&owner,groups):count==1?rf_entity_impact_sound_group_read(text,(uint32_t)size,argv[3],&owner,groups):count==2?rf_entity_pain_groups_read(text,(uint32_t)size,argv[3],&owner,groups):
    ...
    if(argc==4 && !strcmp(argv[1],"--unholster-delay")) {
        ...
        text=malloc((size_t)size+1);if(!text){fclose(f);return 2;}
        if(fread(text,1,(size_t)size,f)!=(size_t)size){fclose(f);free(text);return 2;}fclose(f);
        memset(&value,0xa5,sizeof(value));status=rf_entity_unholster_delay_read(text,(uint32_t)size,argv[3],&value);
        free(text);_setmode(_fileno(stdout),_O_BINARY);fwrite(&status,4,1,stdout);fwrite(&value,sizeof(value),1,stdout);return 0;
    }
