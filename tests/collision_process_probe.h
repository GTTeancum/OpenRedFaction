typedef struct pair_process_probe {
 uint32_t words[164],trace[72],count;
...
static uint32_t pair_process_call(void *ctx,uint32_t op,const void *first,const void *second)
{
 ...
 if(op<2){index=(uint32_t)((const rf_collision_pair_record *)first-p->nodes);a=0x30001000u+index*16;}
 p->trace[p->count++]=op;p->trace[p->count++]=a;p->trace[p->count++]=c;
 if(op<2){return p->words[4+index*6+4+op];}
 return 0;
}
...
static int collision_process_probe(void)
{
 ...
 while(fread(p.words,sizeof(p.words),1,stdin)==1) {
  memcpy(p.actors,p.words+52,sizeof(p.actors));p.count=0;
  for(uint32_t i=0;i<8;++i){uint32_t *r=p.words+4+i*6;p.nodes[i].pair.next=r[0]==UINT32_MAX?NULL:&p.nodes[r[0]].pair;
  ...
  for(uint32_t i=0;i<8;++i){p.words[4+i*6]=process_node_index(p.nodes[i].pair.next,&p);p.words[7+i*6]=p.nodes[i].flags;}
