typedef struct pair_process_probe {
 uint32_t words[404],trace[72],count;
...
{
 pair_process_probe *p=ctx;uint32_t a=(uint32_t)(uintptr_t)first,c=(uint32_t)(uintptr_t)second;
...
static int collision_process_probe(void)
{
 ...
 while(fread(p.words,sizeof(p.words),1,stdin)==1) {
  for(uint32_t i=0;i<16;++i){memcpy(&p.actors[i],p.words+36+i*23,36);p.actors[i].allowed_handles=p.words+36+i*23+9;memcpy(p.actors[i].position,p.words+36+i*23+17,24);}p.count=0;
  for(uint32_t i=0;i<8;++i){uint32_t *r=p.words+4+i*4;p.nodes[i].pair.next=r[0]==UINT32_MAX?NULL:&p.nodes[r[0]].pair;
  ...
  for(uint32_t i=0;i<8;++i){p.words[4+i*4]=process_node_index(p.nodes[i].pair.next,&p);p.words[7+i*4]=p.nodes[i].flags;}
