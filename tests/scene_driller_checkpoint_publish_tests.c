/* Actual registered parked host publication; caller geometry admission is outside this test. */
#include <stdio.h>
#include "../src/diagnostic/scene.c"
#include "../src/diagnostic/scene_driller_checkpoint_adapter.inc"
#include "../src/diagnostic/scene_driller_checkpoint_publish.inc"
#define CHECK(x) do{if(!(x)){fprintf(stderr,"driller checkpoint publish line%d: %s\n",__LINE__,#x);return 1;}}while(0)
...
int main(void)
{
    ...
    uint32_t i,handle;char path[128];
    ...
    CHECK(!memcmp(&runtime,&saved,sizeof(runtime)) && runtime.entry.host.driver==UINT32_MAX);
    ...
    /* Rejected occupancy leaves every live byte untouched. */
    saved=runtime;decoded.player_occupied=1;
    CHECK(scene_driller_checkpoint_apply_parked(&stream,&decoded)==RF_RANGE && !memcmp(&runtime,&saved,sizeof(runtime)));
    decoded.player_occupied=0;
    ...
    CHECK(!scene_driller_checkpoint_apply_parked(&stream,&decoded));
    CHECK(runtime.entry.registration.handle==handle && runtime.damage.damage.state.effects.handle==handle);
    CHECK(!memcmp(runtime.physics.state.inverse_inertia,saved.physics.state.inverse_inertia,36));
    ...
    CHECK(stream.driller_weapon.spin==7 && stream.driller_weapon.accepted_cuts==2 && !stream.driller_weapon.phase);
    CHECK(!stream.driller_weapon.held && !stream.driller_weapon.contact_active && !stream.driller_weapon.damage_wait);
    CHECK(!scene_driller_checkpoint_capture(&stream,&record));
    for(i=0;i<3;i++)CHECK(fabsf(record.angular_velocity[i]-decoded.angular_velocity[i])<.0001f);
    decoded.health=-2;decoded.alive=0;CHECK(!scene_driller_checkpoint_apply_parked(&stream,&decoded));
    CHECK(runtime.damage.damage.destroyed && !runtime.entry.host.alive && !runtime.damage.eject_pending);
    ...
    puts("PASS actual parked Driller publish, occupied atomic rejection, fresh handles/inertia, transient resets, alive/dead restore");return 0;
}
