...
int main(void)
{
    ...
    CHECK(rf_event_checkpoint_restore(wire,192,identity,&fresh,1000)==RF_FORMAT&&!memcmp(&fresh,&before,sizeof(fresh)));
    live.state.type=84;live.countdown_armed=live.countdown_fired=1;
    CHECK(!rf_event_checkpoint_encode(identity,&live,20,wire,192));fresh=live;fresh.countdown_armed=fresh.countdown_fired=0;
    CHECK(!rf_event_checkpoint_restore(wire,192,identity,&fresh,1000)&&fresh.countdown_armed&&fresh.countdown_fired);
    put(wire+176,2);checksum(wire);before=fresh;
    CHECK(rf_event_checkpoint_restore(wire,192,identity,&fresh,1000)==RF_FORMAT&&!memcmp(&fresh,&before,sizeof(fresh)));
    live.countdown_armed=live.countdown_fired=0;
    {
        static const uint32_t types[]={0,1,2,3,5,6,7,8,10,11,12,13,14,15,17,18,19,22,24,28,30,34,35,36,37,38,39,41,42,43,44,46,48,51,52,56,59,61,64,65,69,71,73,74,75,81,84};
    ...
    CHECK(rf_event_checkpoint_external_requirements(60)==RF_EVENT_CHECKPOINT_EXTERNAL_UNIMPLEMENTED);
