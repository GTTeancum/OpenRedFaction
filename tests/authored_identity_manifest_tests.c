    ...
    const rf_lightmap_rgb_owner *rgb,const unsigned char known94[32])
{
    ...
    for(n=0;n<15;n++) {
        ...
        rf_geomod_publication_digest_input publication={0};uint32_t face_charts[140];
        if(uids[n]==66)CHECK(!rf_geomod_authored_cavity_open(level,geometry,2*1024*1024,&owner));
        else CHECK(!rf_geomod_authored_post_open_source(level,geometry,uids[n],2*1024*1024,&owner));
        CHECK(!rf_geomod_authored_post_get(owner,&asset));
        manifest.materials=materials;manifest.material_capacity=8;
        manifest.references=refs;manifest.reference_capacity=160;manifest.substrate=&substrate;
        {int status=rf_geomod_authored_identity_capture_manifest(level,geometry,&asset,maps,6,rgb,
            2*1024*1024,digests[n],&peak,&manifest);printf("MANIFEST_CAPTURE uid%u status%d\n",uids[n],status);CHECK(!status);}
        CHECK(peak<=2*1024*1024 && manifest.material_count>=1 && manifest.material_count<=8 && manifest.reference_count<=160);
        ...
        CHECK(asset.windows.face_count==(uids[n]==66?134:(rf_geomod_authored_beam_roof(uids[n])!=0)?8:4));
        ...
        if(rf_geomod_authored_beam_roof(uids[n])) {
            rf_geomod_publication_solid wrong=*asset.neighbor_voids;rf_geomod_authored_post_view invalid=asset;
            wrong.owner++;invalid.neighbor_voids=&wrong;memset(repeat,0xa5,32);peak=123;
