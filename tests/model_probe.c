...
int main(int argc,char **argv)
{
    ...
    if(argc==2 && !strcmp(argv[1],"--static-tag-place")) {
        ...
        while(fread(data,sizeof(data),1,stdin)==1) {
            memcpy(tag.rotation,data,16);memcpy(tag.position,data+4,12);
            memset(&out,0xa5,sizeof(out));out.status=rf_static_model_tag_place(&owner,0,data+7,data+16,out.pose);
    ...
    if(argc==2 && !strcmp(argv[1],"--skeletal-register"))return skeletal_register_probe();
    if(argc==2 && !strcmp(argv[1],"--world-facing")) {
        float in[7];uint32_t out;
        while(fread(in,sizeof(in),1,stdin)==1) {
            out=rf_preview_plane_visible(in,in+4);
            if(fwrite(&out,4,1,stdout)!=1)return 2;
        }
        return ferror(stdin)?1:0;
    }
    ...
    if(argc==2 && (!strcmp(argv[1],"--project-vertex") || !strcmp(argv[1],"--project-static-vertex"))) {
        ...
        while(fread(&data,sizeof(data),1,stdin)==1) {
            ...
            if((!strcmp(argv[1],"--project-static-vertex")?rf_model_project_static_vertex:rf_model_project_vertex)(data.world,&data.view,&cache,clip,vertex,&visible))return 2;
