...
int main(void)
{
    ...
    int32_t ids[11];uint32_t i,supported=0;
    ...
    for(i=0;i<11;i++){ids[i]=rf_weapon_name_find(&campaign_weapon_supply.names,campaign_weapon_names[i]);CHECK(ids[i]>=0 && ids[i]<64);}
    for(i=0;i<64;i++)supported+=catalog.supported[i]!=0;
    CHECK(supported==10 && !catalog.supported[ids[9]]);
    for(i=0;i<11;i++)if(i!=9)CHECK(catalog.supported[ids[i]]);
    CHECK(catalog.weapons[ids[10]].magazine==100);
    ...
    player.player.inventory.owned[ids[10]]=1;player.player.inventory.loaded[ids[10]]=50;
    ...
    CHECK(rf_player_checkpoint_encode(&bad,&catalog,data,sizeof(data))==RF_FORMAT);
    puts("Player checkpoint catalog: ten base weapons, clipless reserves and remote selection round trip");return 0;
}
