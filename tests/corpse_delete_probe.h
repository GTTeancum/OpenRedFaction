typedef struct corpse_delete_probe_context {
    ...
    rf_object_registry registry;rf_corpse_pool pool;uint32_t dummy[7],corpse_count,object_count,found,item_flags;
...
static uint32_t *cd_sound(void *ctx,int32_t id)
{corpse_delete_probe_context *c=ctx;cd_trace(c,8,(uint32_t)id);return c->found?&c->item_flags:NULL;}
static int corpse_delete_probe(void)
{
    ...
    while(fread(input,sizeof(input),1,stdin)==1) {
        ...
        c.update.fade.object_flags_7c=input[0];c.update.model=input[1];c.state.burn=input[2];c.update.item_2cc=(int32_t)input[3];c.found=input[4];
        c.state.update=&c.update;c.state.registered_object=&c.state;c.corpse_count=13;c.object_count=79;c.item_flags=0x500;
        ...
        memset(out,0,sizeof(out));out[0]=(uint32_t)status;out[1]=(uint32_t)c.update.item_2cc;out[2]=c.item_flags;out[3]=c.state.burn;
