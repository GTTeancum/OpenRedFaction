...
int main(int argc,char **argv)
{
    ...
    static const char *weapons[]={"12mm handgun","Assault Rifle","Riot Stick","Shotgun","Rocket Launcher","Grenade","Sniper Rifle","rail_gun","Remote Charge"};
    static const unsigned char gives[]={1,1,1,1,1,1,0,0,0};
    CHECK(pickup_class("First Aid Kit")==10 && pickup_class("SHOTGUN")==8);
    CHECK(pickup_class("not a pickup")==-1 && pickup_class(NULL)==-1);
    CHECK(scene_pickup_weapon_slot(-1)==-1 && scene_pickup_weapon_slot(20)==-1);
    ...
    rf_vpp_close(&tables);puts("All nine added pickup classes match installed table static models and weapon/ammo effects");return 0;
}
