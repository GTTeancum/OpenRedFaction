...
static int demand_test(rf_vpp *tables,const rf_weapon_supply_catalog *supply)
{
    ...
    const char *slots[]={"Undercover 12mm handgun","heavy_machine_gun","Machine Pistol Special","scope_assault_rifle","Machine Pistol","shoulder_cannon","Sniper Rifle","rail_gun"};
    ...
    CHECK(!scene_extra_pickup_demand_slots(slots,8,&optional,&pair));
    CHECK(optional==255 && pair==20);
    pickups.items=&item;pickups.count=1;
    strcpy(item.class_name,"heavy machine gun");
    CHECK(!scene_weapon_resources_plan(0,optional,slots,8,&supply->names,&inventory,&pickups,NULL,resolve_item,tables,&demand));
    CHECK(demand.pickup_mask==2 && demand.mask==2);
    CHECK(!scene_extra_pickup_demand_expand(slots,8,demand.mask,&expanded) && expanded==2);
    strcpy(item.class_name,"scope assault rifle");
    CHECK(!scene_weapon_resources_plan(0,optional,slots,8,&supply->names,&inventory,&pickups,NULL,resolve_item,tables,&demand));
    CHECK(!scene_extra_pickup_demand_expand(slots,8,demand.mask,&expanded) && expanded==8);
    strcpy(item.class_name,"Machine Pistol");
    CHECK(!scene_weapon_resources_plan(0,optional,slots,8,&supply->names,&inventory,&pickups,NULL,resolve_item,tables,&demand));
    CHECK(demand.mask==16);
    CHECK(!scene_extra_pickup_demand_expand(slots,8,demand.mask,&expanded) && expanded==20);
    ...
    CHECK(!scene_weapon_resources_plan(0,optional,slots,8,&supply->names,&inventory,NULL,NULL,NULL,NULL,&demand));
    ...
    {rf_level_owned_event event={0};rf_level_owned_events events={0};
     ...
     CHECK(!scene_weapon_resources_plan(0,optional,slots,8,&supply->names,&inventory,NULL,&events,resolve_item,tables,&demand));
    ...
    CHECK(!scene_extra_pickup_demand_expand(slots,8,demand.mask,&expanded) && expanded==1);
    memset(&inventory,0,sizeof(inventory));strcpy(item.class_name,"shoulder cannon");
    CHECK(!scene_weapon_resources_plan(0,optional,slots,8,&supply->names,&inventory,&pickups,NULL,resolve_item,tables,&demand));
    CHECK(demand.pickup_mask==32 && demand.mask==32);
    CHECK(!scene_extra_pickup_demand_expand(slots,8,demand.mask,&expanded) && expanded==32);
    ...
    {const char *classes[2]={"Sniper Rifle","rail gun"};
     const char *weapons[2]={"Sniper Rifle","rail_gun"};uint32_t i;
     for(i=0;i<2;i++) {
        ...
        CHECK(!scene_extra_pickup_demand_expand(slots,8,demand.mask,&expanded) && expanded==expected);
