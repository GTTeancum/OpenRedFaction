...
static int demand_test(rf_vpp *tables,const rf_weapon_supply_catalog *supply)
{
    ...
    const char *slots[]={"Undercover 12mm handgun","heavy_machine_gun","Machine Pistol Special","scope_assault_rifle","Machine Pistol"};
    ...
    CHECK(!scene_extra_pickup_demand_expand(slots,5,demand.mask,&expanded) && expanded==2);
    strcpy(item.class_name,"scope assault rifle");
    CHECK(!scene_weapon_resources_plan(0,optional,slots,5,&supply->names,&inventory,&pickups,NULL,resolve_item,tables,&demand));
    CHECK(!scene_extra_pickup_demand_expand(slots,5,demand.mask,&expanded) && expanded==8);
    strcpy(item.class_name,"Machine Pistol");
    CHECK(!scene_weapon_resources_plan(0,optional,slots,5,&supply->names,&inventory,&pickups,NULL,resolve_item,tables,&demand));
    CHECK(demand.mask==16);
    CHECK(!scene_extra_pickup_demand_expand(slots,5,demand.mask,&expanded) && expanded==20);
...
{
    const int32_t counts[5]={32,99,20,99,20};rf_vpp tables={0};rf_weapon_supply_catalog supply;
    ...
    for(i=0;i<SCENE_EXTRA_PICKUP_COUNT;i++){
        ...
        CHECK(item.count==counts[i] && item.mesh_kind==1 && item.gives_weapon==(uint32_t)(i<3));
        ...
        CHECK(grant.rounds==(uint32_t)item.count && grant.acquired==(uint32_t)(i<3));
        CHECK(inventory.owned[weapon]==(uint32_t)(i<3));
