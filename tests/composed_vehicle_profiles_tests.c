...
int main(void)
{
    ...
    for(kind=1;kind<=3;kind++)for(occupied=0;occupied<=1;occupied++){
        driller.player_occupied=apc.vehicle.player_occupied=jeep.vehicle.player_occupied=occupied;
        ...
        else {CHECK(!rf_jeep_checkpoint_encode(&jeep,vc,bytes));record=&jeep;}
    ...
    CHECK(!rf_composed_checkpoint_preflight_v3(data,n,1,&catalog,0,0,&out) && !out.vehicle_profile);
    puts("RFCP3 real Driller/APC/Jeep profiles: parked/seated,5000HP,role/reference retention, crossed rejection and legacy compatibility passed");return 0;
}
