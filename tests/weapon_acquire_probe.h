...
static int weapon_acquire_probe(int sp)
{wa_fixture f;int32_t status;
 while(fread(&f,472,1,stdin)==1){f.calls=0;status=sp?rf_weapon_acquire_sp(&f.inventory,&f.definition,f.weapon,f.quantity):rf_weapon_acquire(&f.inventory,&f.definition,f.weapon,f.quantity,wa_notify,&f);if(fwrite(&status,4,1,stdout)!=1 || fwrite(&f.inventory,448,1,stdout)!=1 || fwrite(&f.calls,4,1,stdout)!=1)return 2;}return ferror(stdin)?1:0;}
