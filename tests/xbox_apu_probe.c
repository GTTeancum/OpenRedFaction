...
int main(void)
{
    ...
    rf_apu_probe[1]=17;
    {rf_wave_pcm short_sample={(const uint8_t *)calibration_pcm,128,64,48000,1,16};
     if(rf_xbox_audio_release_idle_sample(NULL)!=RF_RANGE || rf_xbox_audio_release_idle_sample(short_sample.samples) || rf_xbox_audio_open())goto fail;
     if(rf_xbox_audio_events.play_mode(NULL,0xa0001u,&short_sample,0,0,0) ||
     ...
     Sleep(150);
     if(rf_xbox_audio_release_idle_sample(short_sample.samples) || rf_xbox_audio_release_idle_sample(short_sample.samples) ||
        rf_xbox_audio_release_voice(0xa0003u)!=RF_NOT_FOUND)goto fail;
     Sleep(100);
     {const volatile int16_t *output=g_hw_ac97_buffer;
