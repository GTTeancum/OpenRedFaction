...
{
    collision_texture_fixture *f=context;(void)face;if((uint32_t)bitmap!=f->input[0])return RF_RANGE;
    ++f->calls;memcpy(f->point,point,12);if(f->input[2])return RF_IO;*color=f->input[1];return RF_OK;
}
...
int main(int argc,char **argv)
{
    ...
    if(argc==2 && !strcmp(argv[1],"--clutter-skin"))return clutter_skin_probe();
    ...
    if(argc==2 && (!strcmp(argv[1],"--ai-visible-solid") || !strcmp(argv[1],"--flat-query") || !strcmp(argv[1],"--flat-query-textured") || (!strcmp(argv[1],"--preferred-flat-query") || !strcmp(argv[1],"--preferred-flat-query-textured")))) {
        ...
        while(fread(&in,sizeof(in),1,stdin)==1) {
            ...
            if(!strcmp(argv[1],"--ai-visible-solid")) {
                rf_collision_solid_view solid={0};rf_entity_navigation_candidate node={0};uint32_t answer[2]={0,99};
                solid.flat_faces=faces;solid.flat_count=in.count;memcpy(node.query_point,in.start,12);
                answer[0]=in.count>4?RF_RANGE:rf_entity_navigation_visible_solid(&solid,&node,in.delta,in.radius,in.limit,answer+1);
            ...
            if(preferred_mode) {
                ...
                struct {int32_t status;uint32_t matched;rf_collision_sweep_room_hit hit;} answer;
    ...
    if(argc==4 && !strcmp(argv[1],"--world")) {
        ...
        for(i=0;i<world.room_count;i++) {
            ...
            if(!world.contains_liquid || world.contains_liquid[i]!=geometry.data[geometry.room_offsets[i]+32])return 15;
    ...
    if((argc==4 && !strcmp(argv[1],"--level-texture-uv")) || (argc>=5 && argc<=13 && (!strcmp(argv[1],"--level-material-samples") || !strcmp(argv[1],"--level-indexed-material-samples")))) {
        ...
        if(real_materials) {
            const rf_geometry *g=&geometry;
            for(i=4;i<(uint32_t)argc;++i)if(rf_vpp_open(maps+i-4,argv[i]))return 5;
            if(rf_geometry_materials_open(&bundle,&g,1,maps,(uint32_t)argc-4,32*1024*1024))return 5;
            for(i=4;i<(uint32_t)argc;++i)rf_vpp_close(maps+i-4);
            fwrite(&geometry.textures,4,1,stdout);fwrite(bundle.slots,4,geometry.textures,stdout);fwrite(&bundle.textures.count,4,1,stdout);
            for(i=0;i<bundle.textures.count;++i) {
                const rf_material *m=bundle.textures.items+i;fwrite(&m->status,4,1,stdout);fwrite(&m->image.width,16,1,stdout);
    ...
    if(argc==2 && (!strcmp(argv[1],"--thin") || !strcmp(argv[1],"--thin-textured"))) {
        ...
        while(fread(&in,sizeof(in),1,stdin)==1) {
            ...
            if(textured) {
                collision_texture_fixture fixture;rf_collision_texture_backend backend={collision_texture_sample,&fixture};
                if(fread(fixture.input,4,4,stdin)!=4)return 2;fixture.calls=0;memset(fixture.point,0xa5,12);
                out.status=in.count>8?RF_RANGE:rf_collision_thin_face_textured(&face,(int32_t)fixture.input[0],in.start,in.delta,in.limit,
                    fixture.input[3]?&backend:NULL,&out.hit,&out.matched);
                if(fwrite(&out,sizeof(out),1,stdout)!=1 || fwrite(&fixture.calls,4,1,stdout)!=1 || fwrite(fixture.point,12,1,stdout)!=1)return 2;
                continue;
            }
