...
int main(void)
{
    ...
    combat_feedback feedback={0};float delta[3]={-3,0,0},amount=0;uint32_t i;
    ...
    CHECK(!campaign_enemy_shotgun_fire(&scene,&owner,NULL,UINT32_MAX,&definition,delta,10,0,&effects,&feedback,&amount));
    CHECK(amount==40 && campaign_player_damage.state.effects.health==60);
    ...
    CHECK(!campaign_enemy_shotgun_fire(&scene,&owner,NULL,UINT32_MAX,&definition,delta,10,0,&effects,&feedback,&amount));
    CHECK(owner.inventory.loaded[3]==7 && amount==40);
