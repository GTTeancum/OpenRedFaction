...
int main(int argc,char **argv)
{
    ...
    if(argc==2 && !strcmp(argv[1],"--lightmap-shadow-occluder-image")) {
        struct {rf_lightmap_shadow_cull view;rf_lightmap_shadow_face face;uint32_t present,format;} input;
        rf_image image={0};uint32_t status,accepted;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            accepted=0xa5a5a5a5;image.source_format=input.format;
            status=rf_lightmap_shadow_occluder_image(&input.view,&input.face,input.present?&image:NULL,&accepted);
            fwrite(&status,4,1,stdout);fwrite(&accepted,4,1,stdout);
        }
        return 0;
    }
    if(argc==2 && !strcmp(argv[1],"--lightmap-shadow-occluder")) {
        struct {rf_lightmap_shadow_cull view;rf_lightmap_shadow_face face;} input;uint32_t status,accepted;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            accepted=0xa5a5a5a5;status=rf_lightmap_shadow_occluder(&input.view,&input.face,&accepted);
            fwrite(&status,4,1,stdout);fwrite(&accepted,4,1,stdout);
        }
        return 0;
    }
    if(argc==2 && (!strcmp(argv[1],"--lightmap-shadow-filter") || !strcmp(argv[1],"--lightmap-shadow-filter-warm"))) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            if(!strcmp(argv[1],"--lightmap-shadow-filter-warm")) {if(fread(intersection,sizeof(intersection),1,stdin)!=1)return 2;}
            else memset(intersection,0,sizeof(intersection));
            ...
            if(!strcmp(argv[1],"--lightmap-shadow-filter-warm"))fwrite(intersection,sizeof(intersection),1,stdout);
        }
        return 0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--lightmap-shadow-source")) {
        struct {rf_lightmap_shadow_source source;uint32_t local;} in;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&in,sizeof(in),1,stdin)==1) {
            rf_lightmap_shadow_samples samples;uint32_t status;memset(&samples,0xa5,sizeof(samples));
            status=rf_lightmap_shadow_source_samples(&in.source,in.local,&samples);
            fwrite(&status,4,1,stdout);fwrite(&samples,sizeof(samples),1,stdout);
        }
        return 0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--lightmap-shadow-area")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            memset(&area,0xa5,sizeof(area));status=input.count>32?RF_RANGE:rf_lightmap_shadow_area(input.vertices,input.count,&area);
            fwrite(&status,4,1,stdout);fwrite(&area,4,1,stdout);
        }
        return 0;
    }
    if(argc==2 && !strcmp(argv[1],"--lightmap-shadow-mapping")) {
        struct {rf_lightmap_sample_plane view;uint32_t width,height;float origin[3];} input;rf_lightmap_shadow_mapping output;uint32_t status;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            memset(&output,0xa5,sizeof(output));status=rf_lightmap_shadow_mapping_prepare(&input.view,input.width,input.height,input.origin,&output);
            fwrite(&status,4,1,stdout);fwrite(&output,sizeof(output),1,stdout);
        }
        return 0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--lightmap-shadow-ray")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            memset(output,0xa5,sizeof(output));hit=0xa5a5a5a5;
            status=rf_lightmap_shadow_ray(input.start,input.direction,input.plane,output,&hit);
            fwrite(&status,4,1,stdout);fwrite(&hit,4,1,stdout);fwrite(output,sizeof(output),1,stdout);
        }
        return 0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--lightmap-raster-shadow")) {
        ...
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            status=input.count>16 || input.bytes>1024?RF_RANGE:rf_lightmap_raster_shadow(input.vertices,input.count,input.mask,input.bytes,input.width,input.height,(unsigned char)input.amount);
            fwrite(&status,4,1,stdout);fwrite(input.mask,sizeof(input.mask),1,stdout);
        }
        return 0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--light-source-volume")) {
        struct {rf_vfx_light_source source;float angle,minimum[3],maximum[3];} input;
        struct {uint32_t status;rf_light_visibility_volume volume;uint32_t hit;} output;
        _setmode(_fileno(stdin),_O_BINARY);_setmode(_fileno(stdout),_O_BINARY);
        while(fread(&input,sizeof(input),1,stdin)==1) {
            memset(&output,0xa5,sizeof(output));output.status=rf_visibility_light_source_volume(&input.source,input.angle,&output.volume);
            if(!output.status)output.status=rf_visibility_light_bounds(&output.volume,input.minimum,input.maximum,&output.hit);
            fwrite(&output,sizeof(output),1,stdout);
        }return 0;
    }
    ...
    if(argc==2 && !strcmp(argv[1],"--particle-step-random")) {
        struct {float dt;rf_random_state random;rf_particle particle;} in;
        struct {int32_t status;uint32_t live;rf_random_state random;rf_particle particle;} out;
        ...
        while(fread(&in,sizeof(in),1,stdin)==1) {
            ...
            out.random=in.random;out.status=rf_particle_pool_step_random(&pool,0,in.dt,NULL,NULL,NULL,NULL,&out.random);out.live=pool.live[0];out.particle=records[0];
            if(fwrite(&out,sizeof(out),1,stdout)!=1)return 1;
        }
        return ferror(stdin)?1:0;
    }
