...
static int scene_emission(void)
{
    ...
    CHECK(!rf_campaign_actor_register(&rf_scene_defeated_actors,"test.rfl",123,&owner.persistence_slot));
    ...
    CHECK(rf_scene_defeated_actors.drops[owner.persistence_slot].state==0);
    owner.inventory.loaded[6]=2;owner.inventory.reserve[6]=1;before=owner.inventory;
    CHECK(!campaign_weapon_drop_emit(&owner));
    saved=rf_scene_defeated_actors.drops[owner.persistence_slot];
    CHECK(saved.state==1 && saved.weapon==6 && saved.quantity==3);
    ...
    CHECK(campaign_player_inventory.owned[6] && campaign_player_inventory.loaded[6]==3);
    CHECK(campaign_player_inventory.reserve[6]==0);
    CHECK(rf_scene_defeated_actors.drops[owner.persistence_slot].state==2);
    CHECK(rf_scene_weapon_drops[1]==1 && rf_scene_weapon_drops[2]==3);
...
int main(void)
{
    ...
    CHECK(!scene_emission());
    puts("NPC drops respect remaining ammo, one-magazine cap and precision ownership");return 0;
}
