/* Actual chassis resource, material transfer and scene software submission. */
#include <stdio.h>
#include <math.h>
#include "../src/diagnostic/scene.c"
#include "../src/diagnostic/scene_driller_resources.inc"
#include "../src/diagnostic/scene_driller_draw.inc"
#define CHECK(x) do{if(!(x)){fprintf(stderr,"driller draw line%d: %s\n",__LINE__,#x);return 1;}}while(0)
int main(void)
{
 static scene_stream stream;rf_preview_mesh mesh={0};rf_materials materials={0};
 rf_vpp meshes={0},maps[4]={{0}};scene_driller_resources *owner=NULL;
 ...
 CHECK(!scene_driller_resources_open("Installed_Game/tables.vpp",&meshes,maps,4,4*1024*1024,&owner));
 {float lo[3]={1e30f,1e30f,1e30f},hi[3]={-1e30f,-1e30f,-1e30f};uint32_t part,v,axis;
  ...
  printf("DRILLER_AABB min=%g,%g,%g max=%g,%g,%g bound=%g,%g,%g,%g\n",lo[0],lo[1],lo[2],hi[0],hi[1],hi[2],owner->render.bound[0],owner->render.bound[1],owner->render.bound[2],owner->render.bound[3]);
 }
 CHECK(!scene_driller_materials_merge(owner,&materials,&base,&textures));
 ...
 CHECK(!scene_driller_draw(&stream,owner,position,basis,base,textures,&submissions,&vertices));
 ...
 printf("DRILLER_DRAW submissions=%u vertices=%u textures=%u\n",submissions,vertices,textures);
 scene_driller_resources_close(&owner);rf_materials_close(&materials);free(mesh.vertices);
