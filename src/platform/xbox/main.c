...
static int scene_preview(rf_level *level,rf_preview_mesh *mesh)
{
    ...
    stream_flag=fopen("D:\\dev-npc.flag","rb");rf_scene_dev_npc_enabled=stream_flag!=NULL;if(stream_flag){if(fgetc(stream_flag)=='2')rf_scene_dev_npc_enabled=2;fclose(stream_flag);}
    stream_flag=fopen("D:\\fragment-platform-test.flag","rb");rf_scene_fragment_platform_enabled=stream_flag!=NULL;if(stream_flag){int mode=fgetc(stream_flag);if(mode=='2' || mode=='3')rf_scene_fragment_platform_enabled=(uint32_t)(mode-'0');fclose(stream_flag);}
