...
static int player_poll_paced(void *context,uint32_t frame,rf_scene_input *input)
{
    if(campaign_setup_uid && !campaign_total_frames) {
        int status=rf_scene_fire_setup_event(campaign_setup_uid,0);if(status)return status;
    }
...
static int scene_preview(rf_level *level,rf_preview_mesh *mesh)
{
    ...
    stream_flag=fopen("D:\\dev-npc.flag","rb");rf_scene_dev_npc_enabled=stream_flag!=NULL;if(stream_flag){int mode=fgetc(stream_flag);if(mode=='2' || mode=='3' || mode=='4')rf_scene_dev_npc_enabled=(uint32_t)(mode-'0');fclose(stream_flag);}
    stream_flag=fopen("D:\\fragment-platform-test.flag","rb");rf_scene_fragment_platform_enabled=stream_flag!=NULL;if(stream_flag){int mode=fgetc(stream_flag);if(mode=='2' || mode=='3' || mode=='4' || mode=='4')rf_scene_fragment_platform_enabled=(uint32_t)(mode-'0');fclose(stream_flag);}
...
int main(void)
{
    ...
    if (result == RF_OK) {
        ...
        if (result == RF_OK) {
            ...
            FILE *lift_flag=fopen("D:\\campaign-lift.flag","rb");
            int staged_lift=lift_flag!=NULL;if(lift_flag)fclose(lift_flag);
            ...
            if(result==RF_OK) {
                FILE *exit_start=fopen("D:\\campaign-exit-start.bin","rb");
