...
static int scene_preview(rf_level *level,rf_preview_mesh *mesh)
{
    ...
    stream_flag=fopen("D:\\vehicle-test.flag","rb");rf_scene_vehicle_enabled=stream_flag!=NULL;if(stream_flag){if(fgetc(stream_flag)=='2')rf_scene_vehicle_enabled=2;fclose(stream_flag);}
