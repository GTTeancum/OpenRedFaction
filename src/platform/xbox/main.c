...
static int scene_preview(rf_level *level,rf_preview_mesh *mesh)
{
    ...
    stream_flag=fopen("D:\\dev-npc.flag","rb");rf_scene_dev_npc_enabled=stream_flag!=NULL;if(stream_flag){int mode=fgetc(stream_flag);if(mode=='2' || mode=='3' || mode=='4' || mode=='5')rf_scene_dev_npc_enabled=(uint32_t)(mode-'0');fclose(stream_flag);}
