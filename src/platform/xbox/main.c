...
static int player_poll_paced(void *context,uint32_t frame,rf_scene_input *input)
{
    ...
    if(campaign_goto_uid && campaign_total_frames==(campaign_setup_uid?300:30)) {
...
static int scene_preview(rf_level *level,rf_preview_mesh *mesh)
{
    ...
    if(!campaign_total_frames) {
        ...
        if(exit_file){int invalid=fread(&campaign_setup_uid,4,1,exit_file)!=1 || fgetc(exit_file)!=EOF;fclose(exit_file);if(invalid)return RF_FORMAT;}
