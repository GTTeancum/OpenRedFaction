...
extern uint32_t rf_scene_npc_unholster_test[5],rf_scene_npc_ai_reset_test[5],rf_scene_npc_unholster[4];
...
#define CHECK(call) do {status=(call);if(status){fprintf(stderr,"%s failed (%d)\n",#call,status);goto cleanup;}} while(0)
    ...
    if(spawn_profile && !strcmp(level.entry.name,"ctf06.rfl") &&
       (dev_room || (p.headless && getenv("RF_REPLAY_DEV_ROOM")))) {
        ...
        if(source) {
            ...
            else if(!strcmp(source,"75"))uid=75;
            else if(!strcmp(source,"79"))uid=79;
            else if(!strcmp(source,"99"))uid=99;
            else if(!strcmp(source,"103"))uid=103;
    ...
    if(p.headless && getenv("RF_REPLAY_FRAGMENT_CONTACT_TEST")) {
        ...
        printf("FRAGMENT_MOVING_AUDIT");for(word=0;word<16;word++)printf(" %u",rf_scene_fragment_moving_audit[word]);puts("");
    ...
    if(p.headless && getenv("RF_REPLAY_SETUP_UID")) {
        char *end;unsigned long value=strtoul(getenv("RF_REPLAY_SETUP_UID"),&end,10);if(!value || (*end && *end!=','))CHECK(RF_FORMAT);
        p.setup_uid=(uint32_t)value; if(*end==',') {value=strtoul(end+1,&end,10);if(*end || !value)CHECK(RF_FORMAT);p.setup_next_uid=(uint32_t)value;}
    }
...
run_scene:
    ...
    if(p.headless) {
        ...
        if(spawn_profile){
            ...
            printf("NPC_CONTACT_SOUND_AUDIO");for(i=0;i<12;++i)printf(" %u",rf_scene_npc_contact_sound_audio[i]);printf("\n");
            ...
            printf("WEAPON_MUZZLE");for(i=0;i<4;++i)printf(" %u",rf_scene_weapon_muzzle[i]);printf("\n");
            printf("WEAPON_AIM");for(i=0;i<4;++i)printf(" %u",rf_scene_weapon_aim[i]);printf("\n");
            ...
            printf("PLAYER_CONTACT_AUDIO");for(i=0;i<12;++i)printf(" %u",rf_scene_player_contact_audio[i]);printf("\n");
            printf("PLAYER_CONTACT_TEST");for(i=0;i<4;++i)printf(" %u",rf_scene_player_contact_test[i]);printf("\n");
            printf("CLUTTER_CONTACT_TEST");for(i=0;i<8;++i)printf(" %u",rf_scene_clutter_contact_test[i]);printf("\n");
            ...
            printf("NPC_UNHOLSTER_TEST %u %u %u %u %u\n",rf_scene_npc_unholster_test[0],rf_scene_npc_unholster_test[1],rf_scene_npc_unholster_test[2],rf_scene_npc_unholster_test[3],rf_scene_npc_unholster_test[4]);
            ...
            printf("USE_REACH");for(i=0;i<4;i++)printf(" %u",rf_scene_use_reach[i]);puts("");
        ...
        printf("TRIGGER_HISTORY %u %u %u %u\n",rf_scene_trigger_history[0],rf_scene_trigger_history[1],rf_scene_trigger_history[2],rf_scene_trigger_history[3]);
        printf("STARTUP_INVENTORY %u %u %u %u\n",rf_scene_startup_inventory[0],rf_scene_startup_inventory[1],rf_scene_startup_inventory[2],rf_scene_startup_inventory[3]);
        printf("SCRIPT_GRANTS");for(i=0;i<8;i++)printf(" %u",rf_scene_script_grants[i]);puts("");
    ...
    printf("BURNING");for(i=0;i<5;++i)printf(" %u",rf_scene_burning[i]);puts("");
    printf("FLAME_CANISTER");for(i=0;i<5;++i)printf(" %u",rf_scene_flame_canister[i]);puts("");
    ...
    if(rf_scene_fragment_platform_enabled){printf("FRAGMENT_PLATFORM");for(uint32_t k=0;k<32;k++)printf(" %u",rf_scene_fragment_platform_audit[k]);printf("\n");}
    ...
    {extern uint32_t rf_scene_debris_visibility[8];
     printf("DEBRIS_VISIBILITY");for(i=0;i<8;++i)printf(" %u",rf_scene_debris_visibility[i]);puts("");}
    {extern uint32_t rf_scene_debris_wet[8];
     printf("DEBRIS_WET_STATE");for(i=0;i<8;++i)printf(" %u",rf_scene_debris_wet[i]);puts("");}
