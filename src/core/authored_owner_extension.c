...
static int validate(const rf_authored_owner_extension *v,const rf_authored_owner_expected *e,uint32_t cuts,uint32_t collection)
{
...
static int extension_encode(const rf_authored_owner_extension *v,
    const rf_authored_owner_expected *e,uint32_t cuts,uint32_t collection,void *out,uint32_t bytes)
{
    unsigned char p[RF_AUTHORED_OWNER_EXTENSION_BYTES];int status;
    if(!out || bytes!=sizeof(p))return RF_RANGE;
    status=validate(v,e,cuts,collection);if(status)return status;
...
static int extension_decode(const void *data,uint32_t bytes,
    const rf_authored_owner_expected *e,uint32_t cuts,uint32_t collection,rf_authored_owner_extension *out)
{
    ...
    status=validate(&v,e,cuts,collection);if(status)return status;*out=v;return RF_OK;
}

int rf_authored_owner_extension_encode(const rf_authored_owner_extension *v,const rf_authored_owner_expected *e,uint32_t cuts,void *out,uint32_t bytes)
{return extension_encode(v,e,cuts,0,out,bytes);}
int rf_authored_owner_extension_decode(const void *data,uint32_t bytes,const rf_authored_owner_expected *e,uint32_t cuts,rf_authored_owner_extension *out)
{return extension_decode(data,bytes,e,cuts,0,out);}
int rf_authored_owner_collection_encode(const rf_authored_owner_extension *v,const rf_authored_owner_expected *e,uint32_t count,uint32_t cuts,void *out,uint32_t bytes)
{if(count<2 || count>4)return RF_RANGE;return extension_encode(v,e,cuts,count,out,bytes);}
int rf_authored_owner_collection_decode(const void *data,uint32_t bytes,const rf_authored_owner_expected *e,uint32_t count,uint32_t cuts,rf_authored_owner_extension *out)
{if(count<2 || count>4)return RF_RANGE;return extension_decode(data,bytes,e,cuts,count,out);}
