...
static int layout_size(uint32_t core,uint32_t admissions,
    uint32_t maps,uint32_t faces,uint32_t pieces,uint32_t collection,rf_authored_checkpoint_layout *out)
{
    ...
    if(core<(collection?92u:28u) || core>(collection?RF_COMPOSED_CHECKPOINT_RFDS_MAX:RF_GEOMOD_HISTORY_MAX_BYTES) || admissions>128 || maps>RF_GEOMOD_LIGHTMAP_LIMIT || faces>RF_GEOMOD_PUBLICATION_FACES)return RF_FORMAT;
...
{return layout_size(core,admissions,maps,faces,pieces,0,out);}
int rf_authored_collection_layout_size(uint32_t core,uint32_t admissions,
    uint32_t maps,uint32_t faces,rf_authored_checkpoint_layout *out)
{return layout_size(core,admissions,maps,faces,0,1,out);}
...
static int layout_read(const void *data,uint32_t bytes,
    uint32_t collection,rf_authored_checkpoint_layout *out)
{
    ...
    if(memcmp(p,"RFDS",4) || word(p+4)!=(collection?3u:2u) || word(p+8)!=bytes ||
    ...
    if(collection && word(p+12))return RF_FORMAT;
    status=layout_size(word(p+252),word(p+240),word(p+248),word(p+272),word(p+12),collection,&v);
    if(status)return status;if(v.bytes!=bytes)return RF_FORMAT;
    if(collection) {
        rf_authored_sources_layout sources;
        status=rf_authored_sources_read(p+v.core_offset,v.core_bytes,&sources);if(status)return status;
    }
    if(v.piece_bytes) {
...
int rf_authored_checkpoint_layout_read(const void *data,uint32_t bytes,rf_authored_checkpoint_layout *out)
{return layout_read(data,bytes,0,out);}
int rf_authored_collection_layout_read(const void *data,uint32_t bytes,rf_authored_checkpoint_layout *out)
{return layout_read(data,bytes,1,out);}
