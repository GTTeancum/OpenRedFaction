...
{
    switch(type){
    case 1:case 2:case 3:case 5:case 6:case 11:case 12:case 13:case 14:case 15:
    ...
    case 51:case 52:case 56:case 64:case 65:case 81:case 87:case 88:return 1;
    default:return 0;
    }
}
static int owner_valid(const rf_runtime_event *e,int32_t now)
{
    ...
    if(!rf_event_checkpoint_type_supported(e->state.type)||e->state.deadline>=0)return RF_NOT_FOUND;
    if(e->object_kind!=6||e->authored->record.uid==UINT32_MAX||e->retired>1)return RF_FORMAT;
    if(e->retired&&(!(e->state.flags&1)||e->death_fired!=1))return RF_FORMAT;
    if(e->state.type==50&&(e->unhide.on||e->unhide.off))return RF_NOT_FOUND;
    ...
    const rf_event_checkpoint_refs *refs,void *output,uint32_t capacity)
{
    ...
    int32_t cycle_remaining=-1,unhide_remaining=-1;int status;
    ...
    memset(p,0,192);memcpy(p,"RFEC",4);put(p+4,2);put(p+8,192);put(p+16,e->authored->record.uid);put(p+20,e->state.type);memcpy(p+24,identity,32);
    ...
    if(e->state.type==50)put(p+156,(uint32_t)unhide_remaining);
    put(p+12,hash(p));return RF_OK;
}
typedef struct event_checkpoint_resolved {uint32_t source,actor;int32_t cycle_deadline,unhide_deadline;} event_checkpoint_resolved;
...
{
    const unsigned char *p=data;uint32_t type,i;int32_t left;int status;
    if(!data||!identity)return RF_RANGE;
    status=owner_valid(e,now);if(status)return status;
    if(bytes!=192||memcmp(p,"RFEC",4)||word(p+4)!=2||word(p+8)!=192||word(p+12)!=hash(p)||
    ...
    for(i=160;i<192;i+=4)if(word(p+i))return RF_FORMAT;
    if(word(p+72)>1||word(p+76)>(uint32_t)RF_TIMER_PERIOD||word(p+112)>1||
       (word(p+112)&&(!(word(p+64)&1)||word(p+72)!=1)))return RF_FORMAT;
    type=word(p+20);resolved->cycle_deadline=resolved->unhide_deadline=-1;
    if(type==20){
    ...
    rf_runtime_event *e,int32_t now,const rf_event_checkpoint_refs *refs)
{
    ...
    if(e->state.type==50)e->unhide.deadline=resolved.unhide_deadline;
    return RF_OK;
}
