...
int rf_scene_replay_header(FILE *file,uint32_t *count,uint32_t *record_size)
{
    ...
    if(header[0]==0x32494652u || header[0]==0x33494652u || header[0]==0x34494652u || header[0]==0x35494652u || header[0]==0x36494652u) {
        size=header[0]==0x32494652u?28:header[0]==0x33494652u?32:header[0]==0x34494652u?40:header[0]==0x35494652u?44:48;
...
static void step_profile_mark(uint32_t phase,uint32_t *previous)
{
    ...
    if(!profile_clock || !profile_active)return;
    now=profile_clock();elapsed=now-*previous;*previous=now;
    row=rf_scene_step_profile[phase];total=((uint64_t)row[2]<<32)+row[1]+elapsed;
    ++row[0];row[1]=(uint32_t)total;row[2]=(uint32_t)(total>>32);
    if(elapsed>row[3])row[3]=elapsed;
}
...
static int scene_world_dispatch_camera(const rf_scene_world_geometry *geometry,
    ...
    const float orientation[3][3],rf_preview_mesh *mesh,uint32_t capacity_bytes,
    rf_preview_vertex *scratch,uint32_t scratch_bytes,const rf_visibility *visibility)
{
    rf_geometry_materials mapping={0};rf_level camera={0};uint32_t i,j;
...
typedef struct scene_terrain_light_tile {rf_geomod_light_grid grid;uint32_t x,y;} scene_terrain_light_tile;
...
static int scene_checkpoint_registered(scene_stream *);
...
int rf_scene_prepare_lightmaps(rf_lightmaps *maps)
{
    ...
    if(!s || !s->terrain || s->terrain_atlas_registered)return RF_OK;
    if(!maps || !maps->images || maps->count>=256)return RF_RANGE;
    if(s->terrain_checkpoint_loaded==2 && maps->count!=s->terrain_atlas_index)return RF_FORMAT;
    images=calloc(maps->count+1,sizeof(*images));if(!images)return RF_IO;
    memcpy(images,maps->images,maps->count*sizeof(*images));
    s->terrain_atlas_index=maps->count;images[maps->count]=s->terrain_atlas;
    free(maps->images);maps->images=images;maps->count++;
    maps->allocated_bytes+=sizeof(*images)+s->terrain_atlas.bytes;s->terrain_atlas_registered=1;
    rf_scene_terrain_atlas[7]=s->terrain_atlas_index;
    return scene_checkpoint_registered(s);
}
int rf_scene_update_lightmaps(rf_lightmaps *maps)
{
    scene_stream *stream=particle_draw_stream;uint32_t i,j,count,*ids;rf_vfx_light_source *sources;int status=RF_OK;
    ...
    if(!maps || maps->count!=stream->light_rgb.count+stream->terrain_atlas_registered || !stream->light_overlay_work)return RF_RANGE;
    ids=stream->light_overlay_work;sources=(rf_vfx_light_source *)(ids+1100);
    ++rf_scene_lightmap_updates[0];rf_scene_lightmap_updates[6]=0;
    for(i=0;i<stream->light_storage->dirty_count;i++) {
        ...
        if((*dirty&1u) && stream->lights) {
            status=rf_vfx_lights_box(stream->lights->pool.sources,stream->lights->pool.capacity,
                mapping.minimum,mapping.maximum,1,0,ids,1100,&count);if(status)break;
            for(j=0;j<count;j++)sources[j]=stream->lights->pool.sources[ids[j]].source;
        }
        if((*dirty&1u) && count) {
            rf_lightmap_sample_lighting lighting={0};
            lighting.sample=sample;lighting.width=mapping.width;lighting.height=mapping.height;
            lighting.lights=sources;lighting.light_count=count;lighting.directional_scale=.25f; /* original5a38e0 initial value */
            status=rf_lightmap_live_image(&lighting,base,image,dirty);if(status)break;
            ++rf_scene_lightmap_updates[3];++rf_scene_lightmap_updates[2];
        } else if(*dirty&9u) {
            size_t offset=((size_t)mapping.y*base->width+mapping.x)*3;unsigned char upload_dirty=8;
            if(base->width!=image->width || base->height!=image->height || offset>=base->bytes){status=RF_RANGE;break;}
            status=rf_lightmap_upload_image_1555(image,base->pixels+offset,base->bytes-(uint32_t)offset,base->width*3,
                mapping.x,mapping.y,mapping.width,mapping.height,&upload_dirty);if(status)break;
            ++rf_scene_lightmap_updates[2];
        }
        *dirty=0;
    }
    rf_scene_lightmap_updates[7]=(uint32_t)status;return status;
}
...
int rf_scene_fire_goal_setter(uint32_t uid,int32_t now)
{
    uint32_t i;rf_startup_events_report report;
    for(i=0;i<campaign_events.count;i++)if(campaign_events.items[i].authored->record.uid==uid) {
        if(campaign_events.items[i].state.type!=37)return RF_FORMAT;
        return rf_runtime_event_fire(&campaign_triggers,campaign_events.items[i].handle,UINT32_MAX,UINT32_MAX,now,&scene_gravity,NULL,NULL,&report);
    }
    return RF_NOT_FOUND;
}
...
int32_t rf_scene_corpse_motion(void *context,rf_corpse_create_source *source,const char *name)
{
    ...
    selected=campaign_model_motion_selection(source->model);
    if(selected) {
...
int rf_scene_corpse_class_source(uint32_t cls,rf_corpse_create_source *source)
{
    ...
    if(source->model) {
        const rf_entity_motion_selection *selected=campaign_model_motion_selection(source->model);
        if(selected) {
    ...
    if(actor>=campaign_poses.count || actor>=campaign_seeds.records.count)return RF_RANGE;
    {
        ...
        for(i=0;i<pose->playback.completion.active.count+3;++i) {
            ...
            if(i<pose->playback.completion.active.count)selected=pose->playback.completion.active.slots[i].motion;
                ...
                {
                    const rf_entity_motion_selection *selection=campaign_model_motion_selection(actor+1);
                    selected=(selection?&selection->mapping:campaign_motion_catalog.mappings+campaign_seeds.items[actor].class_index)->states[state];
                }
            }
...
static uint32_t campaign_equipped_slot,weapon_cycle_held;static int32_t campaign_rifle_id=-1,campaign_riot_id=-1,campaign_shotgun_id=-1,campaign_rocket_id=-1;
...
{return slot==4?campaign_rocket_id:slot==3?campaign_shotgun_id:slot==2?campaign_riot_id:slot==1?campaign_rifle_id:campaign_pistol_id;}
...
static rf_attachment_node *campaign_attachment_nodes;
static rf_attachment_node **campaign_attachment_scratch;
static uint32_t campaign_attachment_count;
uint32_t rf_scene_attachments[8]; /* passes, visits, lookups, publications, prop refreshes, bytes, hash, errors */
...
static int campaign_audio_open(const char *tables_path,const char *level_name,const char *player_class)
{
    ...
    if(campaign_seeds.class_count || player_class) {
        ...
        if(campaign_seeds.class_count) {
            ...
            campaign_squash_groups=malloc(campaign_seeds.class_count*sizeof(*campaign_squash_groups));
            if(!campaign_squash_groups){status=RF_RANGE;goto audio_done;}
        }
        ...
        for(i=0;i<campaign_seeds.class_count;++i) {
            ...
            if(status)goto audio_done;
            status=rf_entity_squash_sound_group_read(entity_text,entity_entry.size,name,&campaign_foley,campaign_squash_groups+i);
            if(status)goto audio_done;
        }
        if(player_class) {
...
static void campaign_alpha_close(void)
{
...
static int campaign_alpha_open(const rf_geometry_collision_world *world,const rf_materials *materials)
{
    ...
    for(i=0;i<n;++i) {
        rf_geometry_material_collision *view=campaign_alpha_views+i;
        view->materials=&campaign_alpha_mapping;view->work=&campaign_alpha_work;
        view->geometry_index=i<world->room_count?0:i-world->room_count+1;
        view->geometry=campaign_surface_sources[view->geometry_index];
        if(i<world->room_count){view->source_indices=world->rooms[i].tree.source_indices;view->face_count=world->rooms[i].tree.face_count;}
        else view->face_count=campaign_movers.owned[i-world->room_count].count;
        status=rf_geometry_material_collision_bind(view,bitmaps,view->face_count,campaign_alpha_backends+i);
        if(status){campaign_alpha_close();return status;}
        campaign_alpha_backends[i].sample=campaign_alpha_sample;
    ...
    float radius,float limit,rf_geometry_world_sweep_hit *result,uint32_t *matched)
{
    ...
    campaign_alpha_mapping.textures=*campaign_alpha_materials;campaign_alpha_mapping.textures.count=actor_follow_world->material_count;
    if(solid==UINT32_MAX) {
    ...
    if(found) {
        ...
        value.face=solid==UINT32_MAX?campaign_alpha_world->rooms[hit.room].tree.source_indices[hit.tree.face_index]:hit.tree.face_index;
...
static int campaign_alpha_check(uint32_t frame)
{
    ...
    if(!campaign_alpha_world)return RF_RANGE;
    campaign_alpha_mapping.textures=*campaign_alpha_materials;campaign_alpha_mapping.textures.count=actor_follow_world->material_count;
    n=campaign_alpha_world->room_count+campaign_movers.count;
    for(i=0;i<n;++i) {
        ...
        for(j=0;j<count;++j) {
            ...
            if(campaign_alpha_backends[i].bitmaps[index]>=0) {
                status=campaign_alpha_backends[i].sample(campaign_alpha_backends[i].context,index,face,campaign_alpha_backends[i].bitmaps[index],point,&color);
                if(status){++rf_scene_geometry_textures[9];return status;}
            }
            status=rf_scene_geometry_texture_query(i<campaign_alpha_world->room_count?UINT32_MAX:i-campaign_alpha_world->room_count,
...
static void campaign_clutter_render_close(void)
{
    ...
    uint32_t i;for(i=0;i<campaign_clutter_model_count;++i) {
        rf_static_model_tags_close(&campaign_clutter_models[i].tags);
...
static int campaign_clutter_render_open(rf_vpp *archive)
{
    const uint32_t budget=1024*1024;rf_model_file *file=NULL;uint64_t bytes;
    ...
    if(campaign_clutter_records.count) {
        campaign_clutter_models=calloc(campaign_clutter_records.count,sizeof(*campaign_clutter_models));
        campaign_clutter_model_slots=malloc(campaign_clutter_records.count*sizeof(*campaign_clutter_model_slots));
        file=malloc(sizeof(*file));
        if(!campaign_clutter_models || !campaign_clutter_model_slots || !file){status=RF_IO;goto fail;}
    }
    for(i=0;i<campaign_clutter_records.count;++i) {
        const rf_level_clutter *record=campaign_clutter_records.items+i;const rf_clutter_class *c;char name[64];
        campaign_clutter_model_slots[i]=UINT32_MAX;
        for(j=0;j<campaign_clutter_classes.count;++j)if(rf_emitter_name_lookup(&campaign_clutter_classes.items[j].name,1,record->class_name)==0)break;
        if(j==campaign_clutter_classes.count){++rf_scene_clutter_render[2];continue;}
        c=campaign_clutter_classes.items+j;
        if(c->model_kind!=1){++rf_scene_clutter_render[3];continue;}
        status=rf_model_compiled_filename(c->model,name,".v3m");if(status)goto fail;
        for(j=0;j<campaign_clutter_model_count;++j) {
            const char *key=campaign_clutter_models[j].name;
            if(rf_emitter_name_lookup(&key,1,name)==0)break;
        }
        if(j==campaign_clutter_model_count) {
            rf_static_render_resource *resource=&campaign_clutter_models[j].resource;
            status=rf_model_file_open(file,archive,name);if(status)goto fail;
            ...
            status=rf_static_model_tags_open(file,(uint32_t)(budget-bytes-sizeof(*file)+sizeof(rf_static_model_tags)),
                &campaign_clutter_models[j].tags);if(status)goto fail;
            bytes+=campaign_clutter_models[j].tags.allocated_bytes-sizeof(rf_static_model_tags);
        }
        campaign_clutter_model_slots[i]=j;++rf_scene_clutter_render[1];
    }
    rf_scene_clutter_render[0]=campaign_clutter_model_count;rf_scene_clutter_render[4]=(uint32_t)bytes;
    rf_scene_clutter_render[5]=(uint32_t)bytes+(file?sizeof(*file):0);
    for(i=0;i<campaign_clutter_model_count;++i) {
        const campaign_clutter_model *m=campaign_clutter_models+i;const rf_static_render_resource *r=&m->resource;
        ++rf_scene_clutter_tags[0];rf_scene_clutter_tags[1]+=m->tags.count;rf_scene_clutter_tags[2]+=m->tags.allocated_bytes;
        rf_scene_clutter_tags[3]=npc_hash_bytes(rf_scene_clutter_tags[3],m->tags.items,m->tags.count*sizeof(*m->tags.items));
        ...
        for(j=0;j<r->lod_count;++j) {
            ...
            hash=npc_hash_bytes(hash,&lod->threshold,4);hash=npc_hash_bytes(hash,&g->batch_count,16);
            hash=npc_hash_bytes(hash,g->batches,g->batch_count*sizeof(*g->batches));hash=npc_hash_bytes(hash,g->vertices,g->vertex_count*sizeof(*g->vertices));
            hash=npc_hash_bytes(hash,g->triangles,g->triangle_count*sizeof(*g->triangles));hash=npc_hash_bytes(hash,g->reuse,g->vertex_count*4);
            hash=npc_hash_bytes(hash,lod->planes,g->triangle_count*16);
        }
    }
    for(k=0;k<campaign_clutter_records.count;++k)hash=npc_hash_bytes(hash,campaign_clutter_model_slots+k,4);
    rf_scene_clutter_render[7]=hash;free(file);return RF_OK;
 fail:
...
static int campaign_clutter_materials_open(const char *tables_path,rf_vpp *archives,uint32_t archive_count)
{
    ...
    for(i=0;i<campaign_clutter_materials.textures.count;++i) {
        const rf_image *image=&campaign_clutter_materials.textures.items[i].image;
        ...
        rf_scene_clutter_materials[6]+=image->bytes;
    }
...
static int campaign_clutter_open(const char *tables_path,const rf_level *level)
{
    ...
    if(!status) {
        ...
        rf_scene_pistol_rules[0]=campaign_pistol.magazine;rf_scene_pistol_rules[1]=pistol_reload_ticks;
        rf_scene_pistol_rules[2]=pistol_fire_ticks;memcpy(rf_scene_pistol_rules+3,&campaign_pistol.damage,4);
...
uint32_t rf_scene_npc_startup_weapons[4]; /* primary,secondary,owned slots,selection hash */
...
{
    if(owner->script_move.follow!=2)return;
...
static int campaign_npc_bodies_open(const char *tables_path,const rf_geometry_collision_world *world,int32_t now)
{
    ...
    memset(rf_scene_npc_startup_weapons,0,sizeof(rf_scene_npc_startup_weapons));rf_scene_npc_startup_weapons[3]=2166136261u;
    ...
    for(actor=0;actor<campaign_npc_body_count;++actor)if(campaign_poses.items[actor].skeleton!=UINT32_MAX) {
        ...
        view->weapons[0]=view->weapons[1]=-1;view->base_speed=campaign_npc_movement_configs[campaign_seeds.items[actor].class_index].base_speed;
        {
            ...
            rf_scene_npc_startup_weapons[3]=npc_hash_bytes(rf_scene_npc_startup_weapons[3],view->weapons,sizeof(view->weapons));
        }
...
{
    campaign_npc_body *owner;rf_entity_pose *pose;uint32_t cls,slot;int status;
    status=campaign_npc_motion_owner(handle,&owner,&cls,&pose);if(status)return status;
    slot=(uint32_t)(owner-campaign_npc_bodies);if(slot>=campaign_model_owner_count)return RF_RANGE;
    status=rf_physics_publish_position(&owner->body.state,owner->published,&owner->object_flags);if(status)return status;
    owner->view.flags_7c=owner->object_flags;
    memcpy(campaign_model_owners[slot].position,owner->published,12);
    return RF_OK;
}
...
static void campaign_pain_effect(void *context,uint32_t effect,uint32_t first,uint32_t second)
{
    ...
    case RF_PAIN_START:
        ...
        for(i=0;i<45;++i)sounds[i]=c->sounds[i][0]?(int32_t)i:-1;
        ...
        if(!c->status && sound>=0)c->status=c->ops && c->ops->play_sound?
            c->ops->play_sound(c->ops->context,c->owner->registration.handle,c->sounds[sound]):RF_NOT_FOUND;
        break;
...
int rf_scene_npc_pain(uint32_t handle,int32_t now,rf_random_state *random,const rf_scene_npc_pain_ops *ops)
{
    ...
    c.sounds=c.owner->selection.action_sounds;c.model=campaign_playback_resources.models+c.pose->skeleton;
...
{
    campaign_npc_body *owner;rf_entity_pose *pose;rf_entity_playback_model *model;
    const rf_entity_motion_mapping *mapping;const char *const *labels;
    ...
    model=campaign_playback_resources.models+pose->skeleton;
    for(i=0;i<45;++i)sounds[i]=labels[i][0]?(int32_t)i:-1;
    ...
    if(status)return status;
    if(sound>=0)return play_sound?play_sound(context,handle,labels[sound]):RF_NOT_FOUND;
    return RF_OK;
}
...
static void campaign_unholster_reload(campaign_unholster_context *c)
{
    ...
    s->motion_cd4=o->selection.mapping.actions[40];s->word_7bc=(uint32_t)o->unholster.stance_7bc;
...
static int campaign_pain_reset_retained(void *context,uint32_t handle,int32_t weapon)
{
    rf_weapon_reset_context reset={0,campaign_weapon_reset.names.count};(void)context;
    if(!reset.weapon_count)return RF_NOT_FOUND;
    return rf_scene_npc_weapon_reset(handle,weapon,campaign_weapon_reset.definitions,&reset,NULL,NULL);
}
int rf_scene_npc_pain_retained(uint32_t handle,int32_t now,rf_random_state *random)
{
    rf_scene_death_selection_context context={0};rf_scene_npc_pain_ops ops={campaign_pain_reset_retained,rf_scene_npc_death_sound,&context};
    context.random=random;return rf_scene_npc_pain(handle,now,random,&ops);
}
...
uint32_t rf_scene_npc_impact_dispatch[6],rf_scene_npc_impact_test[4];
static int campaign_npc_impact_load(uint32_t handle,rf_entity_impact_actor *a)
{
    campaign_npc_body *o;uint32_t i,cls;
    for(i=0;i<campaign_npc_body_count;++i)if(campaign_npc_bodies[i].registration.view && campaign_npc_bodies[i].registration.handle==handle)break;
    if(i==campaign_npc_body_count)return RF_NOT_FOUND;o=campaign_npc_bodies+i;
    if(rf_entity_lookup(&campaign_entities,(int32_t)handle)!=&o->view)return RF_NOT_FOUND;
    if(!campaign_seeds.items || !campaign_seeds.classes || !campaign_impact_groups || i>=campaign_seeds.records.count || o->movement_slot>=16)return RF_RANGE;
    cls=campaign_seeds.items[i].class_index;if(cls>=campaign_seeds.class_count)return RF_RANGE;
    a->handle=handle;a->object_flags=o->object_flags;a->movement_mode=campaign_modes[o->movement_slot].index;
    a->use_kind=campaign_seeds.classes[cls].physics.use_kind;a->contact_material=o->collision_contact.material;
    a->support_material=o->support.material;a->sound_set=(uint32_t)campaign_impact_groups[cls];a->health=o->damage.effects.health;
    memcpy(a->published,o->published,12);memcpy(a->position,o->body.state.position,12);return RF_OK;
}
static int campaign_npc_impact_suppressed(void *context,rf_entity_impact_actor *a,uint32_t *suppressed)
{
    int status;(void)context;status=rf_physics_force_suppresses_damage(campaign_forces.items,campaign_forces.count,a->published,suppressed);
    if(!status && (*suppressed&255u))++rf_scene_npc_impact_dispatch[1];return status;
}
static int campaign_npc_impact_damage(void *context,rf_entity_impact_actor *a,const rf_damage_request *request)
{
    const rf_scene_npc_impact_services *c=context;float amount;int status;uint32_t handle=a->handle;
    ++rf_scene_npc_impact_dispatch[2];status=rf_scene_npc_damage(handle,request,c->difficulty,c->clock_bits,c->effects,&amount);
    if(status)return status;return campaign_npc_impact_load(handle,a);
}
static int campaign_npc_impact_sound(void *context,rf_entity_impact_actor *a)
{
    const rf_scene_npc_impact_services *c=context;++rf_scene_npc_impact_dispatch[3];return rf_scene_npc_impact_sound(a->handle,c->random);
}
static int campaign_npc_impact_feedback(void *context,rf_entity_impact_actor *a,float amount)
{
    const rf_scene_npc_impact_services *c=context;const rf_entity_view *player;
    ++rf_scene_npc_impact_dispatch[4];player=rf_entity_lookup(&campaign_entities,campaign_player_view.handle);
    if(!player || (player->handle!=(int32_t)a->handle && player->linked_handle!=(int32_t)a->handle))return RF_OK;
    return c->player_feedback?c->player_feedback(c->context,(uint32_t)player->handle,amount):RF_NOT_FOUND;
}
int rf_scene_npc_impact(uint32_t handle,float speed,const rf_scene_npc_impact_services *services)
{
    ...
    rf_entity_impact_backend backend={campaign_npc_impact_suppressed,campaign_npc_impact_damage,campaign_npc_impact_sound,campaign_npc_impact_feedback,(void *)services};
    if(!services || !services->effects || !services->random || !isfinite(services->difficulty))return RF_RANGE;
    status=campaign_npc_impact_load(handle,&actor);if(status)return status;
...
static void campaign_damage_test_notify(void *c,uint32_t kind,uint32_t target,float value,uint32_t source)
{
    ...
    if(kind==RF_DAMAGE_PAIN_ANIMATION && rf_scene_npc_pain_retained(target,1000,c))rf_scene_npc_damage_test_words[63]++;
...
static int campaign_npc_damage_fixture(void)
{
    ...
    memset(rf_scene_npc_impact_dispatch,0,sizeof(rf_scene_npc_impact_dispatch));memset(rf_scene_npc_impact_test,0,sizeof(rf_scene_npc_impact_test));
    ...
    if(!status) {
        rf_scene_npc_impact_services impact={&effects,&random,1,0x3f800000,NULL,NULL};
        campaign_npc_body saved=*owner;rf_physics_force_collection forces=campaign_forces;
        rf_physics_force_region region={0};uint32_t before[6];rf_random_state rng_before=random;
        memcpy(before,rf_scene_npc_impact_dispatch,sizeof(before));
        if(rf_scene_npc_impact(owner->registration.handle^0x10000,11,&impact)!=RF_NOT_FOUND ||
           memcmp(&saved,owner,sizeof(saved)) || memcmp(before,rf_scene_npc_impact_dispatch,sizeof(before)) || random.value!=rng_before.value)status=RF_FORMAT;
        else ++rf_scene_npc_impact_test[0];
        if(!status)status=rf_scene_npc_impact(owner->registration.handle,0,&impact);
        if(!status && memcmp(&saved,owner,sizeof(saved)))status=RF_FORMAT;
        if(!status)++rf_scene_npc_impact_test[0];
        region.shape=1;region.radius_squared=100;region.active=1;region.flags=0x40;memcpy(region.center,owner->published,12);
        campaign_forces.items=&region;campaign_forces.count=1;campaign_forces.allocated_bytes=0;
        if(!status)status=rf_scene_npc_impact(owner->registration.handle,11,&impact);
        campaign_forces=forces;
        if(!status && (memcmp(&saved,owner,sizeof(saved)) || rf_scene_npc_impact_dispatch[1]!=1))status=RF_FORMAT;
        if(!status)++rf_scene_npc_impact_test[0];
        memcpy(rf_scene_npc_impact_test+1,&owner->damage.effects.health,4);
        if(!status)status=rf_scene_npc_impact(owner->registration.handle,11,&impact);
        memcpy(rf_scene_npc_impact_test+2,&owner->damage.effects.health,4);
...
} campaign_npc_stand_context;
static int campaign_npc_stand_clearance(void *context,const float start[3],const float end[3],uint32_t *blocked)
{
    campaign_npc_stand_context *c=context;rf_physics_body_state state=c->owner->body.state;
    rf_collision_body_sphere scratch[8];rf_geometry_body_hit hit;uint32_t flags=state.state_124|4;
    memcpy(state.position,start,12);memcpy(state.next_position,end,12);if(state.bounds.radius<.05f)flags|=0x100;
    return rf_scene_npc_body_sweep(c->world,c->owner->registration.handle,&state,flags,scratch,8,&hit,blocked);
}
static uint8_t *campaign_npc_stand_player(void *context)
{
    campaign_npc_stand_context *c=context;
    c->owner->damage.effects.flags_810=c->owner->view.flags_810;
    return c->services->player_crouch?c->services->player_crouch(c->services->context,c->owner->registration.handle):NULL;
}
static int campaign_npc_stand_ground(void *context)
{
    campaign_npc_stand_context *c=context;
    c->owner->damage.effects.flags_810=c->owner->view.flags_810;
    {int status=c->services->refresh_ground(c->services->context,c->owner->registration.handle);
     c->owner->damage.effects.flags_810=c->owner->view.flags_810;return status;}
}
...
{
    campaign_npc_body *owner;rf_entity_pose *pose;uint32_t cls;int status;
    campaign_npc_stand_context context;const rf_physics_stand_ops ops={campaign_npc_stand_clearance,campaign_npc_stand_player,campaign_npc_stand_ground};
    if(!world || !services || !services->refresh_ground || !stood)return RF_RANGE;
    status=campaign_npc_motion_owner(handle,&owner,&cls,&pose);if(status)return status;
    if(!campaign_npc_stances)return RF_RANGE;
    context.world=world;context.owner=owner;context.services=services;
...
{
    campaign_npc_body *owner;rf_entity_pose *pose;uint32_t cls;int status,stood;
    status=campaign_npc_motion_owner(handle,&owner,&cls,&pose);if(status)return status;
    if(!(campaign_seeds.classes[cls].physics.flags&1u))return rf_scene_npc_set_speed(handle,0);
    if(forced_crouch&255u) {
        owner->view.flags_810|=0x400u;owner->damage.effects.flags_810=owner->view.flags_810;
    } else if(owner->view.flags_810&0x400u) {
        status=rf_scene_npc_try_stand(world,handle,services,&stood);if(status)return status;
        /* Original428030 ignores blocked-standing result and continues. */
    }
    status=rf_scene_npc_set_speed(handle,0);if(status)return status;
    owner->movement_slot=(campaign_modes[1].enabled&255u)?1u:0u;
    owner->movement_orientation=campaign_identity[0];owner->body.state.velocity[1]=0;return RF_OK;
}
...
static void campaign_ammo_publish(void)
{
    ...
    rf_scene_combat[5]=(uint32_t)campaign_player_inventory.loaded[campaign_selected_weapon()];
    rf_scene_player_ammo[0]=(uint32_t)campaign_selected_weapon();rf_scene_player_ammo[1]=(uint32_t)campaign_player_inventory.reserve[d->ammo_type];
    if(!campaign_player_inventory.owned[campaign_selected_weapon()])rf_scene_player_ammo[0]=UINT32_MAX;
...
static int combat_box(const float start[3],const float delta[3],const rf_physics_bounds *box,float limit,float *fraction)
{
    ...
    for(j=0;j<3;j++) {
        double a,d;
        if(fabsf(delta[j])<1e-8f){if(start[j]<box->minimum[j] || start[j]>box->maximum[j])return 0;continue;}
        a=((double)box->minimum[j]-start[j])/delta[j];d=((double)box->maximum[j]-start[j])/delta[j];
        if(a>d){double t=a;a=d;d=t;}if(a>lo)lo=a;if(d<hi)hi=d;if(lo>hi)return 0;
    }
...
uint32_t rf_scene_enemy_melee[4]; /* swings, out-of-reach attempts, pursuit requests, max squared strike distance */
...
static int campaign_enemy_tick(scene_stream *stream,uint32_t frame,const float player_eye[3])
{
    ...
    for(i=0;i<campaign_npc_body_count && campaign_player_damage.state.effects.health>0;i++) {
        ...
        {
            int pending;int32_t now=(int32_t)((uint64_t)frame*1000/60);
...
static int scene_terrain_light_step(scene_stream *s,const rf_geomod_terrain_view *terrain)
{
    ...
    rf_geomod_light_bake bake={cache->sources,cache->modes,cache->count,
        {cache->ambient[0],cache->ambient[1],cache->ambient[2]},.25f,terrain};
    while(remaining && cache->face<terrain->mesh.face_count) {
        const rf_geomod_face *face=terrain->mesh.faces+cache->face;scene_terrain_light_tile *tile=s->terrain_tiles+cache->face;
        uint32_t count,stats[3],x,y,total=tile->grid.width*tile->grid.height;
        if(face->source_face!=UINT32_MAX){cache->face++;continue;}
        count=total-cache->sample;if(count>remaining)count=remaining;
        status=rf_geomod_light_grid_bake_range(&tile->grid,terrain->mesh.vertices+face->first,face->count,&bake,
            s->terrain_tile,tile->grid.width*2,64*64*2,cache->sample,count,stats);if(status)return status;
        cache->sample+=count;remaining-=count;processed+=count;
        rf_scene_terrain_shadows[1]+=stats[1];rf_scene_terrain_shadows[2]+=stats[2];
    ...
    rf_scene_terrain_bake[3]=cache->face<terrain->mesh.face_count;
...
static int scene_terrain_shadow_lighting(scene_stream *s,const rf_geomod_terrain_view *terrain)
{
    ...
    if(cache->valid && cache->generation==terrain->mesh.generation && cache->count==count &&
       ...
       !memcmp(cache->modes,modes,count*sizeof(*modes))) {++rf_scene_terrain_shadows[3];return scene_terrain_light_step(s,terrain);}
    {
        uint32_t tile_x=0,tile_y=0,row_height=0,texels=0,faces=0;
        unsigned char rgb[3],packed[2];uint32_t pixel;
        if(cache->valid && rf_scene_terrain_bake[3])++rf_scene_terrain_bake[5];
        rf_scene_terrain_bake[0]=0;cache->face=cache->sample=0;
        if(!s->terrain_atlas_registered)return RF_RANGE;
        status=rf_lightmap_accumulated_rgb(ambient,rgb);if(status)return status;
        status=rf_lightmap_pack_1555(rgb,3,1,1,0,packed,2,2);if(status)return status;
        for(pixel=0;pixel<512*512;pixel++)memcpy(s->terrain_atlas_pixels+pixel*2,packed,2);
        for(i=0;i<terrain->mesh.vertex_count;i++)for(k=0;k<3;k++)s->terrain_colors[i][k]=1;
        for(i=0;i<terrain->mesh.face_count;i++) {
            const rf_geomod_face *face=terrain->mesh.faces+i;rf_geomod_light_grid grid;
            rf_preview_surface_lightmap *binding=s->terrain_bindings+i;
            binding->image=UINT32_MAX;if(face->source_face!=UINT32_MAX)continue;
            status=rf_geomod_light_grid_open(terrain->mesh.vertices+face->first,face->count,terrain->faces[i].plane,.5f,&grid);if(status)return status;
            if(tile_x+grid.width>512){tile_y+=row_height;tile_x=0;row_height=0;}
            if(tile_y+grid.height>512)return RF_RANGE;
            s->terrain_tiles[i].grid=grid;s->terrain_tiles[i].x=tile_x;s->terrain_tiles[i].y=tile_y;
            binding->image=s->terrain_atlas_index;binding->projection.axes[0]=grid.u;binding->projection.axes[1]=grid.v;
            for(j=0;j<2;j++) {
                double scale=((j?grid.height:grid.width)-3)/((double)grid.maximum[j]-grid.minimum[j]);
                binding->projection.scale[j]=(float)(scale/512);
                binding->projection.offset[j]=(float)(((j?tile_y:tile_x)+1.5-grid.minimum[j]*scale)/512);
            }
...
static int scene_terrain_lighting(scene_stream *s,const rf_geomod_terrain_view *terrain,uint32_t frame)
{
    ...
    if(!owner || (!s->terrain_atlas_registered && s->terrain_checkpoint_loaded!=2) || terrain->mesh.face_count>SCENE_TERRAIN_FACES)return RF_RANGE;
...
static float checkpoint_float(const unsigned char *p)
{uint32_t w=checkpoint_u32(p);float f;memcpy(&f,&w,4);return f;}
static void checkpoint_put_float(unsigned char *p,float f)
{uint32_t w;memcpy(&w,&f,4);checkpoint_put(p,w);}
typedef struct checkpoint_sha {uint32_t h[8],used;uint64_t bytes;unsigned char block[64];} checkpoint_sha;
static uint32_t checkpoint_rotr(uint32_t v,uint32_t n){return (v>>n)|(v<<(32-n));}
static void checkpoint_sha_block(checkpoint_sha *s)
{
    ...
    for(i=16;i<64;i++)w[i]=w[i-16]+(checkpoint_rotr(w[i-15],7)^checkpoint_rotr(w[i-15],18)^(w[i-15]>>3))+w[i-7]+(checkpoint_rotr(w[i-2],17)^checkpoint_rotr(w[i-2],19)^(w[i-2]>>10));
    for(i=0;i<64;i++) {
        uint32_t t=h+(checkpoint_rotr(e,6)^checkpoint_rotr(e,11)^checkpoint_rotr(e,25))+((e&f)^(~e&g))+k[i]+w[i];
        uint32_t u=(checkpoint_rotr(a,2)^checkpoint_rotr(a,13)^checkpoint_rotr(a,22))+((a&b)^(a&c)^(b&c));
        h=g;g=f;f=e;e=d+t;d=c;c=b;b=a;a=t+u;
    }
    s->h[0]+=a;s->h[1]+=b;s->h[2]+=c;s->h[3]+=d;s->h[4]+=e;s->h[5]+=f;s->h[6]+=g;s->h[7]+=h;
}
static void checkpoint_sha_init(checkpoint_sha *s)
{
...
static void checkpoint_sha_add(checkpoint_sha *s,const void *data,uint32_t bytes)
{
    const unsigned char *p=data;s->bytes+=bytes;
    while(bytes){uint32_t n=64-s->used;if(n>bytes)n=bytes;memcpy(s->block+s->used,p,n);s->used+=n;p+=n;bytes-=n;if(s->used==64){checkpoint_sha_block(s);s->used=0;}}
}
static void checkpoint_sha_word(checkpoint_sha *s,uint32_t w)
{unsigned char p[4];checkpoint_put(p,w);checkpoint_sha_add(s,p,4);}
static void checkpoint_sha_float(checkpoint_sha *s,float f)
{unsigned char p[4];checkpoint_put_float(p,f);checkpoint_sha_add(s,p,4);}
static void checkpoint_sha_end(checkpoint_sha *s,unsigned char out[32])
{
    uint64_t bits=s->bytes*8;unsigned char p[8];uint32_t i;
    p[0]=128;checkpoint_sha_add(s,p,1);p[0]=0;
    while(s->used!=56)checkpoint_sha_add(s,p,1);
...
static int scene_checkpoint_identity(scene_stream *s,const rf_level *level)
{
    ...
    rf_image *image;unsigned char test[32];
    ...
    for(i=0;i<6;i++) {
        ...
        status=rf_geometry_initial_collision_filter(s->geometry,i,0,&f);if(status)return status;
        status=rf_geometry_get_face(s->geometry,i,&face);if(status)return status;
        if(face.texture>=s->geometry->textures || actor_follow_world->slots[face.texture]>=s->materials->count)return RF_FORMAT;
        image=&s->materials->items[actor_follow_world->slots[face.texture]].image;
        checkpoint_sha_word(&h,image->width);checkpoint_sha_word(&h,image->height);checkpoint_sha_word(&h,image->source_format);
        for(y=0;y<image->height;y++)for(x=0;x<image->width;x++)checkpoint_sha_add(&h,rf_image_pixel(image,x,y),rf_image_is_packed_1555(image)?2:4);
    ...
    for(i=0;i<s->terrain_template->face_count*3;i++) {
        for(j=0;j<3;j++)checkpoint_sha_float(&h,s->terrain_template->vertices[i].position[j]);
        for(j=0;j<2;j++)checkpoint_sha_float(&h,s->terrain_template->vertices[i].uv[j]);
    }
    ...
    for(i=0;i<s->terrain_region_count;i++) {
        ...
        checkpoint_sha_word(&h,(uint32_t)r->uid);checkpoint_sha_word(&h,r->flags);checkpoint_sha_word(&h,r->hardness);checkpoint_sha_float(&h,r->shallow_depth);
        for(j=0;j<3;j++)checkpoint_sha_float(&h,r->position[j]);for(j=0;j<9;j++)checkpoint_sha_float(&h,r->file_basis[j]);
        for(j=0;j<3;j++)checkpoint_sha_float(&h,r->dimensions[j]);checkpoint_sha_float(&h,r->radius);
    }
    checkpoint_sha_end(&h,s->terrain_checkpoint_identity+96);return RF_OK;
}
static void scene_checkpoint_release(void)
{free(rf_scene_geomod_checkpoint_data);rf_scene_geomod_checkpoint_data=NULL;rf_scene_geomod_checkpoint_memory[0]=0;}
static int scene_checkpoint_allocate(uint32_t bytes)
{
    if(bytes>SCENE_CHECKPOINT_MAX || rf_scene_geomod_checkpoint_data)return RF_RANGE;
...
static int scene_checkpoint_registered(scene_stream *s)
{
    ...
    if(!s->terrain_checkpoint_loaded)return RF_OK;
    if(s->terrain_checkpoint_loaded==2) {
        if(!s->terrain_atlas_registered)return RF_RANGE;
        s->terrain_checkpoint_loaded=0;return RF_OK;
    }
    status=rf_geomod_terrain_get(s->terrain,&view);if(status)return status;
    for(i=0;i<s->terrain_noise->count;i++)s->terrain_noise->maps[i].binding.image=s->terrain_atlas_index;
    for(i=0;i<view.mesh.face_count;i++)if(s->terrain_bindings[i].image!=UINT32_MAX) {
        uint32_t map=s->terrain_bindings[i].image;if(map>=s->terrain_noise->count)return RF_FORMAT;
        s->terrain_bindings[i]=s->terrain_noise->maps[map].binding;
    }
    s->terrain_checkpoint_loaded=s->terrain_atlas_registered?0:2;return RF_OK;
}
static int scene_checkpoint_base(scene_stream *s,scene_terrain_noise_map *map,rf_random_state *chain)
{
    rf_random_state random={map->base_seed};unsigned char rgb[64*3],packed[64*2];uint32_t at=0,total=map->width*map->height,i;int status;
    if(chain->value!=random.value)return RF_FORMAT;
    while(at<total) {
        uint32_t n=total-at;if(n>64)n=64;
        status=rf_geomod_light_noise(rgb,sizeof(rgb),n*3,n,1,&random);if(status)return status;
        status=rf_lightmap_pack_1555(rgb,n*3,n,1,0,packed,n*2,sizeof(packed));if(status)return status;
        for(i=0;i<n;i++)memcpy(s->terrain_atlas_pixels+((map->y+(at+i)/map->width)*512+map->x+(at+i)%map->width)*2,packed+i*2,2);
        at+=n;
    }
...
static int scene_checkpoint_restore(scene_stream *s,unsigned char *data,uint32_t bytes)
{
    ...
    for(i=0;i<admission;i++,at+=48) {
        rf_geomod_shallow_history *h=s->terrain_history+i;
        for(j=0;j<10;j++)if(!isfinite(checkpoint_float(data+at+j*4)))return RF_FORMAT;
        for(j=0;j<3;j++)h->center[j]=checkpoint_float(data+at+j*4);
        for(j=0;j<2;j++)for(k=0;k<3;k++)h->vectors[j][k]=checkpoint_float(data+at+12+(j*3+k)*4);
        h->scale=checkpoint_float(data+at+36);if(h->scale<=0 || data[at+46] || data[at+47])return RF_FORMAT;
    ...
    status=scene_checkpoint_materials(data+SCENE_CHECKPOINT_HEADER,core,0,s->terrain_material);if(status)return status;
    status=rf_geomod_terrain_history_decode(s->terrain,data+SCENE_CHECKPOINT_HEADER,core);if(status)return status;
    status=rf_geomod_terrain_get(s->terrain,&view);if(status)return status;if(view.mesh.face_count!=faces)return RF_FORMAT;
    for(i=0;i<faces;i++,at+=2) {
        const rf_geomod_face *f=view.mesh.faces+i;uint32_t m=data[at]|((uint32_t)data[at+1]<<8);
        s->terrain_bindings[i].image=UINT32_MAX;
        if(f->source_face!=UINT32_MAX){if(m!=65535)return RF_FORMAT;continue;}
        if(m>=maps || f->material!=s->terrain_material || !scene_terrain_noise_plane(owner->maps[m].plane,view.faces[i].plane))return RF_FORMAT;
        for(j=0;j<f->count;j++)for(k=0;k<3;k++){float v=view.mesh.vertices[f->first+j].position[k];if(v<owner->maps[m].minimum[k]-1e-4f || v>owner->maps[m].maximum[k]+1e-4f)return RF_FORMAT;}
        s->terrain_bindings[i].image=m;
    }
    status=scene_terrain_bind(s);if(status)return status;
    s->terrain_history_count=admission;s->terrain_random.value=checkpoint_u32(data+244);
    owner->count=owner->bake=maps;owner->sample=0;owner->x=x;owner->y=y;owner->row=row;owner->random=chain;
    owner->generation=view.cuts?view.mesh.generation:0;owner->cuts=view.cuts;memset(s->terrain_light_cache,0,sizeof(*s->terrain_light_cache));s->terrain_checkpoint_loaded=1;
    scene_terrain_dirty(s,0,0,512,512);
    memset(rf_scene_terrain_noise,0,sizeof(rf_scene_terrain_noise));
    rf_scene_terrain_noise[0]=1;rf_scene_terrain_noise[1]=maps;rf_scene_terrain_noise[6]=sizeof(*owner);rf_scene_terrain_noise[7]=owner->generation;
    rf_scene_terrain_atlas[4]=owner->generation;rf_scene_terrain_atlas[6]=maps;
    for(i=0;i<maps;i++)rf_scene_terrain_noise[3]+=owner->maps[i].width*owner->maps[i].height;
    rf_scene_terrain_atlas[5]=rf_scene_terrain_noise[3];
...
#ifndef RF_IMAGE_XBOX_NATIVE
    path=getenv("RF_REPLAY_GEOMOD_CHECKPOINT_IN");if(!path || !*path)return RF_OK;
#else
    path="D:\\geomod-checkpoint.bin";
#endif
    file=fopen(path,"rb");
    if(!file) {
#ifndef RF_IMAGE_XBOX_NATIVE
        status=RF_IO;goto done;
#else
        return RF_OK; /* Optional staged diagnostic input. */
#endif
    }
    if(fseek(file,0,SEEK_END) || (length=ftell(file))<SCENE_CHECKPOINT_HEADER || length>SCENE_CHECKPOINT_MAX || fseek(file,0,SEEK_SET))status=RF_FORMAT;
    else {
        status=scene_checkpoint_allocate((uint32_t)length);
        if(!status && fread(rf_scene_geomod_checkpoint_data,1,(size_t)length,file)!=(size_t)length)status=RF_IO;
        if(!status){tail=fgetc(file);if(tail!=EOF || ferror(file))status=RF_IO;}
    }
...
static int scene_checkpoint_capture(scene_stream *s)
{
    ...
    checkpoint_put(p+240,s->terrain_history_count);checkpoint_put(p+244,s->terrain_random.value);checkpoint_put(p+248,owner->count);checkpoint_put(p+252,core);
    checkpoint_put(p+256,owner->random.value);checkpoint_put(p+260,owner->x);checkpoint_put(p+264,owner->y);checkpoint_put(p+268,owner->row);checkpoint_put(p+272,view.mesh.face_count);
    status=rf_geomod_terrain_history_encode(s->terrain,p+SCENE_CHECKPOINT_HEADER,core);if(status)goto done;
    status=scene_checkpoint_materials(p+SCENE_CHECKPOINT_HEADER,core,s->terrain_material,0);if(status)goto done;
    at=SCENE_CHECKPOINT_HEADER+core;
    for(i=0;i<s->terrain_history_count;i++,at+=48) {
    ...
    for(i=0;i<owner->count;i++,at+=88) {
        const scene_terrain_noise_map *m=owner->maps+i;if(m->material!=s->terrain_material){status=RF_FORMAT;goto done;}
    ...
    for(i=0;i<view.mesh.face_count;i++,at+=2) {
        uint32_t map=65535;
        if(view.mesh.faces[i].source_face==UINT32_MAX) {
            for(map=0;map<owner->count;map++)if(owner->maps[map].binding.image==s->terrain_bindings[i].image && !memcmp(&owner->maps[map].binding.projection,&s->terrain_bindings[i].projection,sizeof(s->terrain_bindings[i].projection)))break;
            if(map==owner->count){status=RF_FORMAT;goto done;}
        }
...
#ifndef RF_IMAGE_XBOX_NATIVE
    {
        ...
        if(length+5>=sizeof(temporary)){status=RF_RANGE;goto done;}memcpy(temporary,path,length);memcpy(temporary+length,".tmp",5);
        file=fopen(temporary,"wb");if(!file){status=RF_IO;goto done;}
        if(fwrite(p,1,bytes,file)!=bytes)status=RF_IO;if(fclose(file) && !status)status=RF_IO;
        /* Existing destination is not removed on replacement failure. Use a new
    ...
    if(pressed && !s->terrain_held) {
        ...
        if(pressed==2) {
            ...
            if(!status) {
                 ...
                 * Clear retained mappings now; a later count comparison alone
                 * cannot distinguish that new generation from pre-reset maps. */
                memset(s->terrain_noise,0,sizeof(*s->terrain_noise));
                memset(s->terrain_atlas_pixels,0,512*512*2);
...
static int campaign_combat_tick(scene_stream *stream,uint32_t frame,const float position[3],const float orientation[3][3])
{
    ...
    {rf_weapon_trigger_rules rules={alt?(uint32_t)ceilf(campaign_pistol.alt_fire_seconds*60):pistol_fire_ticks,alt && campaign_equipped_slot==1?1:campaign_pistol.burst_count,
     ...
     status=rf_weapon_trigger_step(&combat_trigger,&rules,player_input.fire || alt,inhibit,campaign_equipped_slot==2 && !alt?1:rf_scene_combat[5],&fire);if(status)return status;}
    ...
    if(rf_scene_combat[6]) {
        ...
        if(!--rf_scene_combat[6]) {
            uint32_t moved;status=rf_weapon_reload_transfer(&campaign_player_inventory,campaign_weapon_supply.definitions+campaign_selected_weapon(),campaign_selected_weapon(),&moved);
    ...
    if((fire || alt) && (campaign_equipped_slot!=2 || alt) && !rf_scene_combat[5]) {
        ...
        rf_scene_combat[6]=pistol_reload_ticks;combat_sound(campaign_equipped_slot==4?"Rocket Reload":campaign_equipped_slot==3?"Shotgun Reload Finish":campaign_equipped_slot==2?"Riot Reload":campaign_equipped_slot?"ARifle Reload":"Glock Reload",position);return RF_OK;
    }
    ...
    if(fire){++rf_scene_combat[0];campaign_last_alt=alt;combat_sound(campaign_equipped_slot==4?"Rocket Fire":campaign_equipped_slot==1?"Assault Loop":campaign_equipped_slot==3?(alt?"Shotgun Fire 2":"Shotgun Fire"):alt?"Riot Attack Taser":campaign_equipped_slot==2?"Riot Attack":campaign_equipped_slot?"Assault Loop":"Glock Launch",position);}
    ...
    if(alt && campaign_equipped_slot==1) {
        float spread[3];status=rf_weapon_spread_ray(delta,campaign_pistol.alt_spread_degrees,&campaign_rifle_alt_random,spread);
    ...
    if(campaign_equipped_slot==2 && fire) {
        ...
        status=rf_geometry_collision_ray(stream->collision,&campaign_movers,position,end,0x26,&surface,&matched);if(status)return status;
        if(matched){combat_sound("Riot Impact Default",surface.hit.point);++rf_scene_riot[4];combat_surface_frame=frame;return RF_OK;}
    }
    if(target!=UINT32_MAX) {
...
static rf_attachment_node *campaign_attachment_lookup(void *context,uint32_t handle)
{
    uint32_t i;void *registered=rf_object_registry_lookup(&campaign_registry,handle);(void)context;
    ++rf_scene_attachments[2];if(!registered)return NULL;
    for(i=0;i<campaign_attachment_count;++i)if(campaign_attachment_nodes[i].owner==registered)return campaign_attachment_nodes+i;
    return NULL;
}
static int campaign_attachment_publish(void *context,rf_attachment_node *child,rf_attachment_node *parent)
{
    uint32_t index=(uint32_t)(child-campaign_attachment_nodes),p=(uint32_t)(parent-campaign_attachment_nodes),record[2];
    rf_glare_base_owner *owner;float pose[12];int status;(void)context;
    if(index<campaign_clutter_records.count || p>=campaign_clutter_records.count)return RF_FORMAT;
    index-=campaign_clutter_records.count;if(index>=campaign_glare_instance_count || !campaign_clutter_bodies[p])return RF_FORMAT;
    owner=campaign_glare_instances[index];if(!owner || owner->state.tag<0)return RF_FORMAT;
    status=rf_scene_clutter_tag_place(campaign_clutter_bodies[p]->state.handle,owner->state.tag,pose);if(status)return status;
    status=rf_glare_publish_tag_pose(owner,pose);if(status)return status;
    ++rf_scene_attachments[3];record[0]=owner->handle;record[1]=owner->state.parent;
...
static int campaign_attachments_pass(void)
{
    ...
    rf_attachment_backend backend={campaign_attachment_lookup,campaign_attachment_publish,NULL};
    if(!campaign_attachment_nodes && count) {
        uint64_t bytes=(uint64_t)count*(sizeof(rf_attachment_node)+sizeof(rf_attachment_node *));
        if(bytes>32768)return RF_RANGE;
        campaign_attachment_nodes=calloc(1,(size_t)bytes);if(!campaign_attachment_nodes)return RF_IO;
        campaign_attachment_count=count;campaign_attachment_scratch=(rf_attachment_node **)(campaign_attachment_nodes+count);
        memset(rf_scene_attachments,0,sizeof(rf_scene_attachments));rf_scene_attachments[5]=(uint32_t)bytes;rf_scene_attachments[6]=2166136261u;
    }
    if(count!=campaign_attachment_count)return RF_FORMAT;++rf_scene_attachments[0];
    for(i=0;i<count;++i) {
        rf_attachment_node *node=campaign_attachment_nodes+i;memset(node,0,sizeof(*node));
        if(i<campaign_clutter_records.count) {
            rf_clutter_base_owner *owner=campaign_clutter_bodies[i];if(!owner)continue;
    ...
    for(i=0;i<count;++i)if(campaign_attachment_nodes[i].flags) {
        ++rf_scene_attachments[1];status=rf_attachment_update(campaign_attachment_nodes+i,&backend,campaign_attachment_scratch,count);
        if(status){++rf_scene_attachments[7];return status;}
    }
    /* Generic487a40 room refresh follows attachment publication, retaining the
     * dirty parent flag until every child has consumed it. */
    for(i=0;i<campaign_clutter_records.count;++i)if(campaign_clutter_bodies[i]) {
        rf_clutter_base_owner *owner=campaign_clutter_bodies[i];rf_entity_room_state room;
        actor_room_context context={campaign_alpha_world,0,UINT32_MAX,0};
        if(!(owner->state.flags&0x04000000u) || (owner->state.flags&0x10000000u))continue;
        room.room=owner->state.first_word;room.flags=owner->state.flags;memcpy(room.query_position,owner->query_position,12);
        status=rf_entity_room_refresh(&room,owner->state.position,0,actor_room_locate,NULL,&context);
        if(status){++rf_scene_attachments[7];return status;}
        owner->state.first_word=room.room;owner->state.flags=room.flags;memcpy(owner->query_position,room.query_position,12);++rf_scene_attachments[4];
    }
    return RF_OK;
}
...
uint32_t rf_scene_npc_normal_test[4]; /* cases,blocked,hash,errors */
uint32_t rf_scene_npc_slow_test[4]; /* cases,blocked,hash,errors */
...
{
    npc_stand_test_context *c=context;rf_physics_ground_probe probe;rf_collision_actor_contact contact={0};uint32_t found,i;int status;
    status=rf_scene_npc_ground_query(c->world,handle,1.0f/60,&probe,&contact,&found);if(status)return status;
    ++c->calls;++rf_scene_npc_stand_test[3];
    if(c->reenter) {
        for(i=0;i<campaign_npc_body_count;++i)if(campaign_npc_bodies[i].registration.handle==handle)break;
        if(i==campaign_npc_body_count)return RF_NOT_FOUND;
        campaign_npc_bodies[i].view.flags_810|=0x400;
        campaign_npc_bodies[i].body.spheres.items[0].center[1]-=.125f;
    }
    return RF_OK;
}
static int campaign_npc_stand_fixture(const rf_geometry_collision_world *world,uint32_t frame)
{
    uint32_t i,j;int status=RF_OK;if(frame)return RF_OK;
    memset(rf_scene_npc_stand_test,0,sizeof(rf_scene_npc_stand_test));if(!rf_scene_actor_pair_test_enabled)return RF_OK;
    rf_scene_npc_stand_test[4]=2166136261u;
    memset(rf_scene_npc_normal_test,0,sizeof(rf_scene_npc_normal_test));rf_scene_npc_normal_test[2]=2166136261u;
    memset(rf_scene_npc_slow_test,0,sizeof(rf_scene_npc_slow_test));rf_scene_npc_slow_test[2]=2166136261u;
    for(i=0;i<campaign_npc_body_count && !status;++i)if(campaign_npc_bodies[i].registration.view && campaign_npc_bodies[i].body.spheres.count) {
        campaign_npc_body *owner=campaign_npc_bodies+i,saved=*owner;uint32_t cls=campaign_seeds.items[i].class_index,k;
        rf_physics_sphere spheres[8];rf_physics_stance_cache cache=campaign_npc_stances[cls];
        if(cache.count<owner->body.spheres.count) {
            int stood=99;npc_stand_test_context c={world,0,0};rf_scene_npc_stance_services services={NULL,npc_stand_test_ground,&c};
            if(rf_scene_npc_try_stand(world,owner->registration.handle,&services,&stood)!=RF_RANGE || stood!=99 || c.calls || memcmp(owner,&saved,sizeof(saved)))status=RF_FORMAT;
            else ++rf_scene_npc_stand_test[6];continue;
        }
        if(owner->body.spheres.count>8)return RF_RANGE;memcpy(spheres,owner->body.spheres.items,owner->body.spheres.count*sizeof(*spheres));
        for(j=0;j<3 && !status;++j) {
            rf_physics_body_state state;rf_collision_body_sphere scratch[8];rf_geometry_body_hit hit;
            uint32_t blocked=99,flags;float end[3];int stood=99;uint32_t record[4];
            npc_stand_test_context c={world,0,j==1};rf_scene_npc_stance_services services={NULL,npc_stand_test_ground,&c};
            campaign_npc_stances[cls].height_difference=j==2?16:cache.height_difference;
            *owner=saved;memcpy(owner->body.spheres.items,spheres,owner->body.spheres.count*sizeof(*spheres));
            status=rf_physics_stance_centers(&owner->body.spheres,cache.centers[1],cache.count,&owner->view.flags_810,1);if(status)break;
            owner->damage.effects.flags_810=owner->view.flags_810;
            state=owner->body.state;status=rf_physics_stand_endpoint(owner->published,campaign_npc_stances[cls].height_difference,end);if(status)break;
            memcpy(state.position,owner->published,12);memcpy(state.next_position,end,12);flags=state.state_124|4;if(state.bounds.radius<.05f)flags|=0x100;
            status=rf_scene_npc_body_sweep(world,owner->registration.handle,&state,flags,scratch,8,&hit,&blocked);if(status)break;
            if(rf_scene_npc_try_stand(world,owner->registration.handle^0x10000,&services,&stood)!=RF_NOT_FOUND || stood!=99 || c.calls){status=RF_FORMAT;break;}
            status=rf_scene_npc_try_stand(world,owner->registration.handle,&services,&stood);if(status)break;
            if(stood!=(blocked?0:1) || c.calls!=(blocked?0:1) || owner->damage.effects.flags_810!=owner->view.flags_810){status=RF_FORMAT;break;}
            for(k=0;k<owner->body.spheres.count && !status;++k) {
                float expected[3];memcpy(expected,cache.centers[blocked?1:0][k],12);if(!blocked && j==1 && !k)expected[1]-=.125f;
                if(memcmp(owner->body.spheres.items[k].center,expected,12))status=RF_FORMAT;
            }
            if(((owner->view.flags_810&0x400)!=0)!=(blocked || j==1)){status=RF_FORMAT;break;}
            record[0]=owner->registration.handle;record[1]=stood;record[2]=owner->view.flags_810;record[3]=c.calls;
            ++rf_scene_npc_stand_test[0];++rf_scene_npc_stand_test[blocked?2:1];
            rf_scene_npc_stand_test[4]=npc_hash_bytes(rf_scene_npc_stand_test[4],record,sizeof(record));
            if(campaign_seeds.classes[cls].physics.flags&1u) {
                campaign_npc_body expected=*owner,stand_result=*owner;rf_physics_sphere expected_spheres[8];uint32_t q=rf_scene_npc_stand_test[3];
                memcpy(expected_spheres,owner->body.spheres.items,owner->body.spheres.count*sizeof(*spheres));
                expected.previous_region_13ec=campaign_regions.items;
                if(!blocked) {
                    status=rf_movement_set_mode(&expected.movement,campaign_npc_movement_configs+cls,1,expected.attachment_75c,expected.body.state.mass,0);if(status)break;
                    expected.body.state.coefficients[1]=expected.movement.response;expected.body.state.velocity[1]=0;
                    expected.movement_slot=(campaign_modes[campaign_seeds.classes[cls].physics.movement_index].enabled&255u)?campaign_seeds.classes[cls].physics.movement_index:0;
                    expected.movement_orientation=campaign_identity[0];expected.previous_region_13ec=NULL;
                }
                *owner=saved;owner->previous_region_13ec=campaign_regions.items;memcpy(owner->body.spheres.items,spheres,owner->body.spheres.count*sizeof(*spheres));
                status=rf_physics_stance_centers(&owner->body.spheres,cache.centers[1],cache.count,&owner->view.flags_810,1);if(status)break;
                owner->damage.effects.flags_810=owner->view.flags_810;c.calls=0;
                status=rf_scene_npc_normal(world,owner->registration.handle,&services);rf_scene_npc_stand_test[3]=q;
                if(status || memcmp(owner,&expected,sizeof(expected)) || memcmp(owner->body.spheres.items,expected_spheres,owner->body.spheres.count*sizeof(*spheres)) || c.calls!=(blocked?0:1)) {status=RF_FORMAT;++rf_scene_npc_normal_test[3];break;}
                ++rf_scene_npc_normal_test[0];rf_scene_npc_normal_test[1]+=blocked!=0;
                rf_scene_npc_normal_test[2]=npc_hash_bytes(rf_scene_npc_normal_test[2],&owner->movement,sizeof(owner->movement));
                *owner=stand_result;
            }
            if(campaign_seeds.classes[cls].physics.flags&1u) {
                campaign_npc_body expected=*owner;rf_physics_sphere expected_spheres[8];uint32_t q=rf_scene_npc_stand_test[3];
                memcpy(expected_spheres,owner->body.spheres.items,owner->body.spheres.count*sizeof(*spheres));
                status=rf_movement_set_mode(&expected.movement,campaign_npc_movement_configs+cls,0,expected.attachment_75c,expected.body.state.mass,0);if(status)break;
                expected.body.state.coefficients[1]=expected.movement.response;expected.body.state.velocity[1]=0;
                expected.movement_slot=(campaign_modes[1].enabled&255u)?1:0;expected.movement_orientation=campaign_identity[0];
                *owner=saved;memcpy(owner->body.spheres.items,spheres,owner->body.spheres.count*sizeof(*spheres));
                status=rf_physics_stance_centers(&owner->body.spheres,cache.centers[1],cache.count,&owner->view.flags_810,1);if(status)break;
                owner->damage.effects.flags_810=owner->view.flags_810;c.calls=0;
                status=rf_scene_npc_slow(world,owner->registration.handle,0,&services);
                rf_scene_npc_stand_test[3]=q; /* Keep standalone stand telemetry scoped to its own calls. */
                if(status || memcmp(owner,&expected,sizeof(expected)) || memcmp(owner->body.spheres.items,expected_spheres,owner->body.spheres.count*sizeof(*spheres)) || c.calls!=(blocked?0:1)) {status=RF_FORMAT;++rf_scene_npc_slow_test[3];break;}
                ++rf_scene_npc_slow_test[0];rf_scene_npc_slow_test[1]+=blocked!=0;
                rf_scene_npc_slow_test[2]=npc_hash_bytes(rf_scene_npc_slow_test[2],&owner->movement,sizeof(owner->movement));
            }

        }
        campaign_npc_stances[cls].height_difference=cache.height_difference;
        *owner=saved;memcpy(owner->body.spheres.items,spheres,owner->body.spheres.count*sizeof(*spheres));
    }
    if(status)++rf_scene_npc_stand_test[5];return status;
}
...
static int campaign_npc_drive_fixture(uint32_t frame)
{
    ...
    for(i=0;i<campaign_npc_body_count && !status;++i)if(campaign_npc_bodies[i].registration.view){
        ...
        const rf_entity_rotation_values *r=&campaign_seeds.classes[cls].rotation;float saved_basis[9],saved_position[3];
        memcpy(saved_basis,campaign_model_owners[i].basis,36);memcpy(saved_position,campaign_model_owners[i].position,12);
        for(driven=0;driven<2 && !status;++driven){
            rf_angular_velocity_state expected;rf_angular_prediction prediction;
            *o=saved;memcpy(campaign_model_owners[i].basis,saved_basis,36);memcpy(campaign_model_owners[i].position,saved_position,12);o->body.state.flags&=~0x1000000u;
            ...
            if(!status){
                ...
                for(j=0;j<3;++j)o->body.state.next_position[j]=o->published[j]+(float)(j+1)*.125f;
                o->body.state.flags|=0x40000000u;
                ...
                if(!status){
                    rf_physics_body_state body=o->body.state;float published[3];uint32_t flags=o->object_flags;
                    memcpy(published,o->published,12);
                    status=rf_physics_publish_position(&body,published,&flags);
                    if(!status)status=rf_scene_npc_publish_position(o->registration.handle);
                    if(!status && (memcmp(&body,&o->body.state,sizeof(body)) || memcmp(published,o->published,12) ||
                        memcmp(published,campaign_model_owners[i].position,12) || flags!=o->object_flags || flags!=o->view.flags_7c))status=RF_FORMAT;
                    if(!status && rf_scene_npc_publish_position(o->registration.handle^0x10000)!=RF_NOT_FOUND)status=RF_FORMAT;
                    if(!status)rf_scene_npc_drive_test[2]=npc_hash_bytes(rf_scene_npc_drive_test[2],published,12);
                }
        ...
        *o=saved;memcpy(campaign_model_owners[i].basis,saved_basis,36);memcpy(campaign_model_owners[i].position,saved_position,12);
    }
...
static int campaign_npc_unholster_fixture(uint32_t frame)
{
    ...
    for(i=0;i<campaign_npc_body_count && !status;++i)if(campaign_npc_bodies[i].registration.view) {
        ...
        motion=owner->selection.mapping.actions[40];model=campaign_playback_resources.models+pose->skeleton;
        if(motion<0 || (uint32_t)motion>=model->count)continue;
        ...
        sound.handle=owner->registration.handle;sound.calls=0;sound.name=owner->selection.action_sounds[40];
...
static int campaign_npc_ai_reset_fixture(uint32_t frame)
{
    ...
    for(i=0;i<campaign_npc_body_count && !status;++i)if(campaign_npc_bodies[i].registration.view) {
        ...
        for(action=0;action<45;++action) {
            motion=owner->selection.mapping.actions[action];
        ...
        for(j=0;j<8 && !status;++j) {
            ...
            *owner=saved;pose->playback=kept;
            ...
            if(active) {
                wanted.completion.active.freeze_slot=-1;wanted.completion.frozen=0;wanted.completion.active.primary_slot=-1;
                if(model->resources[motion].looping==0)wanted.completion.active.slots[0].weight=0;
            }
...
static int campaign_npc_motion_request_fixture(uint32_t frame)
{
    ...
    for(i=0;i<campaign_npc_body_count && !status;++i)if(campaign_npc_bodies[i].registration.view) {
        ...
        for(j=0;j<4 && !status;++j) {
            wanted=pose->controller;status=rf_motion_request_state(&wanted,owner->selection.mapping.states,motions[j],.25f);if(status)break;
...
uint32_t rf_scene_script_routes[8]; /* attempts, found, misses, waypoint advances, last count, last index, status, centerline fallback successes */
...
{
    rf_entity_navigation_candidate start={0},goal={0};rf_entity_navigation_graph_request q={0};
    rf_collision_solid_view solid={0};rf_entity_navigation_selection begin,end;uint32_t i;float low=0,high=0;int status;
    *found=0;++rf_scene_script_routes[0];
    if(!campaign_navigation_workspace.storage || !owner->body.spheres.count){++rf_scene_script_routes[2];return RF_OK;}
    for(i=0;i<owner->body.spheres.count;i++) {
        const rf_physics_sphere *sphere=owner->body.spheres.items+i;
        if(sphere->radius>q.radius)q.radius=sphere->radius;
        if(!i || sphere->center[1]-sphere->radius<low)low=sphere->center[1]-sphere->radius;
    ...
    for(i=0;i<(owner->navigation_radius>0?1u:2u);i++) {
        ...
        if(begin.first==UINT32_MAX || end.first==UINT32_MAX)continue;
        ...
        if(*found && owner->navigation.retained.count>1){if(i)++rf_scene_script_routes[7];break;}
    }
...
static int campaign_script_step(scene_stream *stream,float elapsed)
{
    ...
    for(i=0;i<campaign_npc_body_count;i++) {
        ...
        if(o->script_move.follow==1) {
            ...
            float z=o->navigation.goal.position[2]-scene_actor_body.state.position[2];
            if(x*x+z*z>1){o->navigation.retained.count=0;o->script_move.retry=0;}
            memcpy(o->script_move.target,scene_actor_body.state.position,12);
        }
        const float *target=o->script_move.target;
        if(o->script_move.retry)--o->script_move.retry;
        if(!o->navigation.retained.count && !o->script_move.retry) {
            uint32_t found;status=campaign_script_route(stream,o,&found);rf_scene_script_routes[6]=(uint32_t)status;if(status)return status;
            if(!found)o->script_move.retry=60;
        }
        ...
        if(o->navigation.retained.count && o->script_move.route_index>=o->navigation.retained.count) {
            o->navigation.retained.count=0;o->script_move.retry=0;
        }
        ...
        if(blocked && fabsf(hit.contact.normal[1])<.5f) {
            ...
            float dx=proposal.next_position[0]-proposal.position[0];
            float dz=proposal.next_position[2]-proposal.position[2];
            float inward=dx*nx+dz*nz;
            if(length>.0001f && inward<0) {
                dx-=nx*inward/length;dz-=nz*inward/length;
                if(dx*dx+dz*dz>.00000001f) {
                    proposal.next_position[0]=proposal.position[0]+dx;
                    proposal.next_position[1]=proposal.position[1];
                    proposal.next_position[2]=proposal.position[2]+dz;
                    status=rf_physics_body_prepare_sweep(&proposal);if(status)return status;
                    status=rf_scene_npc_body_sweep(stream->collision,o->registration.handle,&proposal,0x460,scratch,8,&hit,&blocked);if(status)return status;
                }
            }
...
uint32_t rf_scene_clutter_render_dispatch[6];
typedef struct scene_clutter_render_context {
    scene_stream *stream;uint32_t placement;
    rf_model_render_buffers *buffers;rf_model_lighting *lights;rf_model_render_output *attributes;
    rf_model_clip_planes *planes;rf_model_clip_projection *projection;
} scene_clutter_render_context;
static int scene_clutter_render_white(void *context)
{
    scene_clutter_render_context *c=context;rf_model_render_output white={1,{255,255,255},255,1,1};
    *c->attributes=white;return RF_OK;
}
static int scene_clutter_render_kind(void *context,uint32_t model,uint32_t *kind)
{
    scene_clutter_render_context *c=context;rf_glare_visibility_object view;int status;
    if(model!=c->placement+1)return RF_RANGE;
    status=rf_scene_clutter_visibility_view(campaign_clutter_bodies[c->placement]->state.handle,&view);if(status)return status;
    *kind=1;return RF_OK; /* Retained static model wrapper. */
}
static int scene_clutter_render_prepare(void *context,uint32_t model)
{(void)context;(void)model;return RF_NOT_FOUND;} /* Kind3 is not a static owner. */
static int scene_clutter_render_family(void *context,uint32_t kind)
{
...
static int scene_clutter_draw(scene_stream *stream,uint32_t frame)
{
    ...
    rf_model_render_output attributes={1,{255,255,255},255,1,1};
    rf_model_clip_planes planes={0};rf_model_clip_projection projection={0};
    uint32_t i,start_all=stream->mesh->count;int status;
    memset(rf_scene_clutter_draw,0,sizeof(rf_scene_clutter_draw));rf_scene_clutter_draw[0]=frame+1;
    if(!frame){memset(rf_scene_clutter_render_dispatch,0,sizeof(rf_scene_clutter_render_dispatch));rf_scene_clutter_render_dispatch[4]=2166136261u;}
    ++rf_scene_clutter_render_dispatch[0];
    ...
    for(i=0;i<campaign_clutter_records.count;++i) {
        ...
        uint32_t model=campaign_clutter_model_slots[i],room,before;
        scene_clutter_render_context context={stream,i,&buffers,&lights,&attributes,&planes,&projection};
        rf_object_render_backend backend={scene_clutter_render_white,scene_clutter_render_kind,
            scene_clutter_render_prepare,scene_clutter_render_family,&context};
        ...
        before=owner->state.flags;++rf_scene_clutter_render_dispatch[1];
        status=rf_object_render_dispatch(&owner->state.flags,4,i+1,&backend);
        if(status){++rf_scene_clutter_render_dispatch[5];return status;}
        if(before&(2|0x4000))++rf_scene_clutter_render_dispatch[3];else ++rf_scene_clutter_render_dispatch[2];
        rf_scene_clutter_render_dispatch[4]=npc_hash_bytes(rf_scene_clutter_render_dispatch[4],&owner->state.handle,4);
...
static int scene_player_weapon_draw(scene_stream *stream,uint32_t frame)
{
    ...
    if(!frame || stream->player_slot!=campaign_equipped_slot)request=0;
        ...
        else request=campaign_equipped_slot==3 && campaign_last_alt?3:1;
    }
...
static int scene_frame(void *context,uint32_t frame,rf_preview_mesh *actor)
{
    ...
    stream->mesh->bytes=rf_scene_actor_eye_enabled?stream->world*sizeof(rf_preview_vertex):(uint32_t)bytes;
    {
        ...
        if(stream->collision && frame+1<rf_scene_actor_frame_count) {
            ...
            step_profile_mark(2,&step_clock);
            if(campaign_spawn) {
            ...
            step_profile_mark(3,&step_clock);
            if(stream->particles.state) {
    ...
    rf_scene_frame_sink sink,void *context,int state_mode,const rf_geometry_collision_world *collision,const rf_geometry *geometry)
{
    ...
    if(sink) {
        ...
        if(!status && collision && campaign_spawn) {
            rf_scene_campaign_load_stage=1;status=rf_entity_materials_open(&campaign_npc_materials,&campaign_appearances,&campaign_render_models,maps,map_count,4*1024*1024);
            ...
            if(status==RF_RANGE)status=rf_entity_materials_open_limit(&campaign_npc_materials,&campaign_appearances,&campaign_render_models,maps,map_count,4*1024*1024,64);
        }
        ...
        if(campaign_spawn && collision) {
            ...
            if(status==RF_NOT_FOUND)status=RF_OK;if(status)goto done;
            rf_scene_campaign_load_stage=5;status=rf_physics_forces_open(level,65536,&campaign_forces);if(status)goto done;
            ...
            if(status==RF_NOT_FOUND)status=RF_OK;if(status)goto done;
            rf_scene_campaign_load_stage=6;status=rf_group_runtime_open(&campaign_groups,0,256*1024,&campaign_group_runtime);
            if(status)goto done;
            rf_scene_campaign_load_stage=7;status=rf_group_registration_open(&campaign_group_runtime,&campaign_registry,65536,&campaign_group_registration);
            if(status)goto done;
            ...
            rf_scene_campaign_load_stage=8;status=campaign_open_movers(&actor_follow_world->movers);if(status)goto done;
            ...
            rf_scene_campaign_load_stage=9;status=campaign_alpha_open(collision,materials);if(status)goto done;
            ...
            rf_scene_campaign_load_stage=15;status=campaign_glare_resources_open(tables_path,maps,map_count);if(status)goto done;
            rf_scene_campaign_load_stage=16;status=campaign_clutter_bodies_open(collision);if(status)goto done;
    ...
    if(state_mode || campaign_spawn) {
        ...
        if(campaign_spawn) {
            ...
            rf_scene_campaign_load_stage=22;status=rf_entity_poses_start_initial(&campaign_seeds,&campaign_skeletons,&campaign_motion_catalog,
            ...
            rf_scene_campaign_load_stage=25;status=campaign_weapon_models_open(tables_path,&archive);if(status)goto done;
            ...
            {rf_vpp tables={0};rf_weapon_view_definition view;
             status=rf_vpp_open(&tables,tables_path);if(status)goto done;
             for(i=0;i<scene_weapon_slots() && !status;i++) {
            ...
            rf_scene_campaign_load_stage=31;status=campaign_npc_geometry_digest();if(status)goto done;
            {
    ...
    rf_scene_campaign_load_stage=32;status=rf_model_materials_open_skin(&bundle,&model,names,binding.assets.texture_count,maps,map_count,
    ...
    rf_scene_campaign_load_stage=33;
    count=(uint64_t)materials->count+bundle.textures.count;
    if(capacity>mesh_budget || capacity>SIZE_MAX || bytes>capacity || count>RF_CAMPAIGN_TEXTURE_SLOTS) {status=RF_RANGE;goto done;}
    for(i=0;i<actor.count;++i) {
